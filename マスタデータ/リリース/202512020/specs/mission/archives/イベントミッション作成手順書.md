...
### Step 3: ミッション多言語設定を作成（MstMissionEventI18n.csv）

| カラム | 設定値 | 例 |
|--------|--------|-----|
| ENABLE | `e` | e |
| id | 連番 | mission_event_i18n_001 |
| release_key | リリースキー | 202512020 |
| mst_mission_event_id | Step1のid | event_osh_00001_1 |
| language | 言語コード | ja |
| description | ミッション説明文 | ステージを5回クリアしよう |

**ポイント**:
- **mst_mission_event_id**: Step1で作成したMstMissionEvent.csvのidを設定
...
**ほとんどのイベントミッションでは前提条件を設定しません**。その場合、以下のように設定します：
...
#### 例1: 特定クエストクリア後に解放されるミッション

**シナリオ**: 「収集クエスト『芸能界へ！』をクリアしたら、次のミッションが解放される」

**MstMissionEvent設定**:
- unlock_criterion_type: `SpecificQuestClear`
- unlock_criterion_value: `quest_event_osh1_charaget01`
- unlock_criterion_count: `1`

#### 例2: ステージクリア回数到達後に解放されるミッション

**シナリオ**: 「ステージを50回クリアしたら、高難易度ミッションが解放される」

**MstMissionEvent設定**:
- unlock_criterion_type: `StageClearCount`
- unlock_criterion_value: （空欄または__NULL__）
- unlock_criterion_count: `50`

#### 例3: 前提条件なし（通常パターン）

**シナリオ**: 「イベント開始時から挑戦可能なミッション」
...
**MstMissionEventI18n**:
- mst_mission_event_id: （MstMissionEventのid）
- language: `ja`
- description: `ステージを5回クリアしよう`
...
- mst_mission_event_id: （MstMissionEventのid）
- language: `ja`
- description: `【汗が輝いてるよ!】ぴえヨンを編成に入れて「ファンと推し合戦！」を1回クリア`
...
- mst_mission_event_id: （MstMissionEventのid）
- language: `ja`
- description: `【胸筋の登山始まってるよ!】収集クエスト「芸能界へ！」をクリアしよう`
...
### MstMissionEventI18n
- [ ] i18nレコード数がMstMissionEventのレコード数と一致しているか（1ミッション = 1レコード）
- [ ] mst_mission_event_idが正しいミッションIDを参照しているか
...
- **ほとんどのイベントミッションでは前提条件を設定しない**（unlock_criterion_type=`__NULL__`）
