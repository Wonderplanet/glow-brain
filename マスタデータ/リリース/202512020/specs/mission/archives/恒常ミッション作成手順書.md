...
3. **MstMissionAchievementI18n.csv** - アチーブメントミッション多言語設定
...
### Step 3: ミッション多言語設定を作成（MstMissionAchievementI18n.csv）

| カラム | 設定値 | 例 |
|--------|--------|-----|
| ENABLE | `e` | e |
| id | 連番 | mission_achievement_i18n_101 |
| mst_mission_achievement_id | Step1のid | achievement_2_101 |
| language | 言語コード | ja |
| description | ミッション説明文 | メインクエスト「【推しの子】」の難易度ノーマルをクリアしよう |
| release_key | リリースキー | 202512020 |

**ポイント**:
- **mst_mission_achievement_id**: Step1で作成したMstMissionAchievement.csvのidを設定
- **language**: 現在は `ja`（日本語）のみサポート
- **description**: 運営仕様書に記載されているミッション内容をそのまま記載
- **1ミッションにつき1レコード**作成（現状は日本語のみなので1対1）
...
| **unlock_criterion_value** | 前提条件の値 | quest_main_osh_normal_16 |
| **unlock_criterion_count** | 前提条件の回数 | 1 |

### unlock_criterionに設定可能なタイプ一覧

**アチーブメントで使用する主なタイプ**:

| unlock_criterion_type | 説明 | unlock_criterion_value | unlock_criterion_count | 用途例 |
|---------------------|------|----------------------|---------------------|--------|
| **SpecificQuestClear** | 指定クエスト初クリア | クエストID | 1 | 前のクエストをクリアしたら解放 |
...
**ほとんどのアチーブメントでは前提条件を設定しません**。その場合、以下のように設定します：
...
#### 例1: 前のメインクエストクリア後に解放されるアチーブメント

**シナリオ**: 「メインクエスト17ノーマルは、メインクエスト16ノーマルをクリアしたら解放される」

**MstMissionAchievement設定**:
- unlock_criterion_type: `SpecificQuestClear`
- unlock_criterion_value: `quest_main_osh_normal_16`
- unlock_criterion_count: `1`

#### 例2: プレイヤーランク到達後に解放されるアチーブメント

**シナリオ**: 「プレイヤーレベル20に到達したら、高難易度ミッションが解放される」

**MstMissionAchievement設定**:
- unlock_criterion_type: `UserLevel`
- unlock_criterion_value: （空欄または__NULL__）
- unlock_criterion_count: `20`

#### 例3: 前提条件なし（通常パターン）

**シナリオ**: 「最初から挑戦可能なメインクエストクリアミッション」
...
#### MstMissionAchievementI18n

| id | mst_mission_achievement_id | language | description |
|----|---------------------------|----------|-------------|
| mission_achievement_i18n_101 | achievement_2_101 | ja | メインクエスト「【推しの子】」の難易度ノーマルをクリアしよう |
| mission_achievement_i18n_102 | achievement_2_102 | ja | メインクエスト「【推しの子】」の難易度ハードをクリアしよう |
| mission_achievement_i18n_103 | achievement_2_103 | ja | メインクエスト「【推しの子】」の難易度エクストラをクリアしよう |

**作成レコード数**:
- MstMissionAchievement: 3レコード
- MstMissionReward: 3レコード
- MstMissionAchievementI18n: 3レコード
...
### MstMissionAchievementI18n
- [ ] i18nレコード数がMstMissionAchievementのレコード数と一致しているか（1ミッション = 1レコード）
- [ ] mst_mission_achievement_idが正しいミッションIDを参照しているか
...
- アチーブメントでよく使う値: QuestSelect, StageSelect, Home, UnitList
- クライアント実装で定義されていない値を設定するとエラーになる
- ミッションの目的に合った遷移先を選択する（例：クエストミッション→QuestSelect、育成ミッション→UnitList）
...
- **ほとんどのアチーブメントでは前提条件を設定しない**（unlock_criterion_type=`__NULL__`）
...
- **アチーブメントで前提条件が必要な例**: 「前のメインクエストをクリアしないと次のミッションが表示されない」など
