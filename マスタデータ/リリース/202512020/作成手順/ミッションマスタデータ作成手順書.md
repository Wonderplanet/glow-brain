...
**設定項目:**
- `group_id`: 報酬グループID
- `resource_type`: 報酬の種類(FreeDiamond, Coin, Item, Emblемなど)
...
- `resource_type`が`FreeDiamond`, `PaidDiamond`, `Coin`の場合はチェック不要(システムリソース)
...
| resource_type | 確認すべきマスタデータ | ファイル名 |
|--------------|---------------------|-----------|
| Item | MstItem | `MstItem.csv` |
| Emblem | MstEmblem | `MstEmblem.csv` |
| Unit | MstUnit | `MstUnit.csv` |
| FreeDiamond | (チェック不要) | - |
| PaidDiamond | (チェック不要) | - |
| Coin | (チェック不要) | - |
...
| resource_type | string | ○ | リソースタイプ | `Item`, `FreeDiamond`, `Coin`, `Emblem` |
...
**resource_id:**
- `resource_type`が`FreeDiamond`または`Coin`の場合は空文字
- それ以外の場合は対応するマスタデータのID
...
- [ ] resource_idで指定したリソースが実際に存在するか確認したか
