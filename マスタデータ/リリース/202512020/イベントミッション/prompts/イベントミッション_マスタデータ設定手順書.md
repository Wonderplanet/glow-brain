...
イベントミッションでは、以下の4シート構成でマスタデータを作成します。

**出力シート**:
- **MstMissionEvent** - イベントミッションの基本情報
- **MstMissionEventI18n** - イベントミッション説明文の多言語対応
- **MstMissionEventDependency** - ミッション間の解放順序
- **MstMissionReward** - ミッション報酬

**重要**: 降臨バトルミッションと異なり、I18n情報は独立したテーブルとして管理されます。
...
### 4. MstMissionEventI18n シートの作成

#### 4.1 シートスキーマ

このシートは、MstMissionEventとは**独立した別ファイル**として出力します。
...
| **id** | I18nレコードの一意識別子。命名規則: `{mst_mission_event_id}_{言語コード}` |
...
**重要**:
- MstMissionEventの各レコードに対応するI18nレコードを必ず作成する
- `mst_mission_event_id`が正しく対応していることを確認する

### 5. MstMissionEventDependency シートの作成
...
最終的な出力は以下の4シート構成で行います。

#### 8.1 MstMissionEvent シート
...
#### 8.2 MstMissionEventI18n シート（独立ファイル）
...
#### 8.3 MstMissionEventDependency シート
...
- **TABLE行は必須**: 各シートの先頭にTABLE行を含める（I18nシートを除く）
- **4シート構成**: MstMissionEvent, MstMissionEventI18n, MstMissionEventDependency, MstMissionRewardの4シート
- **I18nは独立ファイル**: description.jaはMstMissionEventに含まれるが、I18nテーブルとしても独立して出力する
...
**症状**: ゲーム内でミッション説明が表示されない

**原因**: MstMissionEventI18nにレコードが作成されていない

**対処**: すべてのMstMissionEventレコードに対応するI18nレコードを作成する
...
- `released/MstMissionEventI18n.csv` - イベントミッション説明文
...
| 項目 | 降臨バトルミッション | イベントミッション |
|-----|-------------------|------------------|
| 基本テーブル | MstMissionLimitedTerm | MstMissionEvent |
| I18nテーブル | 統合（description.jaカラム） | 独立（MstMissionEventI18n） |
| 依存関係テーブル | なし | MstMissionEventDependency |
| 報酬テーブル | MstMissionReward | MstMissionReward |

### 主な違い

1. **I18n管理**:
   - 降臨バトル: description.jaカラムで管理
   - イベント: 独立したMstMissionEventI18nテーブル

2. **依存関係**:
   - 降臨バトル: progress_group_keyで進捗共有
   - イベント: MstMissionEventDependencyで解放順序管理

3. **イベント紐付け**:
   - 降臨バトル: イベント紐付けなし
   - イベント: mst_event_idで特定イベントに紐付け

4. **解放条件**:
   - 降臨バトル: なし（すべて最初から解放）
   - イベント: unlock_criterion + Dependencyで制御
