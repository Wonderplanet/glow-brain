...
**出力シート**:
- **MstMissionAchievement** - アチーブメントミッションの基本情報
- **MstMissionAchievementI18n** - ミッション説明文の多言語対応（独立シート）
- **MstMissionAchievementDependency** - ミッション間の解放順序の依存関係
- **MstMissionReward** - ミッション報酬

**重要**: MstMissionAchievementI18nは独立したシートとして作成します（イベントミッションや降臨バトルミッションとは異なります）。
...
### 2. MstMissionAchievement シートの作成
...
```
TABLE,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement,MstMissionAchievement
```
...
| **group_key** | グループキー。**アチーブメントミッションでは使用する**。例: `group1`, `group2`... |
| **mst_mission_reward_group_id** | 報酬グループID。MstMissionRewardのgroup_idと対応。命名規則: `achievement_{グループ番号}_{連番}` |
| **sort_order** | 表示順序。小さい数字ほど上位に表示。例: `1`, `2`, `3`... |
| **destination_scene** | タップ時の遷移先シーン。下記の「destination_scene設定ルール」を参照 |

#### criterion_type設定一覧

アチーブメントミッションで使用可能なcriterion_typeは以下の通りです。**大文字小文字を正確に一致**させてください。
...
| **DefeatEnemyCount** | インゲームで敵を撃破 | 空欄（`__NULL__`） | 撃破して欲しい敵数 | StageSelect | 敵を100体撃破しよう |
...
| **DefeatBossEnemyCount** | インゲームで強敵（ボス）を撃破 | 空欄（`__NULL__`） | 撃破して欲しい敵数 | StageSelect | ボスを10体撃破しよう |

**その他のcriterion_type**:

| criterion_type | 説明 | criterion_value | criterion_count | destination_scene候補 | 使用例 |
|---------------|------|----------------|----------------|---------------------|--------|
| **SpecificStageClearCount** | 指定ステージをクリア | ステージID (mst_stages.id)<br>例: `event_glo1_1day_00001` | クリアしてほしい回数 | QuestSelect / Event | デイリークエスト「開運！ジャンブル運試し」を3回クリア |
| **SpecificStageChallengeCount** | 指定ステージに挑戦 | ステージID (mst_stages.id) | 挑戦してほしい回数 | QuestSelect / Event | デイリークエスト「開運！ジャンブル運試し」に5回挑戦 |
| **QuestClearCount** | 通算クエストクリア回数 | 空欄（`__NULL__`） | クリアして欲しいクエスト数 | QuestSelect | クエストを合計20回クリアしよう |
| **StageClearCount** | 通算ステージクリア回数 | 空欄（`__NULL__`） | ステージをクリアして欲しい回数 | StageSelect | ステージを合計50回クリアしよう |
...
| **ReviewCompleted** | ストアレビューを記載 | 空欄（`__NULL__`） | 固定値: `1` | Web | ストアレビューを投稿しよう |
...
| **SpecificSeriesEnemyDiscoveryCount** | 指定作品の敵キャラを発見 | 作品ID (mst_series.id) | 発見してほしいエネミーの種類数 | StageSelect | 【推しの子】の敵キャラを3種類発見しよう |
| **EnemyDiscoveryCount** | 敵キャラを発見 | 空欄（`__NULL__`） | 発見してほしいエネミーの種類数 | StageSelect | 敵キャラを10種類発見しよう |
| **SpecificEnemyDiscoveryCount** | 敵キャラXを発見 | エネミーID (mst_enemy_characters.id) | 発見してほしい体数 | StageSelect | 特定の敵キャラを5体発見しよう |
...
| **AccessWeb** | 指定のWEBサイトにアクセス | リンク先のURL | 固定値: `1` | Web | 「ジャンブルラッシュ情報局」を確認しよう |
...
| **SpecificItemCollect** | 指定アイテムを収集 | アイテムID (mst_items.id)<br>例: `item_glo_00001` | 集めて欲しいアイテム個数 | Home | いいジャンメダル【赤】を200個集めよう |
...
| **FollowCompleted** | 公式X（エックス）をフォロー | 空欄（`__NULL__`） | 固定値: `1` | Web | 公式Xをフォローしよう |
| **AccountCompleted** | アカウント連携を行う | 空欄（`__NULL__`） | 固定値: `1` | LinkBnId | アカウント連携を完了しよう |
...
- **group_key**: **アチーブメントミッションでは使用する**。依存関係のあるミッショングループで同じキーを設定
...
**重要な注意点:**
- group_keyは依存関係のあるミッションで同じ値を設定（例: `group1`, `group2`）
...
### 3. MstMissionAchievementI18n シートの作成
...
```
TABLE,MstMissionAchievementI18n,MstMissionAchievementI18n,MstMissionAchievementI18n,MstMissionAchievementI18n,MstMissionAchievementI18n
```
...
### 4. MstMissionAchievementDependency シートの作成
...
- [ ] **リレーションの整合性**
  ...
  - すべてのミッションにMstMissionAchievementI18n.description（説明文）が設定されている
...
#### 7.2 MstMissionAchievementI18n シート
...
#### 7.3 MstMissionAchievementDependency シート
...
- **3シート構成**: MstMissionAchievement、MstMissionAchievementI18n、MstMissionAchievementDependency、MstMissionRewardの4シート
- **I18nは独立シート**: description情報は別シート（MstMissionAchievementI18n）で管理
