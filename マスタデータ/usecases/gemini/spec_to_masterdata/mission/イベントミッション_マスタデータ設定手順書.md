...
|---------------|------|----------------|----------------|--------|
| **SpecificItemCollect** | 指定アイテムを収集 | アイテムID (mst_items.id)<br>例: `item_glo_00001` | 集めて欲しいアイテム個数 | いいジャンメダル【赤】を200個集めよう |
| **DefeatEnemyCount** | インゲームで敵を撃破 | 空欄(NULL) | 撃破して欲しい敵数 | 敵を100体撃破しよう |
| **DefeatBossEnemyCount** | インゲームで強敵(ボス)を撃破 | 空欄(NULL) | 撃破して欲しい敵数 | ボスを10体撃破しよう |
| **SpecificGachaDrawCount** | 特定ガシャを引く | ガシャID (opr_gachas.id)<br>例: `gasho_001` | ガシャを引いて欲しい回数 | 賀正ガシャ2026を10回引こう |
| **GachaDrawCount** | 通算でガチャを引く | 空欄(NULL) | 通算でガシャを引いて欲しい回数 | ガシャを合計50回引こう |
| **OutpostEnhanceCount** | ゲートを強化 | 空欄(NULL) | ゲートを強化してほしい回数 | ゲートを5回強化しよう |
| **SpecificOutpostEnhanceLevel** | 指定したゲート強化項目がレベル到達 | 強化項目ID (mst_outpost_enhancements.id) | 到達してほしいレベル | ゲート強化「体力増強」をLv.5に到達 |
| **IaaCount** | 広告視聴 | 空欄(NULL) | 広告視聴してほしい回数 | 広告を3回視聴しよう |
| **FollowCompleted** | 公式X(エックス)をフォロー | 空欄(NULL) | 固定値: `1` | 公式Xをフォローしよう |
| **AccountCompleted** | アカウント連携を行う | 空欄(NULL) | 固定値: `1` | アカウント連携を完了しよう |
| **SpecificQuestClear** | 特定クエストを初クリア | クエストID (mst_quests.id)<br>例: `quest_event_osh1_charaget01` | 固定値: `1`<br>※2以上は設定不可 | 収集クエスト「芸能界へ!」をクリアしよう |
| **SpecificStageClearCount** | 指定ステージをクリア | ステージID (mst_stages.id)<br>例: `event_glo1_1day_00001` | クリアしてほしい回数 | デイリークエスト「開運!ジャンブル運試し」を3回クリア |
| **SpecificStageChallengeCount** | 指定ステージに挑戦 | ステージID (mst_stages.id) | 挑戦してほしい回数 | デイリークエスト「開運!ジャンブル運試し」に5回挑戦 |
| **QuestClearCount** | 通算クエストクリア回数 | 空欄(NULL) | クリアして欲しいクエスト数 | クエストを合計20回クリアしよう |
| **StageClearCount** | 通算ステージクリア回数 | 空欄(NULL) | ステージをクリアして欲しい回数 | ステージを合計50回クリアしよう |
| **SpecificUnitStageClearCount** | 特定ユニット編成でステージクリア | ユニットID.ステージID<br>例: `chara_osh_00601.event_osh1_1day_00001` | クリアして欲しい回数 | ぴえヨンを編成に入れて「ファンと推し合戦!」を5回クリア |
| **SpecificUnitStageChallengeCount** | 特定ユニット編成でステージ挑戦 | ユニットID.ステージID<br>例: `chara_osh_00601.event_osh1_challenge01_00001` | 挑戦して欲しい回数 | ぴえヨンを編成に入れて「推しの子になってやる」1話を1回挑戦 |
| **MissionClearCount** | ミッションをクリア | 空欄(NULL) | 同じミッションタイプの内でクリアして欲しい数 | ミッションを5個クリアしよう |
| **SpecificMissionClearCount** | 指定したミッショングループをクリア | group_key | 同じgroup_key内でクリアして欲しい数 | 指定グループのミッションを全てクリアしよう |
| **MissionBonusPoint** | ミッションボーナスポイントを集める | 空欄(NULL) | 集めて欲しいポイント数 | ミッションボーナスポイントを100集めよう |
| **UserLevel** | プレイヤーレベル到達 | 空欄(NULL) | 到達してほしいレベル | プレイヤーレベル10に到達しよう |
| **CoinCollect** | コインを集める | 空欄(NULL) | 集めてほしいコイン数 | コインを10000枚集めよう |
| **CoinUsedCount** | コインを使用 | 空欄(NULL) | 使用してほしいコイン数 | コインを5000枚使用しよう |
| **UnitLevelUpCount** | ユニットのレベルアップ | 空欄(NULL) | レベルアップを実行してほしい回数 | ユニットのレベルアップを10回実行しよう |
| **UnitLevel** | 全ユニットの内でいずれかがレベル到達 | 空欄(NULL) | 到達して欲しいレベル | いずれかのユニットをLv.30に到達させよう |
| **SpecificUnitLevel** | 指定ユニットがレベル到達 | ユニットID (mst_units.id)<br>例: `chara_osh_00601` | 到達して欲しいレベル | ぴえヨンをLv.50に到達させよう |
| **SpecificUnitRankUpCount** | 指定ユニットのランクアップ回数 | ユニットID (mst_units.id) | ランクアップをしてほしい回数 | ぴえヨンのランクアップを3回実行しよう |
| **SpecificUnitGradeUpCount** | 指定ユニットのグレードアップ回数 | ユニットID (mst_units.id) | グレードアップをしてほしい回数 | ぴえヨンのグレードアップを2回実行しよう |
| **ReviewCompleted** | ストアレビューを記載 | 空欄(NULL) | 固定値: `1` | ストアレビューを投稿しよう |
| **LoginCount** | 通算ログイン日数 | 空欄(NULL) | 通算ログインして欲しい日数 | 通算ログイン7日目 |
| **LoginContinueCount** | 連続ログイン日数 | 空欄(NULL) | 連続ログインして欲しい日数 | 連続ログイン3日目 |
| **IdleIncentiveQuickCount** | クイック探索 | 空欄(NULL) | クイック探索をして欲しい回数 | クイック探索を5回実行しよう |
| **IdleIncentiveCount** | 探索 | 空欄(NULL) | 探索をして欲しい回数 | 探索を10回実行しよう |
| **SpecificSeriesUnitAcquiredCount** | 特定作品のユニットを獲得 | 作品ID (mst_series.id) | 獲得してほしいユニットの種類数 | 【推しの子】のユニットを5種類獲得しよう |
| **UnitAcquiredCount** | ユニットを入手 | 空欄(NULL) | 入手して欲しいユニット体数 | ユニットを10体入手しよう |
| **SpecificUnitAcquiredCount** | 指定ユニットを獲得 | ユニットID (mst_units.id) | 獲得してほしい体数 | ぴえヨンを1体獲得しよう |
| **SpecificSeriesEnemyDiscoveryCount** | 指定作品の敵キャラを発見 | 作品ID (mst_series.id) | 発見してほしいエネミーの種類数 | 【推しの子】の敵キャラを3種類発見しよう |
| **EnemyDiscoveryCount** | 敵キャラを発見 | 空欄(NULL) | 発見してほしいエネミーの種類数 | 敵キャラを10種類発見しよう |
| **SpecificEnemyDiscoveryCount** | 敵キャラXを発見 | エネミーID (mst_enemy_characters.id) | 発見してほしい体数 | 特定の敵キャラを5体発見しよう |
| **SpecificSeriesEmblemAcquiredCount** | 指定作品のエンブレムを獲得 | 作品ID (mst_series.id) | 獲得してほしいエンブレムの種類数 | 【推しの子】のエンブレムを3種類獲得しよう |
| **EmblemAcquiredCount** | エンブレムを獲得 | 空欄(NULL) | 獲得してほしいエンブレムの種類数 | エンブレムを10種類獲得しよう |
| **SpecificEmblemAcquiredCount** | 指定エンブレムを獲得 | エンブレムID (mst_emblems.id) | 獲得してほしい個数 | 特定のエンブレムを1個獲得しよう |
| **SpecificSeriesArtworkCompletedCount** | 指定作品の原画を完成 | 作品ID (mst_series.id) | 完成させてほしい原画数 | 【推しの子】の原画を5つ完成させよう |
| **ArtworkCompletedCount** | 原画を完成 | 空欄(NULL) | 完成させてほしい原画数 | 原画を10つ完成させよう |
| **SpecificArtworkCompletedCount** | 指定原画を完成 | 原画ID (mst_artworks.id) | 固定値: `1` (原画完成は生涯1回のみ) | 特定の原画を1つ完成させよう |
| **PvpChallengeCount** | 決闘に挑戦 | 空欄(NULL) | 挑戦してほしい回数 | 決闘に3回挑戦しよう |
| **PvpWinCount** | 決闘に勝利 | 空欄(NULL) | 勝利してほしい回数 | 決闘に5回勝利しよう |
| **AccessWeb** | 指定のWEBサイトにアクセス | リンク先のURL | 固定値: `1` | 「ジャンブルラッシュ情報局」を確認しよう |
...
- **destination_scene**: ミッションに関連するシーン(例: `Event`, `Gacha`, `UnitList`)
...
osh_00001_event_reward_1    (推しの子 イベント1 報酬段階1) ※例外
osh_00001_event_reward_53   (推しの子 イベント1 報酬段階53) ※例外
glo_00001_event_reward_01   (GLOW全体 イベント1 報酬段階1)
```

**重要な注意点:**
- 新規作品では**通常ルール(2桁ゼロパディング)を推奨**
- 推しの子(osh)の例外パターンは既存データとの整合性のためのみ使用
