...
| **DefeatEnemyCount** | インゲームで敵を撃破 | 空欄(`__NULL__`) | 撃破して欲しい敵数 | 敵を100体撃破しよう |
| **DefeatBossEnemyCount** | インゲームで強敵(ボス)を撃破 | 空欄(`__NULL__`) | 撃破して欲しい敵数 | ボスを10体撃破しよう |
| **SpecificGachaDrawCount** | 特定ガシャを引く | ガシャID (opr_gachas.id)<br>例: `gasho_001` | ガシャを引いて欲しい回数 | 賀正ガシャ2026を10回引こう |
| **GachaDrawCount** | 通算でガチャを引く | 空欄(`__NULL__`) | 通算でガシャを引いて欲しい回数 | ガシャを合計50回引こう |
| **OutpostEnhanceCount** | ゲートを強化 | 空欄(`__NULL__`) | ゲートを強化してほしい回数 | ゲートを5回強化しよう |
| **SpecificOutpostEnhanceLevel** | 指定したゲート強化項目がレベル到達 | 強化項目ID (mst_outpost_enhancements.id) | 到達してほしいレベル | ゲート強化「体力増強」をLv.5に到達 |
| **IaaCount** | 広告視聴 | 空欄(`__NULL__`) | 広告視聴してほしい回数 | 広告を3回視聴しよう |
| **FollowCompleted** | 公式X(エックス)をフォロー | 空欄(`__NULL__`) | 固定値: `1` | 公式Xをフォローしよう |
| **AccountCompleted** | アカウント連携を行う | 空欄(`__NULL__`) | 固定値: `1` | アカウント連携を完了しよう |
...
| **QuestClearCount** | 通算クエストクリア回数 | 空欄(`__NULL__`) | クリアして欲しいクエスト数 | クエストを合計20回クリアしよう |
| **StageClearCount** | 通算ステージクリア回数 | 空欄(`__NULL__`) | ステージをクリアして欲しい回数 | ステージを合計50回クリアしよう |
...
| **MissionClearCount** | ミッションをクリア | 空欄(`__NULL__`) | 同じミッションタイプの内でクリアして欲しい数 | ミッションを5個クリアしよう |
| **SpecificMissionClearCount** | 指定したミッショングループをクリア | group_key | 同じgroup_key内でクリアして欲しい数 | 指定グループのミッションを全てクリアしよう |
| **MissionBonusPoint** | ミッションボーナスポイントを集める | 空欄(`__NULL__`) | 集めて欲しいポイント数 | ミッションボーナスポイントを100集めよう |
| **UserLevel** | プレイヤーレベル到達 | 空欄(`__NULL__`) | 到達してほしいレベル | プレイヤーレベル10に到達しよう |
| **CoinCollect** | コインを集める | 空欄(`__NULL__`) | 集めてほしいコイン数 | コインを10000枚集めよう |
| **CoinUsedCount** | コインを使用 | 空欄(`__NULL__`) | 使用してほしいコイン数 | コインを5000枚使用しよう |
| **UnitLevelUpCount** | ユニットのレベルアップ | 空欄(`__NULL__`) | レベルアップを実行してほしい回数 | ユニットのレベルアップを10回実行しよう |
| **UnitLevel** | 全ユニットの内でいずれかがレベル到達 | 空欄(`__NULL__`) | 到達して欲しいレベル | いずれかのユニットをLv.30に到達させよう |
...
| **ReviewCompleted** | ストアレビューを記載 | 空欄(`__NULL__`) | 固定値: `1` | ストアレビューを投稿しよう |
| **LoginCount** | 通算ログイン日数 | 空欄(`__NULL__`) | 通算ログインして欲しい日数 | 通算ログイン7日目 |
| **LoginContinueCount** | 連続ログイン日数 | 空欄(`__NULL__`) | 連続ログインして欲しい日数 | 連続ログイン3日目 |
| **IdleIncentiveQuickCount** | クイック探索 | 空欄(`__NULL__`) | クイック探索をして欲しい回数 | クイック探索を5回実行しよう |
| **IdleIncentiveCount** | 探索 | 空欄(`__NULL__`) | 探索をして欲しい回数 | 探索を10回実行しよう |
| **SpecificSeriesUnitAcquiredCount** | 特定作品のユニットを獲得 | 作品ID (mst_series.id) | 獲得してほしいユニットの種類数 | 【推しの子】のユニットを5種類獲得しよう |
| **UnitAcquiredCount** | ユニットを入手 | 空欄(`__NULL__`) | 入手して欲しいユニット体数 | ユニットを10体入手しよう |
...
| **EnemyDiscoveryCount** | 敵キャラを発見 | 空欄(`__NULL__`) | 発見してほしいエネミーの種類数 | 敵キャラを10種類発見しよう |
...
| **EmblemAcquiredCount** | エンブレムを獲得 | 空欄(`__NULL__`) | 獲得してほしいエンブレムの種類数 | エンブレムを10種類獲得しよう |
...
| **ArtworkCompletedCount** | 原画を完成 | 空欄(`__NULL__`) | 完成させてほしい原画数 | 原画を10つ完成させよう |
| **SpecificArtworkCompletedCount** | 指定原画を完成 | 原画ID (mst_artworks.id) | 固定値: `1` (原画完成は生涯1回のみ) | 特定の原画を1つ完成させよう |
| **PvpChallengeCount** | 決闘に挑戦 | 空欄(`__NULL__`) | 挑戦してほしい回数 | 決闘に3回挑戦しよう |
| **PvpWinCount** | 決闘に勝利 | 空欄(`__NULL__`) | 勝利してほしい回数 | 決闘に5回勝利しよう |
