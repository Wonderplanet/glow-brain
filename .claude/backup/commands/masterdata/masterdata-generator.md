...
```bash
# 仕様書ファイルの検索
マスタデータ/運用仕様書/${施策名}/要件/${施策名}_仕様書.xlsx
```

**ファイルが見つからない場合**:
- エラーメッセージを表示: `「施策名 '${施策名}' の仕様書が見つかりません」`
- `マスタデータ/運用仕様書/${施策名}/要件/` 内の候補ファイル一覧を表示（Globツール使用）
- 処理を終了
...
- **施策ディレクトリ**: `マスタデータ/運用仕様書/${施策名}/`
- **要件CSV出力先**: `マスタデータ/運用仕様書/${施策名}/要件/`
- **マスタCSV出力先**: `マスタデータ/運用仕様書/${施策名}/`（施策ディレクトリ直下）

```bash
# ディレクトリ作成
mkdir -p "マスタデータ/運用仕様書/${施策名}/要件"
```
...
- **要件CSV**: マスタデータ/運用仕様書/${施策名}/要件/${シート名}.csv
- **出力CSV**: マスタデータ/運用仕様書/${施策名}/${テーブル名}.csv
...
```bash
マスタデータ/運用仕様書/${施策名}/Mst*.csv
マスタデータ/運用仕様書/${施策名}/Opr*.csv
```
...
python scripts/validate_masterdata_template.py \
  --generated "マスタデータ/運用仕様書/${施策名}/${テーブル名}.csv" \
