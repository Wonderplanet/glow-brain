...
- [ ] マスタデータのクライアント定義・サーバー定義・使われ方を整理した（表形式）
...
- Modelのプロパティからカラムをリストアップし、以下の形式で記載：
  - **クライアント定義**: アッパーキャメルの単数クラス名.プロパティ名（例：`OprGacha.GachaId`）
  - **サーバー定義**: スネークケースの複数形テーブル名.スネークケースのカラム名（例：`opr_gachas.id`）
  - **クライアント説明**: 対象画面での使われ方を簡潔に説明
