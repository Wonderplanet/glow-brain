...
**フォルダ名の命名規則**: 実際のフォルダ名は `YYYYMMDD_HHMMSS_{タスク名}` の形式にします。
タイムスタンプは Step 3 のフォルダ作成直前に `date +%Y%m%d_%H%M%S` で取得してください。

例: `20260228_143052_id-numbering-analysis`

**重要**: 既存の `domain/tasks/` 配下のフォルダと重複していないか確認してください（タイムスタンプが異なれば基本的に重複しません）。
...
`domain/tasks/{YYYYMMDD_HHMMSS}_{タスク名}/` フォルダを作成し、**最小構成**で初期化します。

```bash
# タイムスタンプを取得してフォルダ名を確定する
FOLDER_NAME="$(date +%Y%m%d_%H%M%S)_{タスク名}"
mkdir -p "domain/tasks/${FOLDER_NAME}"
```

**初期構成**（最小限）:
- `README.md` - タスク概要
- `next-actions.md` - ネクストアクション
- `CLAUDE.md` - タスクコンテキスト（次回セッション引き継ぎ用）
...
**CLAUDE.md生成手順**:
1. `assets/CLAUDE.template.md` を読み込む
2. テンプレート変数を実際の値に置換する
3. `domain/tasks/{タスク名}/CLAUDE.md` に書き込む

**CLAUDE.mdテンプレート変数** (README.mdと共通の変数に加えて):
- `{TASK_FOLDER_PATH}` - タスクフォルダのパス (`domain/tasks/{YYYYMMDD_HHMMSS}_{TASK_NAME}`)
...
全ステップが完了したら、以下を宣言してください：

---
タスクフォルダの準備が完了しました。

以降の作業はすべて以下のフォルダを起点として進めます：
  domain/tasks/{タスク名}/

ファイル構成：
- README.md       → タスクの概要・目的・成果定義
- next-actions.md → 次にやること（チェックリスト）
- CLAUDE.md       → 次回セッション用の引き継ぎコンテキスト

今後このタスクを継続する際は、Claude Codeを起動後に
「CLAUDE.mdを読んで現状を把握してください」と伝えることで
スムーズに作業を再開できます。
---

以降のファイル操作は `domain/tasks/{タスク名}/` を起点とした
パスを使用してください。
...
5. **スキル完了後はタスクフォルダを起点にする** - 完了宣言後はすべてのファイル操作を `domain/tasks/{タスク名}/` を基準として行う
...
- `assets/CLAUDE.template.md` - CLAUDE.mdのテンプレート
...
→ 背景・問題・成果をヒアリング
→ フォルダ作成: domain/tasks/20260228_143052_create-gacha-masterdata/
→ README.md生成
...
→ 背景・問題・成果をヒアリング
→ フォルダ作成: domain/tasks/20260228_150300_user-retention-analysis/
→ README.md生成
...
→ 背景・問題・成果をヒアリング
→ フォルダ作成: domain/tasks/20260228_161500_new-feature-prototype/
→ README.md生成
