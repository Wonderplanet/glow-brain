...
> **フォーマット参考**: `domain/knowledge/masterdata/in-game/guides/raid_jig1_00001_v2.md`（`masterdata-ingame-detail-explainer` スキルが生成した実例）
...
15. **c_キャラ（プレイアブル敵）の召喚制約**: `action_value` が `c_` で始まるキャラは、同一トリガーで `summon_count >= 2` かつ `summon_interval = 0`（瞬間複数召喚）を設定しない。世界観的に同一プレイアブルキャラが複数体同時にフィールドに存在する状態は矛盾が生じる。撃破後の再出撃は `FriendUnitDead`（累積撃破数トリガー）で制御する
...
- [raid_jig1_00001_v2.md](domain/knowledge/masterdata/in-game/guides/raid_jig1_00001_v2.md) — design.md フォーマット参考実例（`masterdata-ingame-detail-explainer` スキル生成）
...
| `domain/knowledge/masterdata/in-game/guides/raid_jig1_00001_v2.md` | design.md フォーマット参考実例（masterdata-ingame-detail-explainer 生成） |
| `domain/tasks/20260310_115400_vd_ingame_masterdata_generation/knowledge/MstAutoPlayerSequence_c_キャラ召喚パターン分析.md` | c_キャラ（プレイアブル敵）の召喚パターン分析（実データに基づく設計ガイド） |
