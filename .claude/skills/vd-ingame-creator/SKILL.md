...
限界チャレンジ（VD）のインゲームマスタデータCSVを、作品・ブロック種別の指定から正確に生成します。
...
| `MstKomaLine` 行数 | **1行**（固定） | **1行**（コマ数1〜4でランダム） |
...
| ブロック種別 | `boss` / `normal` / 両方 |
| ブロック数 | normalの場合：何ブロック作成するか |
...
## 生成するインゲームID一覧
| ID | ブロック種別 | ボスキャラ | 雑魚キャラ |
|-----|-----------|---------|---------|
| vd_{作品ID}_boss_00001 | boss | {キャラ名/ID} | {雑魚A, 雑魚B} |
| vd_{作品ID}_normal_00001 | normal | — | {雑魚A, 雑魚B} |
...
### bossブロック（2〜4行）
- 行1: InitialSummon → SummonEnemy(ボス) × 1（summon_position=1.7）
