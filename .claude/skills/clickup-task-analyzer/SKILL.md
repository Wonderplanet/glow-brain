...
1. **メンバー一覧.csv登録者の職種分類**
   ...
   - 例: Keisuke Isayamaは必ず「クライアントエンジニア」（CSV: エンジニア, クライアント）
...
乾 健太,Kenta Inui,エンジニア,クライアント,インゲーム・アウトゲーム | 設計 | 実装 | テスト,inui@wonderpla.net
川崎 大樹,Daiki Kawasaki,エンジニア,サーバー,API・ツール・インフラ | 設計 | 実装 | テスト,
望月 りら,Rira Mochizuki,デザイナー,UI,AD補佐 | UIレイアウト制作 | 素材作成 | 実装,mochizuki@wonderpla.net
```
...
**ClickUpのassignee名形式**:
- "Daiki Kawasaki"（First Last）
- "daichi_takishima"（first_last）
- "donghyeok kim"（first last）
- "KobayashiRikaco"（FirstLast）
...
**マッチング例**:
- "Daiki Kawasaki" → "daiki kawasaki" → マッチ成功
- "daichi_takishima" → "daichi takishima" → マッチ成功
- "KobayashiRikaco" → "kobayashirikaco" → マッチ失敗（姓名連結形式）→ フォールバックへ
...
- **判断**: 最初の担当者（daichi_takishima）の職種で分類
