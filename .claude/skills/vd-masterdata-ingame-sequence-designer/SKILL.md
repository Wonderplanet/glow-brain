---
name: vd-masterdata-ingame-sequence-designer
description: VDインゲーム設計書（design.md）の「敵キャラシーケンス設計」セクションをMermaid flowchart図付きで生成・更新するスキル。出現タイミング・順序設計からelemテーブルを生成します。「VDシーケンス設計」「敵出現設計」「sequence-design」「出現タイミング」などのキーワードで使用します。
---
...
VDインゲーム設計書（design.md）の **`### 敵キャラシーケンス設計`** セクションを生成・更新する専門スキル。
...
| `キャラリスト` | ✓ | キャラID・色属性・体数のリスト（enemy-stats-designerの結果と対応） |
| `[ボスキャラID]` | ボスのみ | bossブロックのボスキャラID |
| `[コマ行数]` | 任意 | 召喚位置の範囲決定用。省略時はブロック種別から自動判断 |
| `[--batch]` | 任意 | 確認ループをスキップ |
...
**参照ドキュメント（必須）**:
- `.claude/skills/vd-masterdata-ingame-design-creator/references/MstAutoPlayerSequence_具体例集.md` — 過去実例集
- `.claude/skills/vd-masterdata-ingame-design-creator/references/MstAutoPlayerSequence_設計パターン集.md` — 設計パターン解説
...
- `.claude/skills/vd-masterdata-ingame-design-creator/references/MstAutoPlayerSequence_具体例集.md` — 過去15ステージの実例集
- `.claude/skills/vd-masterdata-ingame-design-creator/references/MstAutoPlayerSequence_設計パターン集.md` — condition_type・summon_count・aura_type等の設計パターン
- `domain/knowledge/masterdata/table-docs/MstAutoPlayerSequence.md` — テーブル定義
