...
このドキュメントは、ゲーム体験仕様書から抽出した要件を6つのカテゴリに分類するための詳細な定義と判定基準を提供します。
...
### 定義
ユーザー体験・ゲーム仕様として **「何が起きるか」「どう振る舞うか」** を定義する要件。
...
### 定義
機能の中身ではなく **性能・品質・安全性・信頼性** などの性質を定義する要件。
...
1. **1つの仕様項目に複数の要件が含まれることは珍しくない**
   - 例: 「設定テーブルでON/OFF切り替え可能な新機能」
     - 機能要件: 新機能の体験内容
     - 運用要件: 設定テーブルでON/OFF切り替え可能

2. **分類の目的は「混ぜないこと」**
   - 明確に分離することで、API設計時に適切な設計判断ができる
   - 各要件カテゴリに応じた実装アプローチを選択できる

3. **迷ったら判定軸に立ち返る**
   - 即判定チェックリストを活用する
   - 複数の要件に該当する場合は、主要な要件を優先

## 参考資料

このルールは以下のドキュメントをベースにしています:
- `domain/tasks/design/api/rules/ゲーム体験仕様書_要件分類ルール.md`
