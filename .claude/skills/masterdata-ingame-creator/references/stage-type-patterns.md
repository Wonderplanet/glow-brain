...
1作品のうちボスバトルを担当するブロック。MstInGame.id 1件 = 1ブロック。
...
### MstAutoPlayerSequence: シーケンス行数目安
- `2 〜 4行`
- ボス1体 + 時間差雑魚1〜2パターンが基本

### 基本敵シーケンス（Boss block の動作仕様）

- 敵ゲート前にボスが召喚され、N秒おきに進軍してくる能動的な構成
- HPはインゲームで通常登場するキャラより低いが、攻撃力は高い設定
- 敵ゲートはボスが撃破されるまでダメージ無効（`is_damage_invalidation` = true 相当の挙動）

---

## dungeon_normal（限界チャレンジ・通常ブロック）

1作品のうち通常バトルを担当するブロック。複数のnormalブロックを連結して運用する。
...
| カラム | 値 |
|--------|-----|
| 行数 | **3行**（normalブロックは3行固定） |

### MstAutoPlayerSequence: シーケンス行数目安
- `3 〜 8行`
- 雑魚複数パターンを時間差で出現させる構成が基本

### 基本敵シーケンス（Normal block の動作仕様）

- フロア毎に配置された敵が、味方キャラが該当フロアに進行した際に動き出す
- 設定された射程範囲内で味方と接触（会頭）した場合に攻撃開始
- 敵はフロア単位で独立しており、味方の進行に反応して起動する受動的な構成
