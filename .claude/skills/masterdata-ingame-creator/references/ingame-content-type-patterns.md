...
| **dungeon** | — | `dungeon_kai_boss_00001` | 限界チャレンジ（ブロック単位構成） |
...
### dungeon（限界チャレンジ・ブロック単位構成）

- **コンセプト**: 「ブロック」という単位でインゲームを構成する。MstInGame.id 1件 = 1ブロック。複数ブロックを連結して長いコンテンツとして運用する
- **規模**: 1作品につき boss 1個 + normal N個のペアで構成する
- **ブロック種別ごとの動作**:
  - **Normal block**: フロア毎に配置。味方が進行した際に起動し、射程範囲内で会頭したら攻撃開始
  - **Boss block**: 敵ゲート前に召喚され、N秒おきに進軍。HP低め・攻撃高め。ボス撃破まで敵ゲートはダメージ無効

#### ブロック種別

| ブロック種別 | ID例 | MstKomaLine行数 | MstEnemyOutpost HP |
|------------|------|---------------:|------------------:|
| `dungeon_boss` | `dungeon_kai_boss_00001` | **1行**（固定） | **1,000**（固定） |
| `dungeon_normal` | `dungeon_kai_normal_00001` | **3行**（固定） | **100**（固定） |

#### MstEnemyOutpost HP固定値
...
- **MstAutoPlayerSequence**: boss は 2〜4行、normal は 3〜8行が目安
- **質問すべきこと**: シリーズID・作品ごとのブロック数・各ブロックの敵キャラ設定
...
Q: 作成するインゲームのコンテンツ種別は何ですか？
  → event / normal / hard / veryhard / raid / dungeon / pvp / tutorial / その他

（raidの場合）Q: フェーズは何段階ありますか？各フェーズでのボス形態（AdventBoss1/2/3）は？

（raidの場合）Q: フェーズ切り替えのトリガーは何ですか？（ボスHP〇%等）

（dungeonの場合）Q: ブロック種別は boss / normal どちらですか？（両方作成する場合は全ブロック数を確認）

（dungeonの場合）Q: シリーズ略称と各ブロックの連番を教えてください
```
