...
**{シリーズ名}の{コンテンツ種別}**（{ゲームモード: 砦破壊型/スコアアタック型}）。

- **砦HP**: `{hp}`（ダメージ{有効/無効}）・背景アセット=`{artwork_asset_key}`
- **BGM**: 通常=`{bgm_asset_key}`・ボス=`{boss_bgm_asset_key}`（なしの場合は「ボスBGMなし」）
- **コマ**: {N}行構成（各rowの幅レイアウトを `row{N}: {幅+幅}コマ` 形式で全行記載・コマ効果がある場合は対象コマと効果種別を明記。効果なしの場合も「コマ効果なし」と明記）
- **グループ**: デフォルト + {w1〜wN}（遷移条件の種別: 撃破数 `FriendUnitDead` / 時間 `ElapsedTime` / 砦HP `OutpostHpPercentage`・ループ有無と起終点・1ループあたりの体数）
- **使用する敵**: {N}種類（ボスN種: {日本語名+属性} / 中ボスN種: {日本語名} / 雑魚N種: {日本語名+属性}）
- **特殊ルール**: {スピードアタックルール等 / なし}
- {グループ構造の特記事項: 分岐・並行グループ・難度推移。各敵の hp_coef・atk_coef・速度の特性を具体値で記載（MstAutoPlayerSequence の係数も言及）}
- {特殊な召喚設計がある場合: 砦ダメージトリガー・EnterTargetKoma 待ち伏せ・InitialSummon等の仕組みと発動条件を記載}
- {result_tips と description の要点（バトルヒントとステージ説明。データが空または __NULL__ の場合は省略）}
```

> **各箇条書きの記載方針**:
> - **砦HP・BGM・コマ**: MstEnemyOutpost / MstInGame / MstPage+MstKomaLine の値をそのまま使う。コマのアセットキー・各rowの幅レイアウトを全行漏れなく記載する。
> - **グループ**: ループ有無・起点と終点のグループID・1ループの体数・デフォルトのみの場合は「ループなし・終端」と明記する。
> - **使用する敵**: Step 2-6 で取得した日本語名を必ず使用（IDのまま記載しない）。kind（Boss/Normal）・属性（Blue/Red等）を添える。
> - **特記事項行**: hp_coef・atk_coef・speed 等の具体数値を記載する（抽象化不要）。グループ間の難度推移・特殊なトリガー設計・待ち伏せ配置・MstInGame 係数の扱い等も記載する。
> - **バトルヒント行**: result_tips / description の要点を記載。省略可。
>
