...
1. **出力ディレクトリの作成**:
   - 運営仕様ディレクトリ内に `マスタデータCSV/` サブディレクトリを作成
   - 例: `マスタデータ/運営仕様/<運営仕様名>/マスタデータCSV/`
...
4. **出力先**: `マスタデータCSV/` サブディレクトリ内
   - 例: `マスタデータ/運営仕様/<運営仕様名>/マスタデータCSV/MstEvent.csv`

5. **enum値の確認**（masterdata-explorerスキル使用）:
   ```bash
   .claude/skills/masterdata-explorer/scripts/search_schema.sh enum <テーブル名> <カラム名>
   ```
