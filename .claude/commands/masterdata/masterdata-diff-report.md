...
3. **テキストの引用符の違い** (`""` vs `''` vs 引用符なし)
   - 理由: テキスト内容が同じであれば、周囲の引用符の種類や有無は差異として扱わない
   - 例: `hello` vs `"hello"` vs `'hello'` は同一とみなす

4. **備考欄の違い**
   - 理由: マスタデータ自体には関係なく、ただのメモのため、差分レポートに含めない
...
**注意**: 備考欄の差異は差分レポートに含めません。備考欄については比較対象から除外してください。
...
レポートは生成データディレクトリに`diffs`というディレクトリを作成して、そこに保存してください。
...
**重要**: それ以外の場所への保存は禁止です。必ず`diffs/`ディレクトリに保存してください。
