...
2. **Phase 2**: ファイル変換（XLSX→CSV等）
3. **Phase 3**: 生成対象シートの特定
...
```bash
# 仕様書ファイルの検索（今回はXLSX想定）
マスタデータ/施策/src/${施策名}_仕様書.xlsx
```

**ファイルが見つからない場合**:
- エラーメッセージを表示: `「施策名 '${施策名}' の仕様書が見つかりません」`
- `マスタデータ/施策/src/` 内の候補ファイル一覧を表示（Globツール使用）
- 処理を終了

**ファイルが見つかった場合**:
- ファイルパスを確認
- 拡張子から変換方法を判定（今回は `.xlsx` → `scripts/xlsx_to_csv.py` を使用）
...
1. **Readツールで先頭3行を読み取り**:
   - Row 1: `memo`（説明行）
   - Row 2: `TABLE,テーブル名,テーブル名,...` の形式か確認
   - Row 3: `ENABLE,カラム名,カラム名,...` の形式か確認

2. **除外パターン**:
   以下のいずれかに該当するCSVは、マスタデータ生成対象外とします：
   - Row 2に "TABLE" が含まれていない
   - シート名（CSVファイル名）が以下のパターンに一致:
     - `*仕様概要*`
     - `*更新履歴*`
     - `*依頼リスト*`
     - `*クリエイティブ*`
...
生成対象のマスタデータCSVを最大3-5個のグループに分けます。
...
Taskツールを使用して、各グループをgeneral-purpose subagentに並列で処理させます。
...
- **PDF→Markdown変換**: `scripts/pdf_to_markdown.py`（将来実装）
- **Markdown直接処理**: 変換不要で直接CSVに変換
