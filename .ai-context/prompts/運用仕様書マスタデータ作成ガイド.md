...
このガイドは、運用仕様書からマスタデータCSVを作成する際の手順と注意点を示します。

## 前提条件

- 運用仕様書が `マスタデータ/運用仕様書/<運用仕様書名>/要件/` に配置されている
- マスタデータの出力先は運用仕様書ディレクトリ直下（`マスタデータ/運用仕様書/<運用仕様書名>/`）
- **CSVテンプレート部分の変更は厳禁**
...
運用仕様書を読み、必要なマスタデータテーブルをリストアップします。
...
まず、運用仕様書の概要ファイルに新リソース情報が記載されているか確認します。

```bash
# 01_概要ファイルを確認
cat "マスタデータ/運用仕様書/${運用仕様書名}/要件/${運用仕様書名}_仕様書_01_概要.csv"
```
...
```bash
# 報酬一覧ファイルを確認
cat "マスタデータ/運用仕様書/${運用仕様書名}/要件/${運用仕様書名}_仕様書_05_報酬一覧.csv"
```

**Step 3: 他の要件CSVファイルを検索**

概要ファイルに記載がない場合、運用仕様書ディレクトリ内の全要件CSVを検索します。

```bash
# 運用仕様書ディレクトリ内の全要件CSVからキャラクターIDを検索
grep -r "chara_xxxxx" "マスタデータ/運用仕様書/${運用仕様書名}/要件/"

# 特定のキャラクターIDが他のシートに記載されているか確認
grep "chara_100kano_00001" "マスタデータ/運用仕様書/${運用仕様書名}/要件/"*.csv
```

**Step 4: 既存マスタデータを検索**

運用仕様書開始前から存在する既存リソースの場合、既存マスタデータCSVに記載があります。
