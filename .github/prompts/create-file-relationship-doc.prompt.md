---
description: ディレクトリ内のファイル関係性まとめMarkdownを生成
argument-hint: directory-path
agent: 'agent'
---

# File Relationship Documentation Generator

指定されたディレクトリ内のファイル構造と関係性を分析し、わかりやすいMarkdownドキュメントを生成します。

## コンテキスト

- **対象**: 任意のディレクトリ（HTMLファイル、仕様書、設計書など）
- **言語**: 言語非依存
- **範囲**: ユーザー指定のディレクトリ
- **出力**: Markdownドキュメント
...
指定されたディレクトリ（引数で受け取る）内のファイルを分析し、以下の手順でドキュメントを生成してください：

1. **ファイル一覧の取得**
   - Globツールを使用してディレクトリ内の全ファイルをリストアップ
   - ファイルの拡張子、名前、パターンを把握
...
### 基本的な使用方法
\`\`\`
/create-file-relationship-doc docs/仕様書/
\`\`\`

### HTMLファイル群の場合
\`\`\`
/create-file-relationship-doc docs/マスタデータ作成/src/イベント仕様書_html/
\`\`\`

生成されるファイル:
\`\`\`
docs/マスタデータ作成/src/イベント仕様書_html/ファイル関係性まとめ.md
\`\`\`
