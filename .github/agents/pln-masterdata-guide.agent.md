...
#### ステップ3: DB定義・ドキュメントで補足情報を確認（必要に応じて）

```bash
# テーブル構造の確認（マイグレーションファイル）
masterdata-explorerスキルを使って確認する。
...
3. **簡潔に回答**：関連テーブルと重要項目のみを説明（環境に応じて図解）
...
3. **実際に使われているテーブル・カラムのみ回答**：CSVの全カラムは説明しない
...
3. **簡潔に図解**：環境に応じてテキストツリーまたはMermaid図
...
**調査・回答の手順:**
1. **CSVで既存の類似データを確認**：同レアリティの他データと比較
...
2. **実際に使われているものだけ回答** - CSVにある全カラムではなく、コードで使われているものだけを説明
