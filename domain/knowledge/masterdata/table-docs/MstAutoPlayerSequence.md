...
| `FriendUnitDead` | `sequence_element_id`（文字列） | 指定の sequence_element_id で召喚されたキャラが1体でも倒されたとき発火 |
...
> **FriendUnitDeadの注意点**: `condition_value` には **sequence_element_id の文字列値** を指定する。
> 累計撃破数ではない。「condition_value="2"」 = 「sequence_element_id=2 の行で召喚されたキャラが1体でも倒されたとき」。
> 自分自身の sequence_element_id を condition_value に設定すると循環参照になり永遠に発火しない。
> 同じ condition_value を複数行に設定すると、そのキャラが倒されたとき**全行が同時発火**する。
...
### ポイント3: FriendUnitDead は sequence_element_id への参照

- `condition_value` には `sequence_element_id` の文字列値を指定する（累計撃破数ではない）
- 「condition_value=2」= 「sequence_element_id=2 の行で召喚されたキャラが1体でも倒されたとき」
- **自分自身の sequence_element_id を condition_value に書くと循環参照になり発火しない**
