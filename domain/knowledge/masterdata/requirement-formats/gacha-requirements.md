...
> ※ 前リリースの OprGachaPrize.csv を参照してClaudeが全量再構築します。プランナーは以下の最小情報のみ記入してください。

- 前リリースのPickupガチャID: {例: Pickup_kim_001}
  （ClaudeがこのIDを手がかりに前リリースCSVを参照し、全排出枠を把握します）

【新規恒常追加キャラ（今リリースで初めてガチャプールに追加するキャラがある場合のみ）】
- 恒常追加: {キャラID} / レアリティ: {UR / SSR / SR / R}
（なければ「なし」）

【前回PU以外で削除するキャラ（同シリーズ旧キャラ除外などがある場合のみ）】
- 削除: {キャラID}（理由: 例「今回PUシリーズ（{作品略称}）の旧キャラのため」）
（なければ「なし」）

※ 以下はClaudeが自動処理します（記入不要）:
  - 前回PUキャラ（pickup=1だったキャラ）を排出リストから除外
  - 今回PUキャラ（「ピックアップキャラ」セクション記載）を追加してpickup=1に設定
  - 全キャラのweightを確率ルールに従い再計算
...
> ※ 前リリースのOprGachaPrize.csvをClaudeが参照して全量再構築します。プランナーは差分情報のみ記入してください。

【新規追加するキャラ（今リリースで新しくスペシャルガチャに追加するキャラ）】
- 追加: {キャラID} / レアリティ: {UR / SSR / SR / R}
（なければ「なし」）
...
- 前リリースのPickupガチャID: Pickup_kim_001
...
- 前リリースのPickupガチャID: Pickup_sur_002
...
- 前リリースのPickupガチャID: Pickup_sur_002
  （ガチャAと同一のキャラプールを使用。ピックアップキャラのみ異なる）
...
- 前リリースのPickupガチャID: Pickup_jig_002
