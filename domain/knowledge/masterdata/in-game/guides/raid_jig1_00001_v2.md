...
敵の召喚時のアニメーション設定（MstAutoPlayerSequence.`summon_animation_type`）。

| summon_animation_type | 説明 | raid_jig1_00001での使用 |
|----------------------|------|----------------------|
| `None` | アニメーションなし | **全敵（23行すべて）** |
| `Fall` | 落下アニメーション（標準） | 他ステージで使用 |
| `Fall0` | 落下アニメーション（バリエーション0） | 他ステージで使用 |
| `Fall4` | 落下アニメーション（バリエーション4） | 他ステージで使用 |

raid_jig1_00001では全敵の`summon_animation_type`が`None`（アニメーションなし）。
