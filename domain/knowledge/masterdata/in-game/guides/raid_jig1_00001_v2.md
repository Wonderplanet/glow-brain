...
jig1シリーズのレイドバトル（スコアアタック型）。砦のHPは100万で設定されているがダメージは完全無効化されており、プレイヤーの目的は砦破壊ではなく制限時間内に敵を倒し続けてバトルポイントを稼ぐことにある。

フィールドは3行5コマ構成で、コマ効果はなく全アセットjig_00001で統一されたシンプルな設計。BGMはSSE_SBG_003_008が通常再生され、ボスBGMへの切り替えはない。

敵はデフォルトグループ + w1〜w5の6グループで構成され、w5完了後はw1へ戻る無限ループ構造を持つ。グループ遷移はバトル開始2秒後のみ時間トリガー（ElapsedTime）で行われ、それ以降はすべて累計撃破数（FriendUnitDead）で制御される。累計撃破数はループ後もリセットされないため、2周目以降はw1をスキップしてw2から実質スタートとなり、後半ほど高難度のグループが連続する。

登場する敵は4種類。最速（spd=70）の極楽蝶はdrop_bp=0でスコアに貢献しないが大量出現して数的プレッシャーを与える。低速（spd=30〜31）の門神・門神(大)は高耐久のタンク型で、門神(大)はBoss扱いだが同一モデルを使用。中速（spd=35〜40）の竈神 魚と朱槿はBossとして登場し、特に竈神 魚はdrop_bp=400と高く効率的なスコア源となる。

各グループには属性テーマがあり、デフォルト＝無属性、w1〜w2＝黄、w3＝緑、w4＝赤、w5＝全属性混合となっている。w5では竈神 魚ボスを先頭に、撃破連鎖で門神(大)・朱槿・門神が次々と出現する設計で、w5の朱槿はAdventBoss3（最高演出ランク）のオーラを持ち、hp倍=6・atk倍=5と最大強化での登場となる。

バトル開始直後は位置1.7に竈神 魚が初期配置（InitialSummon）されており、コマ1到達まで静止したまま戦場に鎮座することで、序盤から高耐久ボスが「既にそこにいる脅威」として演出される。
...
3行5コマ構成（全行height=0.55固定）。全コマeffect=None（コマ効果なし）、アセットはjig_00001で統一。
