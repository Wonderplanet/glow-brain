...
| MstAdventBattle.AdventBattleName | mst_advent_battles_i18n.name | 降臨バトル名（画面タイトルに表示） |
| MstAdventBattle.AdventBattleType | mst_advent_battles.advent_battle_type | バトルタイプ（Raid/ScoreChallengeを判定し、この画面はRaidのみ表示） |
| MstAdventBattle.DisplayMstUnitId1 | mst_advent_battles.display_mst_unit_id1 | 表示する敵ユニットID（ボスキャラクター表示に使用） |
| MstAdventBattle.EndAt | mst_advent_battles.end_at | 降臨バトル終了日時（ランキング集計完了タイミングの計算に使用） |
...
APIから取得されるユーザーのランキング情報（`GET /api/advent_battle/info`）：
