...
- 2026-02-28: **式をハードコード化（直接セル参照）に簡略化**
  - MATCH+INDIRECT+LET を廃止し、シートグループ別の固定行番号による直接参照に変更
  - GROUP_A（ストーリー/チャレンジ1-2話/降臨バトル）: er=13, rr=13, kr_start=31
  - GROUP_B（チャレンジ3-4話/高難度）: er=13, rr=13, kr_start=32
  - GROUP_C（通常/ボスブロック）: er=14, rr=14, kr_start=19
  - 式長を最大82%削減（例: FR列 200文字→35文字）
