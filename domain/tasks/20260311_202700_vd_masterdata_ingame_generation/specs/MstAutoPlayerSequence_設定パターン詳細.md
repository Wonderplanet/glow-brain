...
| `FoeEnterSameKoma` | `0` | 敵（プレイヤー側）が同じコマ（マス単位）に入ったら移動開始 |
| `FoeEnterSameKomaLine` | `0` | 敵（プレイヤー側）が同じコマライン（行単位）に入ったら移動開始 |
...
#### `FoeEnterSameKoma` vs `FoeEnterSameKomaLine` の違い

コマとコマラインはページの構成単位が異なる。

```
MstPage（ページ）
  └─ KomaLine[0]（コマライン＝行）
  │    └─ Koma[0], Koma[1], Koma[2]  ← 1行内に横並びで複数のコマ
  ├─ KomaLine[1]
  │    └─ Koma[3], Koma[4]
  └─ KomaLine[2]
       └─ Koma[5]
...
| 値 | 判定単位 | 粒度 | 用途 |
|----|---------|------|------|
| `FoeEnterSameKoma` | コマ ID（個別マス）が一致するか | 細粒度 | 特定のマスに入ったとき反応させたい |
| `FoeEnterSameKomaLine` | コマライン番号（行）が一致するか | 粗粒度 | 同じ「行」に入ったら幅広く反応させたい |

> **⚠️ 実装状況（2026-03時点）**
>
> `FoeEnterSameKomaLine` は `MoveStartConditionType` enum（AutoGenerated）に追加済み。
> ただし `UnitGenerationModelFactory` の switch 文への追加はまだ行われていないため、
> 現時点ではマスタデータに設定しても `None`（即時移動）として動作する。
> 今後のクライアント更新で正式に動作するようになる予定。
...
使える値は `MoveStartConditionType` と同じ（`None`, `ElapsedTime`, `Damage`, `FoeEnterSameKoma`, `FoeEnterSameKomaLine`, `EnterTargetKoma`, `DeadFriendUnitCount`, `OnFieldPlayerCharacterCount`）。
