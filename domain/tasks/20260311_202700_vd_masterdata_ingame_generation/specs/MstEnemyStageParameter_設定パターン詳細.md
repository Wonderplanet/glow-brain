...
## 13. 既存マスタデータからのパラメータ傾向まとめ

`projects/glow-masterdata/MstEnemyStageParameter.csv` 全 1,073 件の統計:

| パラメータ | Normal（617件）の傾向 | Boss（448件）の傾向 |
|---|---|---|
| `hp` | 1,000 〜 900,000（平均 69,617） | 1,000 〜 5,000,000（平均 133,249） |
| `damage_knock_back_count` | 0〜12 | 0〜13 |
| `move_speed` | 8〜100（平均 40） | 5〜75（平均 36） |
| `well_distance` | 0.11 / 0.2 / 0.24 が多い | 0.2 / 0.3 / 0.4 が多い |
| `attack_power` | 50〜2,500（平均 374） | 10〜3,800（平均 364） |
| `attack_combo_cycle` | 0〜12 | 0〜13 |
| `drop_battle_point` | 0〜2,000 | 0〜2,000 |

### well_distance の実績値分布（上位）

| 値 | 件数 | 意味 |
|---|---|---|
| `0.2` | 215件（最多） | 標準的な索敵距離 |
| `0.11` | 152件 | 近接型（射程短め） |
| `0.3` | 83件 | 中距離型 |
| `0.24` | 81件 | 近〜中距離 |
| `0.22` | 64件 | 近〜中距離 |

**推奨設定値**: `0.11` / `0.2` / `0.3` の3択が最も多用される。
...
|---|---|---|
| `e_` | 570件 | 敵専用キャラクター |
| `c_` | 503件 | プレイヤーキャラが敵として登場 |

`boss_` / `general_` 等のプレフィックスは存在しない。全レコードが `e_` か `c_` の2種のみ。

### mst_unit_ability_id1 の使用状況（実績）

| ability_id | 件数 | 効果 |
|---|---|---|
| `enemy_ability_knockback_block` | 15件 | ノックバック無効 |
| `enemy_ability_gust_3_zombie` | 7件 | 突風Lv3＋ゾンビ化 |
| `enemy_ability_gust_1_zombie` | 5件 | 突風Lv1＋ゾンビ化 |
| （未設定） | 1,046件 | アビリティなし（約97.5%） |

大多数の敵にアビリティは設定されていない。アビリティは特定の強敵・ギミック敵のみに付与する。

### 変身設定の使用状況（実績）

| transformationConditionType | 件数 |
|---|---|
| `None`（変身なし） | 1,060件（約99%） |
| `HpPercentage`（HP%で変身） | 13件（約1%） |

変身は非常に限定的な機能。変身トリガーの実績値は **HP 30%** と **HP 50%** が主流（HP 1% は「登場直後に別形態」の演出用特殊パターン）。

> **move_speed の設計傾向**: Normal（平均40）に比べ Boss（平均36）はやや遅い傾向があるが、Boss でも move_speed=75 の例も存在する。「ボスは遅い」は傾向であって必須条件ではない。
