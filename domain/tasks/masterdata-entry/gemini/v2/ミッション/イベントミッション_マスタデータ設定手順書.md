...
「イベントミッション マスタデータ設定手順書」セクションの手順に従って、
notebookLMにソース登録されている「20260116_地獄楽 いいジャン祭_仕様書」の
下記のシートから、イベントミッションのマスタデータを作成してください。
- 04_ミッション
...
- **最後に、仕様書、設計書の内容で、正確に漏れなく、マスタデータを作成できているかを、必ず確認する**
...
イベントミッション(mst_event_id指定)で使用可能なcriterion_typeは
...
|---------|-----------|
| **ENABLE** | 固定値: `e` (有効化) |
| **id** | 依存関係の一意識別子。命名規則: `mission_depend_event_{作品ID}_{イベント連番}_{連番}`  |
...
|---------|-----------|
| **ENABLE** | 固定値: `e` (有効化) |
| **id** | 報酬の一意識別子。命名規則: `mission_reward_event_{作品ID}_{イベント連番}_{連番}` |
| **release_key** | リリースキー。MstMissionEventと同じ値 |
...
- [ ] **ID採番ルール**
  ...
  - MstMissionEventDependency.id: `mission_depend_event_{作品ID}_{イベント連番}_{連番}`
    - 例: `mission_depend_event_osh_00001_03`
  - MstMissionEventDependency.group_id: 依存関係グループの最初のミッションIDと同じ
    - 例: `event_osh_00001_1` (そのグループの最初のミッション)
  - MstMissionReward.id: `mission_reward_event_{作品ID}_{イベント連番}_{連番}`
    - 例: `mission_reward_531`, `mission_reward_532`
    - 例: `mission_reward_event_jig_00001_01`, `mission_reward_event_kai_00002_05`
...
- [ ] **仕様書、設計書の内容で、正確に漏れなく、マスタデータを作成できているかを、必ず確認する**
...
|-------|--------------------------|--------------------------|--------------------------|--------------------------|--------------------------|--------------------------|
| **ENABLE** | **id** | **release_key** | **group_id** | **mst_mission_event_id** | **unlock_order** | **備考** |
| e | mission_depend_event_osh_00001_1_01 | 202512020 | event_osh_00001_1 | event_osh_00001_1 | 1 | |
| e | mission_depend_event_osh_00001_1_02 | 202512020 | event_osh_00001_1 | event_osh_00001_2 | 2 | |
...
|-------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|
| **ENABLE** | **id** | **release_key** | **group_id** | **resource_type** | **resource_id** | **resource_amount** | **sort_order** | **備考** |
| e | mission_reward_event_osh_00001_01 | 202512020 | osh_00001_event_reward_1 | Item | ticket_glo_10001 | 2 | 1 | 推しの子ミッション(賀正チケット) |
| e | mission_reward_event_osh_00001_03 | 202512020 | osh_00001_event_reward_5 | FreeDiamond | | 50 | 1 | 推しの子ミッション(賀正チケット) |
