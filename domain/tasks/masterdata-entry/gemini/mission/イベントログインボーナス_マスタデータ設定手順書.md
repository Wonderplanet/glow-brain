...
|---------|-----------|
| **ENABLE** | 固定値: `e` (有効化) |
| **id** | ログインボーナスの一意識別子。命名規則: `{MstMissionEventDailyBonusSchedule.id}_{login_day_count}` |
...
```
{MstMissionEventDailyBonusSchedule.id}_{login_day_count}
```
...
- [ ] **ID採番ルール**
  ...
  - MstMissionEventDailyBonus.id: `{MstMissionEventDailyBonusSchedule.id}_{MstMissionEventDailyBonus.login_day_count}`
