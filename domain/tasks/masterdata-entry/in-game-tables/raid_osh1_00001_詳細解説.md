...
- elem12が **4行ある**（`sequence_element_id`が全て`12`）: 同一条件・同一タイミングで無属性雑魚4体を異なる位置(2.2/2.4/2.6/2.8)に個別召喚。**element_idが重複しているが動作する**
...
- ボスが `c_osh_00301`（Yellow・Technical）に変わる。スコアも50→100に倍増
...
- 雑魚のhp倍も20→30に上昇
- elem27: 累計22体倒したとき無属性雑魚1体（hp倍20だが atk倍1=攻撃は弱い）→砦へ向かうが倒せない「タンク」的な役割
...
- elem37: `c_osh_00401`（Colorless）が AdventBoss3 オーラで登場。score=300（最高値）
- 3体が位置2.3/2.5/2.7に分散配置され、250msずつずらして落下（壮観な演出）
- 雑魚もhp倍60と大幅強化。累計41体倒したとき無属性雑魚10体追加
...
- elem47: 緑Boss（hp倍70=700,000, atk倍20）。こちらにはauraなし。実質最強の雑魚
...
### 4. Normal種別でも超強敵を作れる
w5の `c_osh_00301`（hp倍300）や `c_osh_00201`（hp倍350）はkind=Normal。
