...
#### API
- **新規追加:**
  - `POST /api/{domain}/{xxxx}`: {エンドポイントの説明}
...
sequenceDiagram
    ...
    Client->>Server: POST /api/{domain}/{xxxx}
...
| start_at | INDEX | timestamp NOT NULL | 開始日時 |
| end_at | INDEX | timestamp NULL | 終了日時（NULL: 無期限） |
...
| created_at | | timestamp NULL DEFAULT NULL | 作成日時 |
...
**ログ記録タイミング:**
- POST /api/{domain}/{xxxx}: {処理完了時に1レコード記録など}
