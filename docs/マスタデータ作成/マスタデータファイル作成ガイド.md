...
d,chara_old_00001,piece_old_00001,Attack,Blue,Middle,...,202509010
```
...
- `e` (enabled): データが有効で、ゲームで使用される
- `d` (disabled): データが無効で、ゲームでは使用されない（削除せずに無効化）
...
データを削除する代わりに、`ENABLE` フラグを `d` に設定します。
