...
このWireFrameは画面遷移管理層であり、直接DBにアクセスしません。ただし、画面遷移時の引数として以下のマスタデータIDを受け渡しています：

| クライアント定義 | サーバー定義 | クライアント説明 |
|----------------|-------------|----------------|
| MasterDataId (ガチャID) | opr_gachas.id | ガチャコンテンツ画面への遷移時に使用 |
| MasterDataId (ユニットID) | mst_units.id | ユニット詳細モーダルへの遷移時に使用 |
| PlayerResourceModel.Id<br>(アイテムID) | mst_items.id | アイテム詳細画面への遷移時に使用（ResourceType.Itemの場合） |

**補足**：
- このWireFrameが受け渡すマスタデータの実際の内容は、各遷移先の画面（ガチャコンテンツ画面、ユニット詳細モーダル、アイテム詳細画面など）で使用されます
- `PlayerResourceModel`は他の画面から渡されるデータ構造で、リソースタイプ（ユニット/アイテム）とIDを含みます
- 画面遷移の判定に使用される`HomeContentTypes`はマスタデータではなく、クライアント側の定数です

#### ユーザーデータ（GameFetch）

このWireFrameは画面遷移管理層であり、ユーザーデータに直接アクセスしません。
