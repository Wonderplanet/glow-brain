...
|----------------|-------------|----------------|
| OprGacha.GachaFixedPrizeDescription | opr_gachas.gacha_fixed_prize_description | ガチャ確定枠の説明テキスト（例：「SR以上1体確定」） |
| MstCharacter.Id<br>（API経由で取得したMasterDataId） | mst_units.id | 排出対象ユニットID（ResourceTypeがUnitの場合） |
| MstCharacter.Name | mst_units.name | ユニット名（ラインナップリストに表示） |
| MstCharacter.Rarity | mst_units.rarity | ユニットのレアリティ（UR/SSR/SR/R） |
| MstCharacter.SortOrder | mst_units.sort_order | ユニット表示順序 |
| MstCharacter.AssetKey | mst_units.asset_key | ユニットアイコン表示用のアセットキー |
| MstItem.Id<br>（API経由で取得したMasterDataId） | mst_items.id | 排出対象アイテムID（ResourceTypeがItemの場合） |
| MstItem.Name | mst_items.name | アイテム名（ラインナップリストに表示） |
| MstItem.Description | mst_items.description | アイテム説明文 |
| MstItem.Rarity | mst_items.rarity | アイテムのレアリティ（UR/SSR/SR/R） |
| MstItem.Type | mst_items.type | アイテムタイプ（素材、チケットなど） |
| MstItem.SortOrder | mst_items.sort_order | アイテム表示順序 |
| MstItem.ItemAssetKey | mst_items.item_asset_key | アイテムアイコン表示用のアセットキー |

**データ取得の流れ**:
1. `GET /api/gacha/prize` APIから、各排出対象の`ResourceType`と`MasterDataId`と`Probability`（確率）を取得
2. `PlayerResourceModelFactory`が、`ResourceType`（UnitまたはItem）に応じて、対応するマスタデータを参照
   - `ResourceType.Unit`の場合: `MstCharacterDataRepository`から`MstCharacterModel`を取得
   - `ResourceType.Item`の場合: `MstItemDataRepository`から`MstItemModel`を取得
3. 取得したマスタデータから、名前・アイコン・レアリティなどを`PlayerResourceModel`に変換
4. 画面に表示
