...
|----------------|-------------|----------------|
| MstItem.Id | mst_items.id | アイテムのマスターデータID（画面初期化引数として渡される） |
| MstItem.Name | mst_items.name | アイテム名（画面に表示） |
| MstItem.Description | mst_items.description | アイテムの説明文（画面に表示） |
| MstItem.ItemAssetKey | mst_items.item_asset_key | アイテムアイコンのアセットキー（アイコン表示用） |
| OprGacha.GachaId | opr_gacha.id | ガチャID（開催中のガシャを特定） |
| OprGacha.StartAt | opr_gacha.start_at | ガチャ開始日時（開催中判定用） |
| OprGacha.EndAt | opr_gacha.end_at | ガチャ終了日時（開催中判定用） |
| OprGachaUseResource.OprGachaId | opr_gacha_use_resource.opr_gacha_id | ガチャID（コストとガチャの紐付け） |
| OprGachaUseResource.MstCostId | opr_gacha_use_resource.mst_cost_id | コストアイテムのマスターID（チケットとガチャの紐付け） |

#### ユーザーデータ（GameFetch）

| クライアント定義 | サーバー定義 | クライアント説明 |
|----------------|-------------|----------------|
| UserItem.MstItemId | user_items.mst_item_id | アイテムのマスターID（所持しているアイテムの特定） |
| UserItem.Amount | user_items.amount | アイテムの所持数（画面に表示） |
