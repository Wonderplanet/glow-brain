...
| テーブル名 | カラム名 | 説明 |
|-----------|---------|------|
| **OprGacha**<br>ガチャ基本情報 | GachaId | ガチャID（APIレスポンスのOprGachaIdに対応） |
| | GachaName | ガチャ名称（履歴セルに表示） |
| **MstCharacter** (mst_units)<br>キャラクターマスタ | Id | ユニットID（報酬がユニットの場合） |
| | Name | キャラクター名（履歴詳細で表示） |
| **MstItem**<br>アイテムマスタ | Id | アイテムID（コストタイプがItemの場合） |
| | ItemAssetKey | アイコン表示用のアセットキー（コストアイコン表示） |

#### ユーザーデータ（API取得）

| エンドポイント | データ名 | 説明 |
|-------------|---------|------|
| **GET `/api/gacha/history`**<br>ガチャ履歴API | GachaHistoryModels | ガチャ履歴データ配列（最大50件） |
| | ├ OprGachaId | 実行したガチャのID |
| | ├ CostType | 使用したコストタイプ（Diamond, Item, Ad等） |
| | ├ MstCostId | コストアイテムID（CostTypeがItemの場合） |
| | ├ CostAmount | 消費したコスト数量 |
| | ├ PlayedAt | ガチャ実施日時 |
| | └ GachaHistoryRewardModels | 獲得報酬データ配列（履歴詳細用） |
| | 　　├ SortOrder | 表示順序 |
| | 　　└ RewardModel | 報酬詳細（ResourceType, ResourceId, Amount等） |
