...
|----------------|-------------|----------------|
| OprGacha.GachaFixedPrizeDescription | opr_gacha.gacha_fixed_prize_description | 確定枠の説明文（タブに表示するテキスト） |
| MstCharacter.Name | mst_units.name | ユニットタイプのガチャ景品の名前表示 |

**データ取得元の補足**:
- ガチャ景品の詳細情報（ResourceType, MasterDataId, ResourceAmount, Probability, Pickup）は、`GET /api/gacha/prize` APIから直接取得されます
- APIレスポンスには以下が含まれます：
  - `GachaProbabilityGroupModel`: レアリティごとのグループ分け（Rarity, Prizes）
  - `GachaPrizeModel`: 各景品の情報（ResourceType, MasterDataId, ResourceAmount, Probability, Pickup）
- クライアント側では、APIから取得したMasterDataIdを使用して、必要に応じてMstCharacterRepositoryからキャラクター名を取得します
