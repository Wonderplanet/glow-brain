...
| テーブル名 | カラム名 | 説明 |
|-----------|---------|------|
| **OprGacha**<br>ガチャ基本情報 | GachaFixedPrizeDescription | 確定枠の説明文（例：「SR以上1体確定」） |
| **MstCharacter** (mst_units)<br>キャラクターマスタ | Name | キャラクター名（提供割合一覧で表示） |
| | AssetKey | キャラクターアセットキー（アイコン表示用） |

#### ゲーム仕様データ（API経由）

| データモデル | 用途 |
|-----------|------|
| **GachaPrizePageModel**<br>ガチャ提供割合ページ | 各タブに表示するレアリティ別の提供割合情報 |
| | - `RarityProbabilities`: レアリティごとの総提供確率 |
| | - `ProbabilityGroupModels`: レアリティ別のアイテム一覧と確率 |
| **GachaProbabilityGroupModel**<br>確率グループ | レアリティごとのアイテムと確率をグループ化 |
| | - `Rarity`: レアリティ（R, SR, SSR, UR） |
| | - `Prizes`: そのレアリティに属するアイテム一覧 |
| **GachaPrizeModel**<br>ガチャ排出アイテム | 個別のアイテム情報と確率 |
| | - `ResourceType`: リソースタイプ（Unit, Itemなど） |
| | - `MasterDataId`: ユニットまたはアイテムのID |
| | - `Probability`: 排出確率 |
| | - `Pickup`: ピックアップフラグ |
