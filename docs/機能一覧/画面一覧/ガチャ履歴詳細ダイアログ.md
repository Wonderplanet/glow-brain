...
このダイアログで表示するデータは、ガチャ履歴ダイアログ（GachaHistoryDialogUseCase）で取得したものを使用します。

#### マスタデータ

| クライアント定義 | サーバー定義 | クライアント説明 |
|----------------|-------------|----------------|
| OprGacha.GachaId | opr_gachas.id | ガチャIDの識別子 |
| OprGacha.GachaName | opr_gachas.gacha_name | ガチャ名の表示 |
| MstCharacter.Id<br>例：OprGacha.GachaId | mst_units.id | 獲得したユニットのID |
| MstCharacter.Name | mst_units.name | ユニットの表示名（キャラクター名） |
| MstItem.Id | mst_items.id | コストアイテムのID（CostTypeがItemの場合） |
| MstItem.ItemAssetKey | mst_items.item_asset_key | コストアイテムのアイコン表示用アセットキー |

#### ユーザーデータ（GameFetch / API取得）

| クライアント定義 | サーバー定義 | クライアント説明 |
|----------------|-------------|----------------|
| GachaHistoryModel.OprGachaId | gacha_histories.opr_gacha_id | ガチャ履歴のガチャID |
| GachaHistoryModel.CostType | gacha_histories.cost_type | ガチャのコストタイプ（ダイヤ、アイテム、広告など） |
| GachaHistoryModel.MstCostId | gacha_histories.cost_id | コストアイテムのマスタID |
| GachaHistoryModel.CostAmount | gacha_histories.cost_amount | 使用したコスト量 |
| GachaHistoryModel.PlayedAt | gacha_histories.played_at | ガチャ実行日時 |
| GachaHistoryRewardModel.SortOrder | gacha_history_rewards.sort_order | 獲得報酬の表示順序 |
| RewardModel.ResourceType | gacha_history_rewards.resource_type | 報酬のリソースタイプ（ユニット、アイテムなど） |
| RewardModel.ResourceId | gacha_history_rewards.resource_id | 報酬のリソースID（ユニットIDやアイテムID） |
| RewardModel.Amount | gacha_history_rewards.amount | 獲得個数 |
| RewardModel.PreConversionResource | gacha_history_rewards.pre_conversion_* | 変換前のリソース情報（重複ユニット→アイテム変換時） |
