...
| テーブル名 | カラム名 | 説明 |
|-----------|---------|------|
| **OprGacha**<br>ガチャ基本情報 | GachaName | ガチャ名（ガチャ情報ヘッダーに表示） |
| **MstCharacter** (mst_units)<br>キャラクターマスタ | Id | ユニットID（初獲得判定に使用） |
| | Name | キャラクター名（獲得アイテムに表示） |
| | Rarity | レアリティ（レアリティアイコン表示） |
| | AssetKey | キャラクターアセットキー（アイコン表示） |
...
| テーブル名 | カラム名 | 説明 |
|-----------|---------|------|
| **GachaHistory**<br>ガチャ実行履歴 | PlayedAt | ガチャ実行日時（ガチャ情報ヘッダーに表示） |
| | OprGachaId | ガチャID（ガチャ名取得用） |
| | CostType | コスト種別（コストアイコン表示用） |
| | MstCostId | コスト対象ID（コスト表示用） |
| | CostAmount | コスト量（コスト個数表示） |
| **GachaHistoryReward**<br>ガチャ獲得報酬 | ResourceType | リソースタイプ（ユニット/アイテム判定） |
| | ResourceId | リソースID（キャラ名・アイコン取得用） |
| | Amount | 獲得個数（獲得個数欄に表示） |
| | PreConversionResource | 変換前リソース情報（変換ガチャの場合） |
| | SortOrder | ソート順（表示順序） |
