...
| テーブル名 | カラム名 | 説明 |
|-----------|---------|------|
| **MstCharacter** (mst_units)<br>キャラクターマスタ | Id | ユニットID（初期化引数で指定） |
| | AssetKey | ユニットのSpineアニメーションアセットキー（SetupAvatarで使用） |
| | Color | キャラクターカラー（属性色、影表示で使用） |
| | Rarity | レアリティ（UI表示用） |

#### ユーザーデータ（GameFetch）

このコンポーネントはユーザーデータを直接参照していません（ガチャコンテンツ画面から引き渡されるパラメータのみを使用）。
