...
| | GachaType | ガチャタイプ（フェス、ピックアップ、無料、チケット、有償限定、メダル、プレミアム） |
| | GachaBannerAssetKey | ガチャバナーのアセットキー |
| | Description | ガチャ説明文 |
| | GachaPriority | ガチャ優先度（表示順序の決定に使用） |
| | StartAt | ガチャ開始日時 |
| | EndAt | ガチャ終了日時 |
| | EnableAdPlay | 広告ガチャ有効フラグ |
| | AdIntervalTimeMinutes | 広告ガチャの再生可能インターバル（分） |
| | MultiDrawCount | 10連ガチャの引く回数 |
| | DailyPlayLimitCount | 1日あたりの実行回数上限 |
| | TotalPlayLimitCount | 期間中の実行回数上限 |
| | DailyAdPlayLimitCount | 1日あたりの広告ガチャ実行回数上限 |
| | TotalAdPlayLimitCount | 期間中の広告ガチャ実行回数上限 |
...
| | UnlockDurationHours | 開放期間（時間数、N時間開放ガチャ用） |
| | AppearanceCondition | 表示条件（常に表示 or 対象アイテム所持時） |
| | AnnouncementId | アナウンスメントID |
| | GachaCautionId | ガチャ注意ID |
...
| | MstCostId | コストアイテムID（CostTypeがItemの場合に使用） |
| | CostAmount | コスト量 |
...
| | AdPlayedAt | 最後に広告ガチャを実行した日時 |
| | PlayedAt | 最後にガチャを実行した日時 |
| | AdPlayedCount | 広告ガチャ実行済み総回数 |
| | DailyAdPlayedCount | 本日の広告ガチャ実行回数 |
| | PlayedCount | ガチャ実行済み総回数（天井カウント用） |
| | DailyPlayedCount | 本日のガチャ実行回数 |
| | GachaExpireAt | 個別有効期限（N時間開放ガチャ用） |
| **UserGachaDrawCountThreshold**<br>ユーザー天井カウント | DrawCountThresholdGroupId | 天井グループID |
| | UpperType | 天井タイプ（`MaxRarity` or `Pickup`） |
| | GachaPlayedCount | 実行済み回数（天井までの回数をカウント） |
| **UserParameter**<br>ユーザーパラメータ | Diamond | ダイヤ所持量（無償+有償の合計） |
| | PaidDiamond | 有償ダイヤ所持量 |
| **UserItem**<br>ユーザーアイテム | MstItemId | アイテムID（チケット、メダルなど） |
| | Amount | 所持量 |
