...
**注記：GachaWireFrameは画面遷移管理層のため、直接DBデータにアクセスしません。**

ただし、このWireFrameが管理する各画面は、以下のマスタデータを参照しています：

#### 管理画面が使用するマスタデータ

##### ガチャコンテンツ画面（GachaContent）
参照: `docs/機能一覧/画面一覧/ガチャコンテンツ画面.md`

主要テーブル：
- **OprGacha**: ガチャ基本情報（ガチャID、名前、終了日時、コスト設定など）
- **OprGachaUpper**: ガチャ天井情報（天井回数、タイプ）
- **OprGachaUseResource**: ガチャコスト情報（必要ダイヤ数、チケット数など）
- **OprGachaDisplayUnit**: ガチャ表示ユニット情報
- **MstCharacter** (mst_units): キャラクターマスタ（ユニット情報）
- **MstItem**: アイテムマスタ

##### ガチャ提供割合ダイアログ（GachaRatio）
参照: `docs/機能一覧/画面一覧/ガチャ提供割合表示画面.md`

主要データ：
- **API `/api/gacha/prize`**から取得の提供割合情報
  - 各レアリティの出現確率
  - ピックアップ対象の個別確率
  - 確定枠ごとの確率情報

##### ガチャラインナップダイアログ（GachaLineup）
参照: `docs/機能一覧/画面一覧/ガチャラインアップダイアログ.md`

主要テーブル：
- **OprGacha**: ガチャ情報（コンテンツ数など）
- **OprGachaContent**: ガチャコンテンツ（排出対象ユニット・アイテム）
- **MstCharacter** (mst_units): キャラクターマスタ
- **MstItem**: アイテムマスタ

##### ユニット詳細モーダル（UnitDetailModal）
参照: `docs/機能一覧/画面一覧/ユニット詳細モーダル.md`

主要テーブル：
- **MstCharacter** (mst_units): キャラクターステータス、スキル、必殺技情報
- **MstCharacterLeague**: キャラクター所属リーグ
- **その他**: ユニット関連の全マスタデータ
...
管理対象の各画面は以下のユーザーデータも参照：
- **UserGacha**: ユーザーガチャ実行履歴（今回のガチャ実行を追跡）
- **UserGachaDrawCountThreshold**: ユーザー天井カウント（天井までの進捗）
- **UserParameter**: ユーザーリソース（ダイヤ、チケット所持量）
- **UserItem**: ユーザーアイテム（アイテムの所持数）
