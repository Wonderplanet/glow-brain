...
RSpec.describe "MstCharacterVariantVoiceTranslator" do
  subject { MstCharacterVariantVoiceTranslator.translate(use_case_data) }

  context do
    let(:use_case_data) {double(:use_case_data, id: 0, mst_character_variant_id: 1, name: 2, text: 3, voice_key: 4, list_display_type: 5, release_condition: 6, release_at: 7, releasable_start_at: 8, releasable_end_at: 9, sort_number: 10)}

    it do
      view_model = subject
      expect(view_model.is_a?(MstCharacterVariantVoiceViewModel)).to be_truthy
