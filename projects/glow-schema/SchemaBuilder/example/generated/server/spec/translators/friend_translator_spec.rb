...
RSpec.describe "FriendTranslator" do
  subject { FriendTranslator.translate(use_case_data) }

  context do
    let(:use_case_data) {double(:use_case_data, user_name: 0, user_id: 1, rank_level: 2, description: 3, last_accessed_at: 4, character_variant: 5)}

    it do
      view_model = subject
      expect(view_model.is_a?(FriendViewModel)).to be_truthy
