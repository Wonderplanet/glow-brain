...
class AdmS3ObjectService
{
     ...
     *
     * @param string|null $admS3BucketScopeId 同期対象のバケットスコープID（nullの場合は全て同期）
     */
    public function syncFromS3(?string $admS3BucketScopeId = null): void
    {
        $result = [];
        ...
        // 同期対象のバケットスコープと削除対象のハッシュリストを取得
        if ($admS3BucketScopeId !== null) {
            // 特定のバケットスコープのみ同期
            $admS3BucketScope = AdmS3BucketScope::getById($admS3BucketScopeId);
            ...
            // 単一のバケットスコープをエンティティに変換
            $bucketScopeEntities = [
                new S3BucketScopeEntity(
                    $admS3BucketScope->bucket,
                    collect([$admS3BucketScope->prefix])
                )
            ];
            // 部分同期：指定されたバケットとプレフィックスに一致するレコードのみを削除対象に
            $removeKeyHashes = AdmS3Object::getBucketKeyHashesByBucketAndPrefix(
                $admS3BucketScope->bucket,
        ...
        return Action::make('upload_assets')
            ...
            ->form([
                ...
                FileUpload::make('upload_files')
                    ->label('アセット')
                    ->multiple()
                    ->acceptedFileTypes(['image/png', 'image/jpeg', 'text/html'])
                    ->maxSize(5120) // 5MB
        ...
        return Action::make('sync_adm_s3_object_from_s3')
            ...
            ->form([
                Select::make('adm_s3_bucket_scope_id')
                    ...
                    ->required()
                    ->helperText('同期対象のS3フォルダを選択してください。'),
            ])
            ->action(function (array $data) {
                $this->syncFromS3($data['adm_s3_bucket_scope_id']);
            })
    ...
    public function getAddS3BucketScopeAction(): Action
    {
        return Action::make('add_s3_bucket_scope')
            ...
            ->form([
                Select::make('bucket')
                    ...
                    ->afterStateUpdated(function ($set) {
                        // バケット変更時に両方のprefixをリセット
                        $set('select_prefix', null);
                        $set('text_prefix', null);
                    })
                    ->required(),
                // prefix選択肢があるバケット用のSelect
                Select::make('select_prefix')
                    ->label('フォルダパス')
                    ->options(function (callable $get) {
                        $options = $this->getBucketPrefixOptions($get('bucket')) ?? [];
                        return array_combine(array_keys($options), array_keys($options));
                    })
                    ->visible(function (callable $get) {
                        return $this->hasPrefixOptions($get('bucket'));
                    })
                    ->afterStateUpdated(function (callable $set, callable $get, ?string $state) {
                        // 選択されたprefixに対応するラベルからmemoを自動設定
                        if ($state !== null) {
                            $options = $this->getBucketPrefixOptions($get('bucket'));
                            if ($options !== null && isset($options[$state])) {
                                $set('memo', $options[$state]);
                            }
                        }
                    })
                    ->reactive()
                    ->required(),
                // prefix選択肢がないバケット用のTextInput（自由入力）
                TextInput::make('text_prefix')
                    ->label('フォルダパス')
                    ->visible(function (callable $get) {
                        return $get('bucket') !== null && !$this->hasPrefixOptions($get('bucket'));
                    })
                    ->required()
            ...
            ->action(function (array $data) {
                // 表示されているフィールドからprefixを取得
                $prefix = $this->hasPrefixOptions($data['bucket'])
                    ? $data['select_prefix']
                    : $data['text_prefix'];

                // 既存チェック
                if (AdmS3BucketScope::existsBucketPrefix($data['bucket'], $prefix)) {
                    $this->sendDangerNotification(
                        '登録済みのフォルダ',
                        "バケット「{$data['bucket']}」のフォルダ「{$prefix}」は既に登録されています。",
                    );
                    return;
                }
                ...
                $this->sendProcessCompletedNotification(
                    'S3フォルダ追加完了',
                    "バケット「{$data['bucket']}」のフォルダ「{$prefix}」を追加しました。",
                );
            })
            ->requiresConfirmation()
    ...
    private function getSelectableBuckets(): array
    {
        return [
            ...
            $this->configGetService->getS3WebviewBucket() => 'Webview',
        ];
    }

    /**
     * バケットごとのprefix選択肢を取得
     * 選択肢がある場合は配列を返し、自由入力の場合はnullを返す
     *
     * @param string|null $bucket バケット名
     * @return array<string, string>|null prefix選択肢、または自由入力の場合はnull
     */
    private function getBucketPrefixOptions(?string $bucket): ?array
    {
        if ($bucket === null) {
            return null;
        }

        $webviewBucket = $this->configGetService->getS3WebviewBucket();

        // key: prefix, value: label
        return match ($bucket) {
            $webviewBucket => [
                'policies' => 'ポリシー',
                'policies/plugin_licences' => '権利表記',
            ],
            default => null,
        };
    }
    ...
    /**
     * バケットがprefix選択肢を持つかどうか
     *
     * @param string|null $bucket バケット名
     * @return bool 選択肢がある場合はtrue、自由入力の場合はfalse
     */
    private function hasPrefixOptions(?string $bucket): bool
    {
        return $this->getBucketPrefixOptions($bucket) !== null;
    }
