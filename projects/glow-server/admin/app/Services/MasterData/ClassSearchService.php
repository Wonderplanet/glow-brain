...
class ClassSearchService
{
    ...
    public function verifyMasterModelClassName(string $className): bool {
        try {
            if (
                class_exists("\App\Domain\Resource\Mst\Models\\" . $className) || class_exists("\App\Domain\Resource\Mst\Models\\" . $className) ||
                class_exists("\App\Domain\Resource\Mst\Models\\" . $className) || class_exists("\App\Domain\Resource\Mst\Models\\" . $className) ||
    ...
    public function createMasterModelClass(string $title): mixed
    {
        if ($this->verifyMasterModelClassName($title)) {
            if (class_exists("\App\Domain\Resource\Mst\Models\\" . $title)) {
                return new ("\App\Domain\Resource\Mst\Models\\" . $title);
            ...
            } else if (class_exists("\App\Domain\Resource\Mst\Models\\" . $title)) {
                return new ("\App\Domain\Resource\Mst\Models\\" . $title);
            }
        }
        return null;
