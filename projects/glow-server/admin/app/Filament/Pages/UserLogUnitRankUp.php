...
use App\Models\Log\LogUnitRankUp;
...
class UserLogUnitRankUp extends UserDataBasePage implements HasTable
{
    ...
    protected static string $view = 'filament.pages.user-log-unit-rank-up';

    public string $currentTab = UserSearchTabs::LOG_UNIT_RANK_UP->value;
    ...
    private function table(Table $table)
    {
        ...
        $addColumn = [
            TextColumn::make('before_rank')
                ->label('強化前のランク')
                ->searchable()
                ->sortable(),
            TextColumn::make('after_rank')
                ->label('強化後のランク')
                ->searchable()
                ->sortable(),
        ...
        return $table
            ...
            ->headerActions([
                SimpleCsvDownloadAction::make()
                    ->fileName('user_log_unit_rank_up')
            ]);
    }
}
