...
class GachaSimulator extends Page
{
    ...
    public function mount()
    {
        ...
        $this->oprGacha = OprGacha::query()
            ...
            ->first();
        if (is_null($this->oprGacha)) {
            $this->sendDangerNotification(
                '指定されたガシャIDのガシャが見つかりません。',
                sprintf('ガシャID: %s', $this->oprGachaId),
            );
            $this->redirect(GachaSimulatorResource::getUrl());
            return;
        }
    ...
    {
        $this->oprGachaType = $this->oprGacha->gacha_type_label;

        return [
            'ガシャID' => $this->oprGacha->id,
            'ガシャ名' => $this->oprGacha->opr_gacha_i18n?->name ?? '',
            'ガシャタイプ' => $this->oprGacha->gacha_type_label,
            '説明' => $this->oprGacha->opr_gacha_i18n?->description ?? '',
            '開始日時' => $this->oprGacha->start_at,
            '終了日時' => $this->oprGacha->end_at,
        ];
    }

    private function getPickUp(): array
    {
        $oprGachaPrizes = $this->gachaSimulatorService->getOprGachaPrizesByOprGacha($this->oprGacha, $this->stepNumber ?? 1)
            ->filter(function (OprGachaPrize $oprGachaPrize) {
                return $oprGachaPrize->pickup;
            });
    ...
    private function getUpperTableRows(): array
    {
        ...
        $oprGachaUppers = $this->oprGacha->opr_gacha_uppers;
        $oprGachaI18n = $this->oprGacha?->opr_gacha_i18n;
    ...
    public function getRarityProbability(): array
    {
        ...
        $rarityProbabilities = $this->adminGachaService->getGachaRarityProbabilitiesArray(
            $this->oprGacha,
            $this->stepNumber ?? 1,
        );

        $raritys = array_column($rarityProbabilities, 'rarity');
        array_multisort($raritys, SORT_DESC, $rarityProbabilities);
        foreach ($rarityProbabilities as $rarityProbability) {
            $row = [
                'レアリティ' => $rarityProbability['rarity'],
                '提供割合（％）' => $rarityProbability['probability'],
            ];
    ...
    {
        $oprGacha = $this->oprGacha->toEntity();

        $gachaLotteryBoxData = $this->adminGachaService->getGachaLotteryBox($oprGacha, $this->stepNumber);
    ...
    {
        $targetSimulationData = $admGachaSimulationLog->getSimulationDataByParams(
    ...
    public function simulation(): void
    {
        ...
        $oprGachaEntity = $this->oprGacha->toEntity();

        // ガチャ抽選BOX取得
        $gachaLotteryBoxData = $this->adminGachaService->getGachaLotteryBox($oprGachaEntity, $this->stepNumber);
        ...
        $mstGachaDataHash = $this->gachaSimulatorService->makeMstGachaDataHash(
            $this->oprGacha,
            $this->gachaSimulatorService->getOprGachaPrizesByOprGacha($this->oprGacha),
        );
                ...
                function () use ($admGachaSimulationLog) {
                    try {
                        ...
                        return $this->gachaSimulationExcelService->downloadGachaResults(
                            ...
                            $simulationDate,
                            $admGachaSimulationLog->getSimulationDataByParams(
                                $this->prizeType->value,
                                $this->stepNumber,
                            ) ?? [],
    ...
    {
        $oprGachaPrizes = $this->gachaSimulatorService->getOprGachaPrizesByOprGacha($this->oprGacha, $this->stepNumber)
            ->groupBy(function (OprGachaPrize $oprGachaPrize) {
                return $oprGachaPrize->group_id;
            });

        // 最低排出率の取得
        $minRate = 100;
        foreach ($oprGachaPrizes as $targetOprGachaPrizes) {
            $totalWeight = $targetOprGachaPrizes->sum('weight');
            if ($totalWeight === 0) {
                // 重みが0の場合はシミュレーション数を0に設定
                return 0;
            }
            $targetMinRate = ($targetOprGachaPrizes->min('weight') / $totalWeight) * 100;

            $minRate = min($minRate, $targetMinRate);
        }
    ...
    {
        // ステップアップガシャの場合、選択されたステップに応じた抽選枠を返す
        if ($this->oprGacha->toEntity()->isStepup() && $this->stepNumber !== null) {
            $oprStepupGachaStep = OprStepupGachaStep::query()
                ->where('opr_gacha_id', $this->oprGachaId)
                ->where('step_number', $this->stepNumber)
                ->first();

            if ($oprStepupGachaStep === null) {
                return [];
            }
            ...
            if (
                StringUtil::isSpecified($this->oprGacha->prize_group_id)
                || StringUtil::isSpecified($oprStepupGachaStep->prize_group_id)
            ) {
                $result[GachaPrizeType::REGULAR->value] = GachaSetName::REGULAR->value;
            }

            // fixed_prize_group_idがnullでない場合のみ固定枠を表示
        ...
        if ($this->oprGacha->toEntity()->hasMultiFixedPrize()) {
            $result[GachaPrizeType::FIXED->value] = GachaSetName::FIXED->value;
        }
        foreach ($oprGachaUppers as $oprGachaUpper) {
    ...
    /**
     * ステップアップガシャの場合に、ステップ選択肢を取得
     *
     * @return array<int, string>
     */
    public function getStepOptions(): array
    {
        if (!$this->oprGacha->toEntity()->isStepup()) {
            return [];
        }

        // ステップガシャ基本設定から最大ステップ数を取得
        ...
        $maxStepNumber = $oprStepupGacha->max_step_number;

        $oprStepupGachaSteps = OprStepupGachaStep::query()
            ->where('opr_gacha_id', $this->oprGachaId)
            ->where('step_number', '<=', $maxStepNumber)
            ->get()
            ->sortBy('step_number');

        $options = [];
        foreach ($oprStepupGachaSteps as $oprStepupGachaStep) {
            $options[$oprStepupGachaStep->step_number] = sprintf(
                'ステップ %d (%d連)',
                $oprStepupGachaStep->step_number,
                $oprStepupGachaStep->draw_count
            );
        }
     ...
     * ステップ選択変更時の処理
     *
     * wire:model.live="stepNumber"により、変更時に自動で呼ばれる
     */
    public function updatedStepNumber()
    {
        // ステップアップガシャの場合、選択された抽選枠がステップで利用可能かチェック
        if ($this->oprGacha->toEntity()->isStepup() && $this->stepNumber !== null) {
            $availablePrizeTypes = $this->getPrizeTypeList();

            // 現在選択されている抽選枠が利用できない場合、最初の利用可能な抽選枠にリセット
            if (!array_key_exists($this->prizeType->value, $availablePrizeTypes)) {
                $firstAvailablePrizeType = array_key_first($availablePrizeTypes);
                if ($firstAvailablePrizeType !== null) {
                    $this->prizeType = GachaPrizeType::from($firstAvailablePrizeType);
                }
            }
        }
        ...
        // 既存のupdatedPrizeType()と同じくシミュレーション結果を更新
        $this->updatedPrizeType();
    }

    public function isVisibleStepupView(): bool
    {
        return $this->oprGacha->toEntity()->isStepup();
    }
}
