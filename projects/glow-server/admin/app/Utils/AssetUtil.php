...
class AssetUtil
{
    ...
    public static function makeS3BannerUrlWithCacheBusting(string $key, ?string $version = null): string
    {
        ...
        return self::appendCacheBustingQuery($url, $version);
    }

    public static function makeS3WebviewUrl(string $key): string
    {
        return StringUtil::joinPath(
            config('filesystems.disks.s3_webview.url', ''),
            $key,
        );
    }

    /**
     * URLにキャッシュバスティング用のクエリパラメータを付与
     *
     * @param string $url ベースURL
     * @param string|null $version キャッシュバスティング用の値（etag、timestamp等）
     * @return string キャッシュバスティング付きのURL
     */
    private static function appendCacheBustingQuery(string $url, ?string $version = null): string
    {
        if (StringUtil::isNotSpecified($version)) {
            return $url;
        }
        $version = trim($version, '"');
        $separator = str_contains($url, '?') ? '&' : '?';
        $query = http_build_query(['v' => $version], '', '&', PHP_QUERY_RFC3986);
        return $url . $separator . $query;
    }

    /**
     * S3 WebviewURLにキャッシュバスティング用のクエリパラメータを付与して返す
     *
     * @param string $key S3オブジェクトのキー
     * @param string|null $version キャッシュバスティング用の値（etag、timestamp等）
     * @return string キャッシュバスティング付きのURL
     */
    public static function makeS3WebviewUrlWithCacheBusting(string $key, ?string $version = null): string
    {
        $url = self::makeS3WebviewUrl($key);
        return self::appendCacheBustingQuery($url, $version);
    }

    /**
     * バケット名に応じてキャッシュバスティング付きS3 URLを生成
     *
     ...
     * @return string キャッシュバスティング付きのURL
     */
    public static function makeS3UrlWithCacheBusting(string $bucket, string $key, ?string $version = null): string
    {
        $webviewBucket = config('filesystems.disks.s3_webview.bucket');

        return match ($bucket) {
            $webviewBucket => self::makeS3WebviewUrlWithCacheBusting($key, $version),
            default => self::makeS3BannerUrlWithCacheBusting($key, $version),
        };
    }
