...
CREATE EXTERNAL TABLE `glow_prod_user_action_logs`.`log_system_message_additions` (
    ...
    `trigger_source` string COMMENT '登録経緯情報ソース',
    `trigger_value` string COMMENT '登録経緯情報値',
    `pre_grant_reward_json` string COMMENT '配布予定の報酬情報',
...
LOCATION
  's3://glow-prod-datalake/raw/tidb/log_system_message_additions'
TBLPROPERTIES (
  'classification'='csv',
  ...
  'storage.location.template' = 's3://glow-prod-datalake/raw/tidb/log_system_message_additions/${dt}/'
);
