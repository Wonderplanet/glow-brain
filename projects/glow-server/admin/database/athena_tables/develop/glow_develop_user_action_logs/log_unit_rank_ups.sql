-- log_unit_rank_ups
CREATE EXTERNAL TABLE `glow_develop_user_action_logs`.`log_unit_rank_ups` (
    ...
    `mst_unit_id` string COMMENT 'ランクアップしたユニット(mst_units.id)',
    `before_rank` int COMMENT '強化前のランク',
    `after_rank` int COMMENT '強化後のランク',
...
LOCATION
  's3://glow-develop-datalake/raw/tidb/log_unit_rank_ups'
TBLPROPERTIES (
  'classification'='csv',
  ...
  'storage.location.template' = 's3://glow-develop-datalake/raw/tidb/log_unit_rank_ups/${dt}/'
);
