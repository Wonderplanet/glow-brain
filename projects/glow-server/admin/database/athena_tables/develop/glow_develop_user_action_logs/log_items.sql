-- log_items
CREATE EXTERNAL TABLE `glow_develop_user_action_logs`.`log_items` (
    ...
    `action_type` string COMMENT 'Get: 獲得 Use: 消費',
    `mst_item_id` string COMMENT 'mst_items.id',
    `before_amount` bigint COMMENT '変動前の量',
    `after_amount` bigint COMMENT '変動後の量',
...
LOCATION
  's3://glow-develop-datalake/raw/tidb/log_items'
TBLPROPERTIES (
  'classification'='csv',
  ...
  'storage.location.template' = 's3://glow-develop-datalake/raw/tidb/log_items/${dt}/'
);
