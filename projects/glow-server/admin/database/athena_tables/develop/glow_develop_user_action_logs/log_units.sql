-- log_units
CREATE EXTERNAL TABLE `glow_develop_user_action_logs`.`log_units` (
    ...
    `mst_unit_id` string COMMENT 'mst_units.id',
    `level` int COMMENT 'レベル',
    `rank` int COMMENT 'ランク',
    `grade_level` int COMMENT 'グレードレベル',
    `trigger_source` string COMMENT '経緯情報ソース',
    `trigger_value` string COMMENT '経緯情報値1',
    `trigger_value_2` string COMMENT '経緯情報値2',
    `trigger_value_3` string COMMENT '経緯情報値3',
...
LOCATION
  's3://glow-develop-datalake/raw/tidb/log_units'
TBLPROPERTIES (
  'classification'='csv',
  ...
  'storage.location.template' = 's3://glow-develop-datalake/raw/tidb/log_units/${dt}/'
);
