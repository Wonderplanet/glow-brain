...
class UsrModelManager
{
     ...
     *
     * キャッシュにない場合は、新規追加する。
     * キャッシュにある場合は、skipCacheOverwriteがtrueの場合は上書きせずにスキップし、falseの場合は上書きする。
     *
     * @param array<UsrModelInterface> $targetModels
     * @param bool $skipCacheOverwrite キャッシュ上書きをスキップするかどうか。キャッシュへの新規追加はどちらでも行う。
     *   true: キャッシュ上書きしない, false: キャッシュ上書きする
     * @return void
     */
    private function syncModelsInternal(
        string $repositoryClass,
        array $targetModels,
        bool $skipCacheOverwrite = false,
    ): void {
        ...
        foreach ($targetModels as $targetModel) {
            ...
            if (isset($models[$modelKey]) && $skipCacheOverwrite) {
                // DBから取得したままのモデルでキャッシュを上書きしてしまうと、デグレしてしまうため、上書きしない
                continue;
            }
            // キャッシュへの追加または上書きを行う

            /**
             * モデルの値の変更をN回行った末、DBから取得した時点のデータと同じになるパターンもあるため、
             * DBから取得した時点と比較して変更がない(isChanged=false)場合でもキャッシュを上書きする。
             *
             * GLOWでの不具合例：コインをシステム上限分所持している状態で、交換所で、コインを消費して、エンブレムを獲得するケース
             *   1. コイン999,999を所持している状態で、UsrUserParameterモデルインスタンスをキャッシュに追加
             *   2. 交換所でコインを500消費して、コイン999,499の状態でキャッシュを上書き
             *   3. エンブレムを獲得したが、重複していたのでコイン1000に変換された結果、上限キャップにかかり、コイン999,999になる
             *   4. コイン999,999 の状態でキャッシュを上書きしようとするが、
             *      DBから取得した状態と比較して変更がない(isChanged=false)ため、キャッシュを上書きしない
             *   5. 結果として、キャッシュ上のコイン999,499が残ってしまい、デグレしてしまう。
             */
    ...
    {
        $this->syncModelsInternal($repositoryClass, $targetModels, skipCacheOverwrite: false);
    }

    /**
     * キャッシュへ複数モデルの追加のみを行うメソッド。上書きはしない。
     * UsrModelCacheRepositoryとその派生クラスから呼び出すメソッドとして用意。
     *
     * DBから取得したモデルをキャッシュに詰め込む場合、
     * キャッシュにないモデルであれば、キャッシュに新規追加するが、
     * すでにキャッシュにあるモデルの場合、DBから取得したモデルでキャッシュを上書きしてしまうと、
     * ビジネスロジック側での変更がデグレしてしまう可能性があるため、上書きはしないようにする。
     * そのため、syncModelsとは別メソッドとして用意。
     *
     * @param string $repositoryClass
     * @param array<UsrModelInterface> $targetModels
     * @return void
     */
    public function addModelsIfAbsent(string $repositoryClass, array $targetModels): void
    {
        $this->syncModelsInternal($repositoryClass, $targetModels, skipCacheOverwrite: true);
    }
