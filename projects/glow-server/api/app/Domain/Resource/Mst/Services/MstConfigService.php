...
use App\Domain\BoxGacha\Constants\BoxGachaConstant;
...
class MstConfigService
{
    ...
    /**
     * BOXガチャの1回のリクエストで抽選できる最大回数を取得
     * @return int
     */
    public function getBoxGachaMaxDrawCount(): int
    {
        $key = MstConfigConstant::BOX_GACHA_MAX_DRAW_COUNT;
        $value = $this->mstConfigRepository->getValueByKey($key);

        if (is_null($value)) {
            $value = BoxGachaConstant::DEFAULT_MAX_DRAW_COUNT;
        }
     ...
     * @return string
     */
    public function getDefaultArtworkPartyArtworkId(): string
    {
        $key = MstConfigConstant::DEFAULT_ARTWORK_PARTY_ARTWORK_ID;
        $value = $this->mstConfigRepository->getValueByKey($key);

        if (is_null($value)) {
            // デフォルト値（本番環境ではmst_configsで設定される想定）
            $value = PartyConstant::DEFAULT_ARTWORK_ID;
        }
