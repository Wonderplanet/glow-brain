...
use App\Domain\Resource\Mst\Entities\MstArtworkEntity as Entity;
use App\Domain\Resource\Mst\Models\MstArtwork as Model;
use App\Infrastructure\MasterRepository;
use Illuminate\Support\Collection;

class MstArtworkRepository
{
    ...
    public function getById(string $id, bool $isThrowError = false): ?Entity
    {
        ...
        if ($isThrowError && $entities->isEmpty()) {
            throw new GameException(
                ...
                sprintf(
                    'mst_artworks record is not found. (id: %s)',
                    $id
                ),
            );
        }
    ...
    public function getByIds(Collection $ids): Collection
    {
        ...
        return $this->getAll()
            ->filter(function ($entity) use ($ids) {
