...
use App\Domain\Resource\Mst\Models\MstOutpostEnhancement as Model;
use App\Infrastructure\MasterRepository;
use Illuminate\Support\Collection;

class MstOutpostEnhancementRepository
{
    ...
    public function getById(string $id, bool $isThrowError = false): Entity
    {
        ...
        if ($isThrowError && $entities->isEmpty()) {
            throw new GameException(
                ...
                sprintf(
                    'mst_outpost_enhancements record is not found. (id: %s)',
                    $id
                ),
            );
        }
    ...
    {
        return $this->getAll()->filter(function (Entity $entity) use ($ids) {
