...
class OprProductRepository
{
     ...
     */
    public function getByMstProductIds(array $mstProductIds, CarbonImmutable $now): Collection
    {
        return $this->getAll()
            ->filter(function (OprProductEntity $entity) use ($mstProductIds, $now) {
                if (!in_array($entity->getMstStoreProductId(), $mstProductIds, true)) {
                    return false;
                }
