...
abstract class UsrModelCacheRepository extends UsrModelRepository implements UsrModelCacheRepositoryInterface
{
    ...
    /**
     * キャッシュへ複数モデルの追加のみを行うメソッド。
     * 上書きはしない。
     * @param Collection<UsrModelInterface> $models
     * @return void
     */
    protected function addModelsIfAbsent(Collection $models): void
    {
        if ($models->isEmpty()) {
            return;
        }
        ...
        $this->usrModelManager->addModelsIfAbsent($this::class, $targetModels);
    }

    protected function addModelIfAbsent(UsrModelInterface $model): void
    {
        $this->addModelsIfAbsent(collect([$model]));
    }
