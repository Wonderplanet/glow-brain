...
class EncyclopediaDelegator
{
    ...
    /**
     * 指定された原画ID配列のusrArtworkを取得する
     * @param string $usrUserId
     * @param Collection<string> $mstArtworkIds
     * @return Collection<UsrArtworkEntity> 所持している原画IDのコレクション
     */
    public function getUsrArtworksByMstArtworkIds(string $usrUserId, Collection $mstArtworkIds): Collection
    {
        return $this->usrArtworkRepository
     ...
     */
    public function createUnownedUsrArtworkFragments(string $usrUserId, Collection $mstArtworkFragmentIds): void
    {
        $this->encyclopediaService->createUnownedUsrArtworkFragments($usrUserId, $mstArtworkFragmentIds);
    }
    ...
    {
        $this->encyclopediaService->addArtworkRewardWhenArtworkCompleted($usrUserId, $rewards);
    }
}
