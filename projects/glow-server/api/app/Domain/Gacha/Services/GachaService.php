...
class GachaService
{
     ...
     *
     * @return Collection<OprGachaUpperEntity>
     */
    public function getOprGachaUppers(string $upperGroup): Collection
    {
        return $this->oprGachaUpperRepository->getByUpperGroup($upperGroup);
    }
    ...
    public function getGachaLotteryBox(OprGachaEntity $oprGacha): GachaLotteryBox
    {
        ...
        // 排出対象のユニットと対象ガチャの全排出アイテムのマスターデータを取得しておく
        ...
        // OprGachaEntityにレアリティを付与したデータクラスに変換
    ...
    public function setConsumeResource(
        ...
        if (
            ...
            if ($oprGachaUseResource === null) {
                ...
                $consumeCostNum = $oprGachaUseResource->getCostNum() * $playNum;
            } else {
                $consumeCostNum = $oprGachaUseResource->getCostNum();
            }
        } else {
            $oprGachaUseResource = $this->oprGachaUseResourceRepository->getByIdAndCostTypeAndDrawCount(
                $oprGacha->getId(),
        ...
        // コストIDの妥当性をチェック
        // 両方とも空（null or 空文字）の場合は妥当、それ以外は一致している必要がある
        $requestCostIdIsEmpty = is_null($costId) || $costId === '';

        $resourceCostId = $oprGachaUseResource->getCostId();
        $resourceCostIdIsEmpty = is_null($resourceCostId) || $resourceCostId === '';
        ...
        $this->costConsumptionManager->setConsumeResource(
            ...
            $billingPlatform,
            $checkedAd,
            $oprGachaUseResource->getCostType(),
            $gachaTrigger
        );
    }
    ...
    public function draw(
        ...
        $regularDrawCount = $drawCount - $oprGacha->getMultiFixedPrizeCount();
...
//        $this->cacheService->setGachaProbability($oprGachaId, $gachaProbabilityData);
     ...
     * @return Collection
     */
    private function generateRarityProbability(Collection $lotteryBox): Collection
    {
        // レアリティごとの確率を計算
     ...
     */
    private function generatePrizeProbabilityByPrize(Collection $prizeBox): Collection
    {
