...
class GachaService
{
    ...
    public function makeGachaRewardByGachaBoxes(
        ...
        return $gachaBoxes->map(function (GachaBoxInterface $gachaBox, int $index) use ($oprGachaId, $prizeTypes) {
            return new GachaReward(
                ...
                $oprGachaId,
                $index,
                $prizeTypes[$index] ?? GachaPrizeType::REGULAR->value,
            );
        });
    }
     ...
     * @param bool                               $isDrawAd
     * @param ?int                                $fixedPrizeCount
     * @param bool                               $skipMinimumDrawCountCheck 確定枠の最低連数チェックをスキップするか（ステップアップガチャ用）
    ...
    public function executeLottery(
        ...
        bool $skipMinimumDrawCountCheck = false
    ): GachaResultData {
        ...
        foreach (range(1, $drawCount) as $count) {
            ...
            if ($this->isReachUpper($usrGachaUpperPickup, $oprGachaUpperPickup)) {
                ...
                if ((!$skipMinimumDrawCountCheck && $drawCount < GachaConstants::MINIMUM_DRAW_COUNT_FOR_FIXED)
                    || $count <= $regularDrawCount
                ) {
                    // 最低連数チェック未スキップで10連未満か、通常枠の範囲は通常抽選
