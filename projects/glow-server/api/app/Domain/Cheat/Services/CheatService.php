...
class CheatService
{
    ...
    public function initBattleStatusMismatch(
        ...
        $artworkPartyStatusArray = $artworkPartyStatus->map(function ($artworkStatus) {
            return $artworkStatus->formatToLog();
        })->toArray();

        $sessionData = new CheatSessionParty(
            collect($partyStatusesArray),
            collect($artworkPartyStatusArray),
        );
        $usrCheatSession->setPartyStatus($cheatContentType, $targetId, $sessionData);
    ...
    public function checkBattleStatusMismatch(
        ...
        return $this->executeCheck(
            ...
            function (MstCheatSettingEntity $mstCheatSetting) use (
                ...
                $currentData = new CheatSessionParty(
                    collect($partyStatusesArray),
                    collect($artworkPartyStatusArray),
                );

                $savedData = json_decode($usrCheatSession->getPartyStatus(), true);
                if ($savedData === null) {
                    $savedData = [];
                }
                $beforeBattleData = CheatSessionParty::fromArray($savedData);
                ...
                return [
                    CheatConstant::LOG_DETAIL_KEY_BEFORE_BATTLE_PARTY_STATUSES => $beforeBattleData->toArray(),
                    CheatConstant::LOG_DETAIL_KEY_PARTY_STATUSES => $currentData->toArray(),
                ];
            }
        );
    }
    ...
    public function checkOpponentBattleStatusMismatch(
        ...
        return $this->executeCheck(
            ...
            function (MstCheatSettingEntity $mstCheatSetting) use (
                ...
                // TODO: 現在実装中で止まっている。
                //       usrCheatSessionに対戦相手情報を保持しチェックする形へ修正する必要がある。
                $beforeBattleData = CheatSessionParty::fromArray($savedData);
                $beforeBattlePartyStatusesArray = $beforeBattleData->getPartyStatuses()->toArray();
