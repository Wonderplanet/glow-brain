...
class CheatService
{
     ...
     * @param Collection<\App\Domain\Resource\Entities\ArtworkPartyStatus> $artworkPartyStatuses
     * @return void
     */
    public function initBattleStatusMismatch(
        ...
        Collection $artworkPartyStatuses,
    ): void {
        $usrCheatSession = $this->usrCheatSessionRepository->findOrCreate($usrUserId);
        $partyStatusesForLog = $partyStatuses->map(function ($partyStatus) {
            return $partyStatus->formatToLog();
        })->toArray();

        $artworkPartyStatusesForLog = $artworkPartyStatuses->map(function ($artworkStatus) {
            return $artworkStatus->formatToLog();
        })->toArray();

        $cheatSessionParty = new CheatSessionParty(
            collect($partyStatusesForLog),
            collect($artworkPartyStatusesForLog),
        );
        $usrCheatSession->setPartyStatus(
            $cheatContentType,
            $targetId,
            $cheatSessionParty
        );
    ...
    /**
     * バトル前後のパーティステータス不一致によるチート判定の準備を行う（対戦相手データ含む、PvP等で使用）
     *
     * @param string $usrUserId
     * @param string $targetId
     * @param string $cheatContentType
    ...
    public function initBattleStatusMismatchWithOpponent(
        ...
        // 対戦相手のパーティデータ（フル情報）
        $opponentPartyStatusesForLog = $opponentPartyStatuses->map(function ($partyStatus) {
            return $partyStatus->formatToLog();
        })->toArray();

        $opponentArtworkPartyStatusesForLog = $opponentArtworkPartyStatuses->map(function ($artworkStatus) {
            return $artworkStatus->formatToLog();
        })->toArray();

        $cheatSessionPartyWithOpponent = new CheatSessionPartyWithOpponent(
            collect($partyStatusesForLog),
            collect($artworkPartyStatusesForLog),
            collect($opponentPartyStatusesForLog),
            collect($opponentArtworkPartyStatusesForLog),
        );
        $usrCheatSession->setPartyStatusWithOpponent(
            $cheatContentType,
            $targetId,
            $cheatSessionPartyWithOpponent
        );
        $this->usrCheatSessionRepository->syncModel($usrCheatSession);
    }
    ...
    public function checkBattleStatusMismatch(
        ...
        return $this->executeCheck(
            ...
            function (MstCheatSettingEntity $mstCheatSetting) use (
                ...
                $afterCheatSessionParty = new CheatSessionParty(
                    collect($partyStatusesForLog),
                    collect($artworkPartyStatusesForLog),
                );

                $saveData = json_decode($usrCheatSession->getPartyStatus(), true);
                if ($saveData === null) {
                    $saveData = [];
                }
                $beforeCheatSessionParty = CheatSessionParty::fromArray($saveData);
                ...
                if (
                    ...
                    $this->arraysAreEqualRecursiveWithTolerance(
                        $beforeCheatSessionParty->toArray(),
                        $afterCheatSessionParty->toArray()
                    )
                ) {
                    return null;
                }
                ...
                return [
                    CheatConstant::LOG_DETAIL_KEY_BEFORE_BATTLE_PARTY_STATUSES => $beforeCheatSessionParty->toArray(),
                    CheatConstant::LOG_DETAIL_KEY_PARTY_STATUSES => $afterCheatSessionParty->toArray(),
                ];
            }
        );
    }
    ...
    /**
     * バトル前後のパーティステータス不一致によるチート判定を行う（対戦相手データ含む、PvP等で使用）
     * チェック時に必要な項目のみを抽出して比較する
     *
     * @param string $usrUserId
     * @param string $targetId 対戦ID
     * @param string $cheatContentType
     * @param CarbonImmutable $now
     * @param Collection<\App\Domain\Resource\Entities\PartyStatus> $partyStatuses
     * @param Collection<\App\Domain\Resource\Entities\ArtworkPartyStatus> $artworkPartyStatuses
    ...
    public function checkBattleStatusMismatchWithOpponent(
        ...
        return $this->executeCheck(
            ...
            function (MstCheatSettingEntity $mstCheatSetting) use (
                ...
                $afterCheatSessionPartyWithOpponent = new CheatSessionPartyWithOpponent(
                    collect($partyStatusesForLog),
                    collect($artworkPartyStatusesForLog),
                    collect($opponentPartyStatusesForLog),
                    collect($opponentArtworkPartyStatusesForLog),
                ...
                $beforeCheatSessionPartyWithOpponent = CheatSessionPartyWithOpponent::fromArray($saveData);

                // チェック時に必要な項目のみを抽出して比較
                if (
                    $usrCheatSession->getContentType() === $cheatContentType &&
                    $usrCheatSession->getTargetId() === $targetId &&
                    $this->arraysAreEqualRecursiveWithTolerance(
                        $beforeCheatSessionPartyWithOpponent->formatForCheatCheck(),
                        $afterCheatSessionPartyWithOpponent->formatForCheatCheck()
                    )
                ) {
                    return null;
                }
                ...
                return [
                    CheatConstant::LOG_DETAIL_KEY_BEFORE_BATTLE_PARTY_STATUSES => $beforeCheatSessionPartyWithOpponent->formatForCheatCheck(),
                    CheatConstant::LOG_DETAIL_KEY_PARTY_STATUSES => $afterCheatSessionPartyWithOpponent->formatForCheatCheck(),
                ];
            }
        );
    }
