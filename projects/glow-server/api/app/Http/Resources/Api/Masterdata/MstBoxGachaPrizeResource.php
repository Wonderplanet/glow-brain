<?php

namespace App\Http\Resources\Api\Masterdata;

use App\Http\Resources\Api\Masterdata\Concerns\InstanceByModelResource;
use App\Http\Resources\Api\Masterdata\Concerns\MstResourceAbstract;

// ----------------------------------------------------------------------
// This file is auto-generated by SchemaBuilder. Do not modify directly.
// ----------------------------------------------------------------------

class MstBoxGachaPrizeResource
{
    use MstResourceAbstract;
    use InstanceByModelResource;

    protected const C_id = 'id';
    protected const C_mstBoxGachaGroupId = 'mstBoxGachaGroupId';
    protected const C_isPickup = 'isPickup';
    protected const C_resourceType = 'resourceType';
    protected const C_resourceId = 'resourceId';
    protected const C_resourceAmount = 'resourceAmount';
    protected const C_stock = 'stock';

    /**
     * @param array<mixed>|null $data
     */
    public function __construct(?array $data = null)
    {
        if (!is_null($data)) {
            $this->id = $data['id'];
            $this->mstBoxGachaGroupId = $data['mstBoxGachaGroupId'];
            $this->isPickup = $data['isPickup'];
            $this->resourceType = $data['resourceType'];
            $this->resourceId = $data['resourceId'];
            $this->resourceAmount = $data['resourceAmount'];
            $this->stock = $data['stock'];
        }
    }

    /** @var array<string> */
    protected $cast = [
        'id' => 'string',
        'mstBoxGachaGroupId' => 'string',
        'isPickup' => 'string',
        'resourceType' => 'string',
        'resourceId' => 'string|null',
        'resourceAmount' => 'int',
        'stock' => 'int',
    ];

    /** @var array<string> */
    protected $snakeCased = [
        'id' => 'id',
        'mstBoxGachaGroupId' => 'mst_box_gacha_group_id',
        'isPickup' => 'is_pickup',
        'resourceType' => 'resource_type',
        'resourceId' => 'resource_id',
        'resourceAmount' => 'resource_amount',
        'stock' => 'stock',
    ];

    /** @var string */
    protected $id;

    /** @var string */
    protected $mstBoxGachaGroupId;

    /** @var string */
    protected $isPickup;

    /** @var string */
    protected $resourceType;

    /** @var string|null */
    protected $resourceId;

    /** @var int */
    protected $resourceAmount;

    /** @var int */
    protected $stock;

    /**
     * @return array<string>
     */
    protected function getValue(): array
    {
        return [
            'id' => $this->id,
            'mstBoxGachaGroupId' => $this->mstBoxGachaGroupId,
            'isPickup' => $this->isPickup,
            'resourceType' => $this->resourceType,
            'resourceId' => $this->resourceId,
            'resourceAmount' => $this->resourceAmount,
            'stock' => $this->stock,
        ];
    }
}
