...
class MissionAdventBattleChallengeCountCriterionTest extends TestCase
{
    use TestMissionTrait;

    private MissionCriterionType $targetCriterionType = MissionCriterionType::ADVENT_BATTLE_CHALLENGE_COUNT;
    ...
    public function test_missionUpdateHandleService_handleAllUpdateTriggeredMissions_進捗が更新されて未クリア()
    {
        ...
        $this->checkUsrMissionLimitedTerm($usrMissions['mst_mission_limited_term_1'], MissionStatus::UNCLEAR, 1, $now, null, null);
    }
    ...
    public function test_missionUpdateHandleService_handleAllUpdateTriggeredMissions_進捗が更新されてクリア()
    {
        ...
        $usrMissions = UsrMissionLimitedTerm::query()->where('usr_user_id', $usrUserId)->get()->keyBy->getMstMissionId();
        $this->assertCount(1, $usrMissions);
        $this->checkUsrMissionLimitedTerm($usrMissions['mst_mission_limited_term_1'], MissionStatus::CLEAR, 10, $now, $now, null);
    }
}
