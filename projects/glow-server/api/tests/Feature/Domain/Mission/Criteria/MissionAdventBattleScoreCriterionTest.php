...
class MissionAdventBattleScoreCriterionTest extends TestCase
{
    use TestMissionTrait;

    private MissionCriterionType $targetCriterionType = MissionCriterionType::ADVENT_BATTLE_SCORE;
    ...
    public function test_missionUpdateHandleService_handleAllUpdateTriggeredMissions_進捗が更新されてクリア()
    {
        ...
        $usrMissions = UsrMissionLimitedTerm::query()->where('usr_user_id', $usrUserId)->get()->keyBy->getMstMissionId();
        $this->assertCount(1, $usrMissions);
        $this->checkUsrMissionLimitedTerm($usrMissions['mst_mission_limited_term_1'], MissionStatus::CLEAR, 100, $now, $now, null);
    }

    public function test_missionUpdateHandleService_handleAllUpdateTriggeredMissions_ハイスコアが更新されないケース()
    {
        // Setup
        $now = $this->fixTime();
        $usrUserId = $this->createUsrUser()->getId();
