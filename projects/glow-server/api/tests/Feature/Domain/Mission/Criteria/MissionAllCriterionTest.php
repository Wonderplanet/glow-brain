...
class MissionAllCriterionTest extends TestCase
{
    ...
    public function test_stageStartUseCase_exec_トリガーと進捗集約値の確認()
    {
        ...
        // Exercise
        app(StageStartUseCase::class)->exec($currentUser, 'stage1', 2, false, 1, 0);
    ...
    public function test_adventBattleStartUseCase_exec_トリガーと進捗集約値の確認()
    {
        ...
        // Exercise
        app(AdventBattleStartUseCase::class)->exec($currentUser, 'adventbattle1', 2, false, [], 0);
    ...
    public function test_artworkGradeUpService_gradeUp_トリガーと進捗集約値の確認()
    {
        // Setup
        $usrUserId = $this->createUsrUser()->getUsrUserId();
        $this->fixTime();
        ...
        $mstArtwork1 = MstArtwork::factory()->create(['id' => 'artwork_grade_up_1']);
        $mstArtwork2 = MstArtwork::factory()->create(['id' => 'artwork_grade_up_2']);
        $mstItem = MstItem::factory()->create();

        // 原画1: グレード1→2→3
        $mstArtworkGradeUp1_2 = MstArtworkGradeUp::factory()->create([
            'mst_series_id' => $mstArtwork1->mst_series_id,
            'rarity' => $mstArtwork1->rarity,
            'grade_level' => 2,
        ]);
        MstArtworkGradeUpCost::factory()->create([
            'mst_artwork_grade_up_id' => $mstArtworkGradeUp1_2->id,
            'resource_id' => $mstItem->id,
            'resource_amount' => 5,
        ]);
        $mstArtworkGradeUp1_3 = MstArtworkGradeUp::factory()->create([
            'mst_series_id' => $mstArtwork1->mst_series_id,
            'rarity' => $mstArtwork1->rarity,
            'grade_level' => 3,
        ]);
        MstArtworkGradeUpCost::factory()->create([
            'mst_artwork_grade_up_id' => $mstArtworkGradeUp1_3->id,
            'resource_id' => $mstItem->id,
            'resource_amount' => 10,
        ]);

        // 原画2: グレード1→2
        $mstArtworkGradeUp2_2 = MstArtworkGradeUp::factory()->create([
            'mst_series_id' => $mstArtwork2->mst_series_id,
            'rarity' => $mstArtwork2->rarity,
            'grade_level' => 2,
        ]);
        MstArtworkGradeUpCost::factory()->create([
            'mst_artwork_grade_up_id' => $mstArtworkGradeUp2_2->id,
            'resource_id' => $mstItem->id,
            'resource_amount' => 5,
        ]);
        ...
        UsrArtwork::factory()->create([
            'usr_user_id' => $usrUserId,
            'mst_artwork_id' => $mstArtwork1->id,
            'grade_level' => 1,
        ]);
        UsrArtwork::factory()->create([
            'usr_user_id' => $usrUserId,
            'mst_artwork_id' => $mstArtwork2->id,
            'grade_level' => 1,
        ]);
        UsrItem::factory()->create([
            'usr_user_id' => $usrUserId,
        ...
        // Exercise
        app(ArtworkGradeUpService::class)->gradeUp($usrUserId, $mstArtwork1->id); // artwork_grade_up_1: 1→2
        app(ArtworkGradeUpService::class)->gradeUp($usrUserId, $mstArtwork1->id); // artwork_grade_up_1: 2→3
        app(ArtworkGradeUpService::class)->gradeUp($usrUserId, $mstArtwork2->id); // artwork_grade_up_2: 1→2

        // Verify
        // 特定原画のグレードレベル（MAX集約）
        $this->checkTriggerAndAggregatedProgress(MissionCriterionType::SPECIFIC_ARTWORK_GRADE_LEVEL, 'artwork_grade_up_1', 3);
        $this->checkTriggerAndAggregatedProgress(MissionCriterionType::SPECIFIC_ARTWORK_GRADE_LEVEL, 'artwork_grade_up_2', 2);
    }
}
