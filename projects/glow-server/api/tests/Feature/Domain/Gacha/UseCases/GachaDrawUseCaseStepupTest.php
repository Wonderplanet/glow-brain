...
class GachaDrawUseCaseStepupTest extends TestCase
{
    ...
    public function testExec_ステップアップガシャ_クライアントステップ番号不一致エラー()
    {
        // Setup
        $usrUser = $this->createUsrUser();
        $currentUser = new CurrentUser($usrUser->getId());
        ...
        // Exercise & Verify - clientStepNumber=3を送信するがサーバー側はstep=2なので不一致エラー
        $this->expectException(GameException::class);
        ...
        $this->useCase->exec(
            ...
            CurrencyConstants::PLATFORM_APPSTORE,
            CostType::DIAMOND,
            3 // clientStepNumber=3、サーバー側はstep=2なので不一致
        );
    }

    public function testExec_ステップアップガシャ_無限周回でステップ実行可能()
    {
        // Setup
        $usrUser = $this->createUsrUser();
        $currentUser = new CurrentUser($usrUser->getId());
        ...
        // maxLoopCount=nullのデータを作成
        $this->createStepupGachaDataWithUnlimitedLoop();
        ...
        $usrGacha = UsrGacha::query()
            ...
            ->first();
        $this->assertNotNull($usrGacha);
        $this->assertEquals(3, $usrGacha->current_step_number); // ステップ3に進む
        $this->assertEquals(50, $usrGacha->loop_count); // 周回数は変わらない（最終ステップではない）
    }

    public function testExec_ステップアップガシャ_GachaHistoryにstepupInfoが含まれる()
    {
        // Setup
        $usrUser = $this->createUsrUser();
        $currentUser = new CurrentUser($usrUser->getId());
        ...
        // Verify - GachaHistoryにstepupInfoが含まれることを確認
        $histories = $this->getFromRedis(CacheKeyUtil::getGachaHistoryKey($currentUser->getId()));
        $this->assertNotNull($histories);
        $this->assertCount(1, $histories);
        ...
        // stepupInfoの確認（stepNumber, loopCountが含まれる）
        $this->assertArrayHasKey('stepupInfo', $response);
        $this->assertNotNull($response['stepupInfo']);
        $this->assertEquals(2, $response['stepupInfo']['stepNumber']); // 実行時のステップ番号
        $this->assertEquals(1, $response['stepupInfo']['loopCount']); // 実行時の周回数

        // おまけ報酬がないのでstepRewardsはnull
        $this->assertNull($response['stepRewards']);
    }

    public function testExec_ステップアップガシャ_logGachaActionsにstep情報が正しく記録される()
    {
        // Setup
        $usrUser = $this->createUsrUser();
        $currentUser = new CurrentUser($usrUser->getId());
        ...
        // Verify - log_gachasにstep_numberとloop_countが記録されているか確認
        $logGacha = LogGacha::query()
            ->where('usr_user_id', $currentUser->getId())
            ->where('opr_gacha_id', 'opr_gacha_id')
            ->first();
        $this->assertNotNull($logGacha);
        $this->assertEquals(4, $logGacha->getStepNumber()); // 実行したステップ番号
    }
    ...
    /**
     * maxLoopCount=null（無限周回）のステップアップガシャのテストデータを作成
     */
    protected function createStepupGachaDataWithUnlimitedLoop(): void
    {
        $this->createBaseData();
        ...
        OprGacha::factory()->create([
            ...
            'gacha_type' => GachaType::STEPUP->value,
            'prize_group_id' => 'prize_group_id',
            'total_play_limit_count' => null, // 無限周回テストなので回数制限なし
        ]);
