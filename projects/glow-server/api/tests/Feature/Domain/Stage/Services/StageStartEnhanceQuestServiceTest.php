...
class StageStartEnhanceQuestServiceTest extends TestCase
{
    ...
    public static function params_test_start_ステージ開始処理が実行できる()
    {
        return [
            '通常挑戦' => [
                'isChallengeAd' => false,
    ...
    /**
     * @dataProvider params_test_start_ステージ開始処理が実行できる
     */
    public function test_start_ステージ開始処理が実行できる(
        ...
        $this->stageStartEnhanceQuestService->start(
            ...
            mstStage: $mstStage,
            mstQuest: $mstQuest,
            isChallengeAd: $isChallengeAd, // 通常挑戦
            lapCount: 1,
            now: $now,
        );
        $this->saveAll();
        ...
        if ($isChallengeAd) {
            ...
            $this->assertEquals(90, $usrUserParameter->getStamina());
        }
        // リセット直後に挑戦できたので、挑戦回数が増えている
        $usrStageEnhance->refresh();
        $this->assertEquals((int)(! $isChallengeAd), $usrStageEnhance->getResetChallengeCount());
        $this->assertEquals((int)($isChallengeAd), $usrStageEnhance->getResetAdChallengeCount());
