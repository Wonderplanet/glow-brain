...
class StageStartUseCaseTest extends TestCase
{
    ...
    public function test_exec_ステージ開始時指定したメソッドが呼ばれる(
        ...
        // Exercise
        $results = $this->useCase->exec($currentUser, $mstStageId, $partyNo, false, $lapCount, $randomSeed);
        $this->saveAll();
        ...
        $this->assertEquals($randomSeed, $usrStageSession->getRandomSeed());
        $this->assertEquals($randomSeed, $results->usrStageStatus->getRandomSeed());
    ...
    public function test_exec_イベントステージ開始時指定したメソッドが呼ばれる()
    {
        ...
        // Exercise
        $results = $this->useCase->exec($currentUser, $mstStageId, $partyNo, false, 1, $randomSeed);
        $this->saveAll();
    ...
    public function testExec_スタミナキャンペーンが適用される場合(
        ...
        // Exercise
        $result = $this->useCase->exec($currentUser, $mstStage->getId(), $partyNo, false, 1, 0);
        $this->saveAll();
    ...
    public function testExec_挑戦回数キャンペーンが適用される場合()
    {
        ...
        $this->useCase->exec($currentUser, $mstStage->getId(), $partyNo, false, 1, 0);
        $this->saveAll();
    ...
    public function test_exec_広告視聴時スタミナが消費されない(string $mstStageId): void
    {
        ...
        $results = $this->useCase->exec($currentUser, $mstStageId, $partyNo, true, 1, 0);
    ...
    /**
     * @return array<string, array{mstStageId: string}>
     */
