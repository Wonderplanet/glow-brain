...
class BoxGachaDrawUseCaseTest extends TestCase
{
    ...
    // Emblem報酬テスト
    // ========================================
    public function test_exec_Emblemの初回獲得が成功する(): void
    {
        // Setup
        $usrUser = $this->createUsrUser();
        $this->createUsrUserRelatedData($usrUser->getId());
        $currentUser = new CurrentUser($usrUser->getId());
        $mstBoxGachaId = 'box_gacha_emblem_new';
        $costItemId = 'cost_item_emblem_new';
        $mstEmblemId = 'test_emblem_001';
        $drawCount = 1;
        $platform = 1;

        $this->fixTime('2024-06-15 12:00:00');

        $this->createMstBoxGachaWithEmblemPrize($mstBoxGachaId, $costItemId, $mstEmblemId, 1);
        $this->createUsrItem($usrUser->getId(), $costItemId, 100);
        ...
        // Verify - UsrEmblem作成
        $usrEmblem = UsrEmblem::where('usr_user_id', $usrUser->getId())
            ->where('mst_emblem_id', $mstEmblemId)
            ->first();
        $this->assertNotNull($usrEmblem);
        $this->assertEquals(EncyclopediaCollectStatus::IS_NEW->value, $usrEmblem->is_new_encyclopedia);

        // Verify - Emblem報酬が含まれる
        $emblemReward = $result->boxGachaRewards->first(fn($r) => $r->getType() === 'Emblem');
        $this->assertNotNull($emblemReward);
        $this->assertEquals($mstEmblemId, $emblemReward->getResourceId());
        $this->assertEquals(1, $emblemReward->getAmount());
        $this->assertNull($emblemReward->getRewardConvertedReason());

        // Verify - usrEmblemsに含まれる
        $this->assertCount(1, $result->usrEmblems);
        $this->assertEquals($mstEmblemId, $result->usrEmblems->first()->mst_emblem_id);
    }

    public function test_exec_Emblemの重複獲得時にコインに変換される(): void
    {
        // Setup
        $usrUser = $this->createUsrUser();
        $this->createUsrUserRelatedData($usrUser->getId());
        $currentUser = new CurrentUser($usrUser->getId());
        $mstBoxGachaId = 'box_gacha_emblem_duplicate';
        $costItemId = 'cost_item_emblem_duplicate';
        $mstEmblemId = 'test_emblem_002';
        $drawCount = 1;
        $platform = 1;
        ...
        UsrEmblem::factory()->create([
            ...
            'is_new_encyclopedia' => EncyclopediaCollectStatus::IS_NOT_NEW->value,
        ]);

        $beforeCount = UsrEmblem::where('usr_user_id', $usrUser->getId())->count();
        $beforeCoin = UsrUserParameter::where('usr_user_id', $usrUser->getId())->first()->coin;

        // Exercise
        $result = $this->useCase->exec($currentUser, $mstBoxGachaId, $drawCount, 1, $platform);

        // Verify - UsrEmblemの数が増えない
        $afterCount = UsrEmblem::where('usr_user_id', $usrUser->getId())->count();
        $this->assertEquals($beforeCount, $afterCount);

        // Verify - Coin変換
        $coinReward = $result->boxGachaRewards->first(
            fn($r) => $r->getType() === 'Coin'
                && $r->getRewardConvertedReason() === RewardConvertedReason::DUPLICATED_EMBLEM
        );
        $this->assertNotNull($coinReward);
        $this->assertEquals(1000, $coinReward->getAmount());

        // Verify - Coin増加
        $usrUserParameter = UsrUserParameter::where('usr_user_id', $usrUser->getId())->first();
        $this->assertEquals($beforeCoin + 1000, $usrUserParameter->coin);

        // Verify - usrEmblemsは空（重複なので新規追加なし）
        $this->assertCount(0, $result->usrEmblems);
    }

    public function test_exec_Emblem初回獲得とコイン変換が混在する(): void
    {
        // Setup
        $usrUser = $this->createUsrUser();
        $this->createUsrUserRelatedData($usrUser->getId());
        $currentUser = new CurrentUser($usrUser->getId());
        $mstBoxGachaId = 'box_gacha_emblem_mixed';
        $costItemId = 'cost_item_emblem_mixed';
        $mstEmblemId = 'test_emblem_003';
        $drawCount = 1;
        $platform = 1;

        $this->fixTime('2024-06-15 12:00:00');

        // amount=3: 1個は初獲得、2個は重複でコイン変換
        $this->createMstBoxGachaWithEmblemPrize($mstBoxGachaId, $costItemId, $mstEmblemId, 3);
        $this->createUsrItem($usrUser->getId(), $costItemId, 100);
        ...
        // Verify - 報酬が2つ（Emblem×1、Coin×1）
        $this->assertCount(2, $result->boxGachaRewards);
        ...
        $this->assertNotNull($coinReward);
        $this->assertEquals(2000, $coinReward->getAmount());

        // Verify - Coin増加
        $usrUserParameter = UsrUserParameter::where('usr_user_id', $usrUser->getId())->first();
        $this->assertEquals($beforeCoin + 2000, $usrUserParameter->coin);

        // Verify - usrEmblemsに1つ
        $this->assertCount(1, $result->usrEmblems);
    }

    public function test_exec_複数のEmblemを同時に獲得できる(): void
    {
        // Setup
        $usrUser = $this->createUsrUser();
        $this->createUsrUserRelatedData($usrUser->getId());
        $currentUser = new CurrentUser($usrUser->getId());
        $mstBoxGachaId = 'box_gacha_emblem_multiple';
        $costItemId = 'cost_item_emblem_multiple';
        $mstEmblemId1 = 'test_emblem_004';
        $mstEmblemId2 = 'test_emblem_005';
        $drawCount = 2;
        $platform = 1;

        $this->fixTime('2024-06-15 12:00:00');

        $this->createMstBoxGachaWithMultipleEmblemPrizes($mstBoxGachaId, $costItemId, [$mstEmblemId1, $mstEmblemId2]);
        $this->createUsrItem($usrUser->getId(), $costItemId, 100);

        // Exercise
        $result = $this->useCase->exec($currentUser, $mstBoxGachaId, $drawCount, 1, $platform);

        // Verify - UsrEmblemが2つ作成
        $usrEmblems = UsrEmblem::where('usr_user_id', $usrUser->getId())
            ->whereIn('mst_emblem_id', [$mstEmblemId1, $mstEmblemId2])
            ->get();
        $this->assertCount(2, $usrEmblems);
        $usrEmblems->each(function ($emblem) {
            $this->assertEquals(EncyclopediaCollectStatus::IS_NEW->value, $emblem->is_new_encyclopedia);
        });

        // Verify - usrEmblemsに両方含まれる
        $this->assertCount(2, $result->usrEmblems);
        $emblemIds = $result->usrEmblems->pluck('mst_emblem_id')->toArray();
        $this->assertContains($mstEmblemId1, $emblemIds);
        $this->assertContains($mstEmblemId2, $emblemIds);

        // Verify - boxGachaRewardsに両方のEmblem報酬が含まれる
        $emblemRewards = $result->boxGachaRewards->filter(fn($r) => $r->getType() === 'Emblem');
        $this->assertCount(2, $emblemRewards);
    }
    ...
    /**
     * Emblem報酬用のBoxGachaマスターデータを作成
     *
     * @param string $mstBoxGachaId BOXガチャID
     * @param string $costItemId コストアイテムID
     * @param string $mstEmblemId エンブレムID
     * @param int $amount 報酬個数
     */
    private function createMstBoxGachaWithEmblemPrize(
        string $mstBoxGachaId,
        string $costItemId,
        string $mstEmblemId,
        int $amount = 1
        ...
        // Emblemマスター
        MstEmblem::factory()->create(['id' => $mstEmblemId]);
        ...
        // Emblem賞品
        MstBoxGachaPrize::factory()->create([
            'id' => $mstBoxGachaId . '_emblem_prize',
            'mst_box_gacha_group_id' => $groupId,
            'is_pickup' => false,
            'resource_type' => 'Emblem',
            'resource_id' => $mstEmblemId,
    ...
    /**
     * 複数のEmblem報酬用のBoxGachaマスターデータを作成
     *
     * @param string $mstBoxGachaId BOXガチャID
     * @param string $costItemId コストアイテムID
     * @param array $mstEmblemIds エンブレムIDの配列
     */
    private function createMstBoxGachaWithMultipleEmblemPrizes(
        string $mstBoxGachaId,
        string $costItemId,
        array $mstEmblemIds
    ): void {
        ...
        // 複数のEmblem賞品（各在庫1で確実に全て引かれるようにする）
        foreach ($mstEmblemIds as $index => $mstEmblemId) {
            MstBoxGachaPrize::factory()->create([
                'id' => $mstBoxGachaId . '_emblem_prize_' . ($index + 1),
                'mst_box_gacha_group_id' => $groupId,
                'is_pickup' => false,
                'resource_type' => 'Emblem',
                'resource_id' => $mstEmblemId,
