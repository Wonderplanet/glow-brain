...
class GameControllerTest extends BaseControllerTestCase
{
    ...
    public function testUpdateAndFetch_結合テスト()
    {
        ...
        // Exercise
        $response = $this->withHeaders(['Language' => Language::Ja->value, 'Access-Token' => $accessToken])->sendRequest('update_and_fetch');
    ...
    public function testFetch_リクエストを送ると200OKが返り想定通りのレスポンスが返ることを確認する()
    {
        ...
        $gameFetchData = new GameFetchData(
            ...
            collect(),
            $usrAdventBattles,
            new GameBadgeData(123, 456, 225, collect(['mstEventId' => 3, 'mstEventId2' => 1]), 999),
            $usrUserBuyCount,
    ...
    public function testBadge_リクエストを送ると200OKが返り想定通りのレスポンスが返ることを確認する()
    {
        ...
        $gameBadgeData = new GameBadgeData(123, 456, 225, collect(['mstEventId' => 3]), 999);
