...
class GachaControllerTest extends BaseControllerTestCase
{
    ...
    public function testDrawFree_ステップアップガシャ_無料ステップで200OKが返ることを確認する()
    {
        // Setup
        $oprGachaId = 'stepup_gacha_free';
        $prizeGroupId = 'prize_group_stepup_free';
        $fragmentMstItemId = 'fragment_stepup_free';

        $usrUser = $this->createUsrUser();
        $this->createDiamond($usrUser->getId(), 0);
        UsrUserParameter::factory()->create([
            'usr_user_id' => $usrUser->getId(),
        ]);
        ...
        $response = $this->sendRequest('draw/free', $params);
    ...
    public function testDrawFree_ステップアップガシャ_初回無料ステップで200OKが返ることを確認する()
    {
        // Setup
        $oprGachaId = 'stepup_gacha_first_free';
        $prizeGroupId = 'prize_group_stepup_ff';
        $fragmentMstItemId = 'fragment_stepup_ff';
    ...
    public function test_prize_ステップアップガシャの排出率にstepupGachaPrizesが含まれることを確認する()
    {
        // Setup
        $oprGachaId = 'stepup_gacha_prize_1';
        $prizeGroupId = 'prize_group_prize_1';
        $fixedPrizeGroupId = 'fixed_prize_group_prize_1';
        $stepFixedPrizeGroupId = 'step_fixed_prize_group_1';

        $this->createUsrUser();

        MstUnit::factory()->createMany([
            ['id' => 'unit_r_prize', 'rarity' => RarityType::R->value],
            ['id' => 'unit_sr_prize', 'rarity' => RarityType::SR->value],
            ['id' => 'unit_ssr_prize', 'rarity' => RarityType::SSR->value],
        ]);
        ...
        OprGachaPrize::factory()->createMany([
            ['group_id' => $prizeGroupId, 'resource_type' => RewardType::UNIT, 'resource_id' => 'unit_r_prize', 'weight' => 700],
            ['group_id' => $prizeGroupId, 'resource_type' => RewardType::UNIT, 'resource_id' => 'unit_sr_prize', 'weight' => 250],
            ['group_id' => $prizeGroupId, 'resource_type' => RewardType::UNIT, 'resource_id' => 'unit_ssr_prize', 'weight' => 50],
        ]);

        // 確定枠景品（親ガシャ）
        OprGachaPrize::factory()->createMany([
            ['group_id' => $fixedPrizeGroupId, 'resource_type' => RewardType::UNIT, 'resource_id' => 'unit_sr_prize', 'weight' => 800],
            ['group_id' => $fixedPrizeGroupId, 'resource_type' => RewardType::UNIT, 'resource_id' => 'unit_ssr_prize', 'weight' => 200],
        ]);

        // 確定枠景品（ステップ2固有: SSRのみ）
        OprGachaPrize::factory()->create([
            'group_id' => $stepFixedPrizeGroupId,
            'resource_type' => RewardType::UNIT,
            'resource_id' => 'unit_ssr_prize',
            'weight' => 1000,
        ]);
        ...
        OprStepupGachaStep::factory()->createMany([
            // ステップ1: 確定枠なし
            ['opr_gacha_id' => $oprGachaId, 'step_number' => 1, 'draw_count' => 10, 'fixed_prize_count' => 0, 'prize_group_id' => null, 'fixed_prize_group_id' => null, 'cost_type' => CostType::DIAMOND->value, 'cost_num' => 1500],
            // ステップ2: 確定枠1（ステップ固有の確定枠グループを使用）
            ['opr_gacha_id' => $oprGachaId, 'step_number' => 2, 'draw_count' => 10, 'fixed_prize_count' => 1, 'prize_group_id' => null, 'fixed_prize_group_id' => $stepFixedPrizeGroupId, 'cost_type' => CostType::DIAMOND->value, 'cost_num' => 1500],
        ]);

        // Exercise
        $response = $this->sendGetRequest('prize', ['oprGachaId' => $oprGachaId]);
        ...
        // ステップ数の確認
        $this->assertCount(2, $response['stepupGachaPrizes']);

        // ステップ1: 確定枠なし（通常枠は親ガシャのフォールバック）
        $step1 = $response['stepupGachaPrizes'][0];
        $this->assertEquals(1, $step1['stepNumber']);
        $this->assertEquals(10, $step1['drawCount']);
        $this->assertEquals(0, $step1['fixedProbabilities']['fixedCount']);

        // 通常枠のレアリティ確率: R=700/1000=70.0%, SR=250/1000=25.0%, SSR=50/1000=5.0%
        $step1Rarities = collect($step1['rarityProbabilities']);
        ...
        $this->assertNotEmpty($step1['probabilityGroups']);
        $this->assertEmpty($step1['fixedProbabilities']['rarityProbabilities']);
        $this->assertEmpty($step1['fixedProbabilities']['probabilityGroups']);

        // ステップ2: ステップ固有の確定枠グループ（SSRのみ）
        $step2 = $response['stepupGachaPrizes'][1];
        $this->assertEquals(2, $step2['stepNumber']);
        $this->assertEquals(10, $step2['drawCount']);
        $this->assertEquals(1, $step2['fixedProbabilities']['fixedCount']);

        // 通常枠のレアリティ確率（ステップ1と同じ）
        $step2Rarities = collect($step2['rarityProbabilities']);
        $this->assertEquals(70.0, $step2Rarities->firstWhere('rarity', RarityType::R->value)['probability']);
        $this->assertEquals(25.0, $step2Rarities->firstWhere('rarity', RarityType::SR->value)['probability']);
        $this->assertEquals(5.0, $step2Rarities->firstWhere('rarity', RarityType::SSR->value)['probability']);
        $this->assertNotEmpty($step2['probabilityGroups']);

        // 確定枠: ステップ固有グループ（SSR=1000/1000=100.0%）
        $step2FixedRarities = collect($step2['fixedProbabilities']['rarityProbabilities']);
        $this->assertCount(1, $step2FixedRarities);
        ...
        $step2FixedGroups = collect($step2['fixedProbabilities']['probabilityGroups']);
        $this->assertCount(1, $step2FixedGroups);
        $this->assertEquals(RarityType::SSR->value, $step2FixedGroups[0]['rarity']);
        $this->assertCount(1, $step2FixedGroups[0]['prizes']);
        $this->assertEquals('unit_ssr_prize', $step2FixedGroups[0]['prizes'][0]['resourceId']);
        $this->assertEquals(100.0, $step2FixedGroups[0]['prizes'][0]['probability']);
    }

    public function test_prize_通常ガシャの場合stepupGachaPrizesがnullで返ることを確認する()
    {
        // Setup
        $oprGachaId = 'normal_gacha_prize_1';
        $prizeGroupId = 'prize_group_normal_1';

        $this->createUsrUser();

        MstUnit::factory()->create([
            'id' => 'unit_r_normal_prize',
        ...
        OprGachaPrize::factory()->create([
            'group_id' => $prizeGroupId,
            'resource_type' => RewardType::UNIT,
            'resource_id' => 'unit_r_normal_prize',
            'weight' => 1000,
        ]);
        ...
        $response->assertStatus(HttpStatusCode::SUCCESS);
        $json = $response->json();
        $this->assertArrayHasKey('stepupGachaPrizes', $json);
        $this->assertNull($json['stepupGachaPrizes']);
        $this->assertNotEmpty($json['rarityProbabilities']);
        $this->assertNotEmpty($json['probabilityGroups']);
    }
}
