...
return new class extends Migration
{
    ...
    {
        // 既存テーブルは削除
        Schema::dropIfExists('usr_mission_limited_terms');

        // 新規設定でテーブル作成
        Schema::create('usr_mission_limited_terms', function (Blueprint $table) {
            ...
            $table->primary(['usr_user_id', 'mst_mission_limited_term_id']);
        });
    }
    ...
    public function down(): void
    {
        ...
        Schema::create('usr_mission_limited_terms', function (Blueprint $table) {
            ...
            $table->string('usr_user_id', 255)->comment('usr_users.id');
            $table->string('mst_mission_limited_term_id', 255)->comment('mst_mission_limited_terms.id');
            $table->tinyInteger('status')->default(0)->comment('0: 未クリア 1: クリア 2: 報酬受取済');
            $table->timestampTz('cleared_at')->nullable()->comment('達成日時');
            $table->timestampTz('received_reward_at')->nullable()->comment('報酬受取日時');
            $table->timestampsTz();
