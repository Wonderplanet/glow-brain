...
# GCP認証情報を環境変数で指定（gcloud storageコマンドで使用される）
...
step2_download_data() {
    ...
    for table in "${CHANGED_TABLES[@]}"; do
        ...
        # gcloud storageは環境変数CLOUDSDK_CORE_ACCOUNTで指定されたアカウントを使用
        if gcloud storage cp "$gcs_path" "$output_file" 2>/dev/null; then
