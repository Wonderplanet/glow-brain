---
description: APIエンドポイントのコードを解析し、詳細なシーケンス図（Mermaid形式）を自動生成してdocsに保存。
argument-hint: "[エンドポイントURL] [サフィックス]"
---
...
```
/api:generate-sequence-diagram [エンドポイントURL] [オプション：ファイル名サフィックス]
```
...
### 基本的な使用方法
```
/api:generate-sequence-diagram /api/users/login
```
→ `docs/api-sequence-diagrams/api_users_login.md` が生成されます

### 条件分岐や特定機能の詳細を記述する場合
```
/api:generate-sequence-diagram /api/stage/start event_quest
```
→ `docs/api-sequence-diagrams/api_stage_start_event_quest.md` が生成されます

```
/api:generate-sequence-diagram /api/gacha/draw normal_gacha
```
→ `docs/api-sequence-diagrams/api_gacha_draw_normal_gacha.md` が生成されます

```
/api:generate-sequence-diagram /api/gacha/draw special_bonus_flow
```
...
### シンプルな例（ユーザー詳細取得API）
```
/api:generate-sequence-diagram /api/users/show
```
...
### 複雑な例（ガチャ実行API - 通常フロー）
```
/api:generate-sequence-diagram /api/gacha/draw normal_flow
```
...
### 複雑な例（ガチャ実行API - 天井到達時）
```
/api:generate-sequence-diagram /api/gacha/draw ceiling_reached
```
