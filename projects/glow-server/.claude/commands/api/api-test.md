---
description: PHPUnitテストを実行し、Error/Failure/Warningが出た場合は修正して全テストを正常に通す。
argument-hint: "[フィルタ条件]"
---
...
```
/api:api-test UserTest|LoginTest
```
...
# 特定のテストクラスのみ実行
/api:api-test UserTest

# 複数のテストを実行
/api:api-test UserTest|LoginTest|RegisterTest

# 特定のテストメソッドを実行
/api:api-test testUserLogin|testUserRegister
```