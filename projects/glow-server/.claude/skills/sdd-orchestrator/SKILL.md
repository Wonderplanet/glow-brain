...
description: |
  PDF仕様書からAPI実装設計書を自動生成する8段階のSpec-Driven Development (SDD)オーケストレーター。要件抽出、コードベース調査、レビュー、プランナー確認、API設計を25～30分で実行。8段階：(1) PDF仕様書からサーバー要件抽出、(2) コード調査による追加要件洗い出し、(3) サーバー仕様レビュー、(4) プランナー確認統合（人間確認必須）、(5) サーバーAPI要件書確定、(6) API全体概要設計（並列実行）、(7) 個別API詳細設計（並列実行）、(8) 実装設計（並列実行）。以下の場合に使用：(1) PDF仕様書から実装設計までの包括的設計、(2) 新機能設計（ガチャ、クエスト、スタミナブースト等）、(3) 大規模機能のリデザイン、(4) ユーザーが「SDD設計フロー」「仕様書から設計書」「PDF仕様書から実装設計」またはエンドツーエンド設計自動化に言及した時。
---
