---
name: pr-unresolved-comments
description: |
  GraphQL APIでコメントスレッド全体を取得し、会話コンテキストに基づいてローカルで修正を実装するGitHubプルリクエスト未解決レビューコメントハンドラー。完全なスレッド対話(最初のコメント → 返信 → 追加コメント)付きの未解決コメントを取得し、会話コンテキストを分析、適切なコード変更を適用。以下の場合に使用: (1) PRレビューフィードバックへの対応と指摘された問題の修正、(2) コードレビュー提案への対応、(3) レビュアーコメントに基づく実装の修正、(4) PRレビューからのバグ修正リクエストの処理、(5) レビューフィードバックに基づくドキュメント更新、(6) 複数の返信を含む複雑なコメントスレッドの解決。ワークフロー: 未解決コメント取得 → スレッドコンテキスト表示 → コンテキストを考慮した修正実装 → サマリー表示 → コミット提案。 (project)
---
