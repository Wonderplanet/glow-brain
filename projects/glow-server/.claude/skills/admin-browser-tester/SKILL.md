---
name: Testing Admin Browser Functionality
description: admin実装や改修後にブラウザで実際に動作確認を行う際に使用。chrome-devtools MCPを使用してブラウザ操作を自動化し、ページ遷移・フォーム操作・表示確認などを実施してテスト結果をレポートする。新規Filamentリソース追加、CRUD機能実装、バリデーション変更、表示項目変更など、あらゆるadmin実装後の動作確認をカバーする。
---
