...
description: |
  新規API実装や既存コード調査時にglow-server APIアーキテクチャを理解するための包括的ガイド。以下の場合に使用: (1) レイヤードアーキテクチャ(Controller/UseCase/Service/Repository/Model)の責務と制約の理解、(2) ドメイン間連携のためのDelegatorパターンの学習、(3) 依存関係ルールと例外(Mst/Game/Common)の確認、(4) データフロー(リクエスト処理、UsrModelManagerキャッシュ、レスポンス生成)の理解、(5) テスト戦略のレビュー、(6) モジュラーモノリス設計の調査。アーキテクチャ確認、レイヤー構造理解、依存関係ルール検証、Delegatorパターン確認、設計原則調査、モジュラーモノリス実装学習、ドメイン間連携理解が必要な時にトリガーされる。
---
