...
description: |
  api/admin環境の変更時にcodebuildディレクトリ内のECS設定ファイルを自動同期。taskdefinitions.json、buildspec.yml、イメージ定義ファイルの同期を適切な検証とともに処理。以下の場合に使用: (1) 環境変数のECS設定への同期(environmentとsecretsの配置判断)、(2) Dockerfileの変更をビルド引数に反映、(3) Task Definition設定の更新(CPU/メモリリソース)、(4) ECSコンテナ設定の調整、(5) 新しいビルドパラメータでのBuildSpec更新。JSON構文検証、プレースホルダー形式の維持、必須フィールドの確認を実施。 (project)
---
