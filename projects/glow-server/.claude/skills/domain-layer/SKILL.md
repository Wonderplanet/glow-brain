---
name: "Implementing Domain Layer"
description: 新規API実装時にDomainレイヤーのコードを実装する際に使用。クリーンアーキテクチャに基づいたドメイン分類（通常ドメイン、Game、Resource、Common）、各サブフォルダ（Entities、Models、Repositories、Services、UseCases、Delegators等）の役割、Entity種別の制約、Delegatorのreturn型ルールを理解し、既存パターンに従った正しいドメイン実装を行う。
---
