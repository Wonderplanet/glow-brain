---
name: bne-external-payment-webhook
description: |
  Xsolla WebStoreウェブフックの受信・処理エンドポイント実装。SHA-1署名検証によるセキュリティ確保、ユーザー情報取得、決済事前確認、カスタムパラメータ処理、エラーハンドリングを実施。以下の場合に使用: (1) Xsollaウェブフック受信エンドポイント、(2) SHA-1署名検証（リクエスト正当性確認）、(3) web_store_user_validationウェブフック（ユーザー情報取得）、(4) web_store_payment_validationウェブフック（決済事前確認）、(5) カスタムパラメータ処理（store_code、user_ip）、(6) Xsollaレスポンス形式のエラーハンドリング、(7) 注文支払い成功通知（将来実装）
---
