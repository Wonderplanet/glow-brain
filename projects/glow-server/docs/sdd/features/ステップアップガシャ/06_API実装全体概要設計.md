...
## 更新履歴
- 2025-12-16: 初版作成
- 2026-01-07: おまけ報酬機能の追加に伴う更新
  - 要件 REQ-MD-3（おまけ報酬設定）の対応関係を追加
  - 要件 REQ-GE-5（おまけ報酬付与）の対応関係を追加
  - レスポンス構造の説明を更新（gachaResults と stepRewards の区別）
  - 実装優先順位にマスタテーブル追加を反映
...
|-------------|------|-----------|------|
| POST /api/gacha/draw/diamond | 既存改修 | REQ-GE-1, REQ-GE-2, REQ-GE-3, REQ-GE-4, REQ-GE-5, REQ-VL-1, REQ-VL-2, REQ-VL-3 | ダイヤモンドガシャ実行（ステップアップ対応、おまけ報酬付与含む） |
| POST /api/gacha/draw/paid_diamond | 既存改修 | REQ-GE-1, REQ-GE-2, REQ-GE-3, REQ-GE-4, REQ-GE-5, REQ-VL-1, REQ-VL-2, REQ-VL-3 | 有償ダイヤモンドガシャ実行（ステップアップ対応、おまけ報酬付与含む） |
| POST /api/gacha/draw/item | 既存改修 | REQ-GE-1, REQ-GE-2, REQ-GE-3, REQ-GE-4, REQ-GE-5, REQ-VL-1, REQ-VL-2, REQ-VL-3 | アイテムガシャ実行（ステップアップ対応、おまけ報酬付与含む） |
| POST /api/gacha/draw/free | 既存改修 | REQ-GE-1, REQ-GE-2, REQ-GE-3, REQ-GE-4, REQ-GE-5, REQ-VL-1, REQ-VL-2, REQ-VL-3 | 無料ガシャ実行（ステップアップ対応、おまけ報酬付与含む） |
...
- **関連要件:** REQ-GE-1, REQ-GE-2, REQ-GE-3, REQ-GE-4, REQ-GE-5, REQ-VL-1, REQ-VL-2, REQ-VL-3
...
- **改修内容:**
  ...
  - ステップアップガシャの場合の処理フロー追加:
    ...
    - 周回数上限チェック
    - レアリティ条件付き確定枠抽選ロジック
    - おまけ報酬付与処理（`opr_stepup_gacha_step_rewards` から取得、RewardDelegator使用）
    - ステップ進行処理（`usr_gachas` の `current_step_number`, `loop_count` を更新）
  - レスポンス `usrGacha` にステップアップガシャ情報を追加
  - レスポンスでガシャ抽選結果とおまけ報酬を区別（`gachaResults` と `stepRewards`）
...
### 3.3 要件 REQ-MD-3: ステップごとのおまけ報酬管理
- **実現に必要なAPI:** なし（マスタデータ管理のため、API不要）
- **実装概要:** マスタデータテーブル（`opr_stepup_gacha_step_rewards`）で管理
- **備考:** 管理画面側で登録・編集、既存のrewards系テーブルと同じ構造（resource_type, resource_id, resource_amount）

### 3.4 要件 REQ-MD-4: ガシャラインナップとレアリティ情報の管理
- **実現に必要なAPI:** なし（マスタデータ管理のため、API不要）
- **実装概要:** 既存のマスタデータテーブル（`opr_gacha_prizes`）で管理
- **備考:** 既存のガシャ賞品管理と同じ

### 3.5 要件 REQ-UD-1: ステップ進行状況の管理
- **実現に必要なAPI:**
...
- **実装概要:**
  ...
  - ステップ進行状況取得 → コスト検証 → 抽選 → 報酬配布 → おまけ報酬付与 → ステップ進行
- **備考:** 既存のガシャ実行フローを拡張

### 3.8 要件 REQ-GE-2: ステップ進行処理
- **実現に必要なAPI:**
  - POST /api/gacha/draw/diamond（既存改修）
  - POST /api/gacha/draw/paid_diamond（既存改修）
  - POST /api/gacha/draw/item（既存改修）
...
- **実現に必要なAPI:**
  ...
  - POST /api/gacha/draw/free（既存改修）
- **実装概要:**
  - `opr_stepup_gacha_step_rewards` テーブルから現在ステップ・周回数に対応するおまけ報酬を取得
  - RewardDelegatorを使用してガシャ抽選結果とは別におまけ報酬を付与
  - おまけ報酬は複数設定可能（例：アイテム×10 + コイン×1000）
  - `loop_count_target` がNULLの場合は全周回で付与、値が指定されている場合はその周回数でのみ付与
- **備考:** 既存のRewardDelegatorシステムを活用、ガシャ抽選結果とおまけ報酬をレスポンスで区別

### 3.11 要件 REQ-RS-1: ガシャ実行レスポンスの拡張
- **実現に必要なAPI:**
  - POST /api/gacha/draw/diamond（既存改修）
  - POST /api/gacha/draw/paid_diamond（既存改修）
  - POST /api/gacha/draw/item（既存改修）
...
2. マスタデータテーブルの作成
   ...
   - `opr_stepup_gacha_step_rewards` テーブル（おまけ報酬管理）
3. 既存テーブルへのカラム追加
...
- 周回数が正しくカウントされる
- 確定枠抽選が正しく動作する
- おまけ報酬が正しく付与される（周回数に応じた報酬切り替えも含む）
- レスポンスにステップアップガシャ情報が含まれる
- レスポンスでガシャ抽選結果とおまけ報酬が区別されている
...
- **影響範囲:** ガシャ実行API全般
- **注意点:**
  - ステップ進行処理とおまけ報酬付与も含めて、全てを同一トランザクション内で実行
  - ステップ進行やおまけ報酬付与に失敗した場合、ガシャ実行全体をロールバック
  - 既存のトランザクション管理パターンを踏襲
  - おまけ報酬の付与はRewardDelegatorを使用（既存の報酬付与パターンと統一）
