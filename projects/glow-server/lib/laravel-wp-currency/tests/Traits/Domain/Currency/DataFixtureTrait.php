...
trait DataFixtureTrait
{
     ...
     * @param integer $paidAmouontGoogle
     * @param integer $freeAmount
     * @return void
     */
    private function createUsrCurrencySummary(string $userId, int $paidAmountApple, int $paidAmouontGoogle, int $freeAmount)
    {
        $model = new UsrCurrencySummary();

        $model->usr_user_id = $userId;
        $model->paid_amount_apple = $paidAmountApple;
        $model->paid_amount_google = $paidAmouontGoogle;
