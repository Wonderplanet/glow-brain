...
class AdmBulkCurrencyRevertTaskTargetPaidLogRepositoryTest extends TestCase
{
    private AdmBulkCurrencyRevertTaskTargetPaidLogRepository $admBulkCurrencyRevertTaskTargetPaidLogRepository;

    protected function setUp(): void
    {
        parent::setUp();
        $this->admBulkCurrencyRevertTaskTargetPaidLogRepository =
            $this->app->make(AdmBulkCurrencyRevertTaskTargetPaidLogRepository::class);
    }
    ...
    public function bulkInsert_一括登録()
    {
        ...
        // Exercise
        $ret = $this->admBulkCurrencyRevertTaskTargetPaidLogRepository->bulkInsert($data);

        // Verify
        $this->assertTrue($ret);

        $actual = AdmBulkCurrencyRevertTaskTargetPaidLog::all()->sortBy('adm_bulk_currency_revert_task_target_id');
        $this->assertCount(3, $actual);
        $this->assertEquals(1, $actual[0]->adm_bulk_currency_revert_task_target_id);
        $this->assertEquals('user-1', $actual[0]->usr_user_id);
        $this->assertEquals(1, $actual[0]->log_currency_paid_id);
        ...
        $this->assertEquals(2, $actual[1]->log_currency_paid_id);
        ...
        $this->assertEquals(3, $actual[2]->log_currency_paid_id);
    ...
    public function makeRecordsFromTargetData_入力データからレコードデータを作成()
    {
        ...
        // Exercise
        $actual = $this->admBulkCurrencyRevertTaskTargetPaidLogRepository->makeRecordsFromTargetData(
        ...
        // Verify
        $this->assertCount(3, $actual);
        $this->assertEquals($bulkCurrencyRevertTaskTargetId, $actual[0]['adm_bulk_currency_revert_task_target_id']);
        $this->assertEquals('user-1', $actual[0]['usr_user_id']);
        $this->assertEquals(1, $actual[0]['log_currency_paid_id']);
        $this->assertEquals($now->format('Y-m-d H:i:s'), $actual[0]['created_at']->format('Y-m-d H:i:s'));
        $this->assertEquals($now->format('Y-m-d H:i:s'), $actual[0]['updated_at']->format('Y-m-d H:i:s'));

        $this->assertEquals($bulkCurrencyRevertTaskTargetId, $actual[1]['adm_bulk_currency_revert_task_target_id']);
        $this->assertEquals('user-1', $actual[1]['usr_user_id']);
        $this->assertEquals(2, $actual[1]['log_currency_paid_id']);
        $this->assertEquals($now->format('Y-m-d H:i:s'), $actual[1]['created_at']->format('Y-m-d H:i:s'));
        $this->assertEquals($now->format('Y-m-d H:i:s'), $actual[1]['updated_at']->format('Y-m-d H:i:s'));

        $this->assertEquals($bulkCurrencyRevertTaskTargetId, $actual[2]['adm_bulk_currency_revert_task_target_id']);
