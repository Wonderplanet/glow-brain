...
trait PlatformPaidTrait
{
    ...
    abstract public function getPaidAmountShare(): int;
    abstract public function getFreeAmount(): int;

    /**
     * プラットフォーム別の有償一次通貨を取得する
     *
     * WebStoreで購入した有償通貨（paid_amount_share）は、
     * iOS/Androidどちらのプラットフォームからでも消費可能なため、
     * AppStoreとGooglePlayの両方に加算される
     *
    ...
    public function getPlatformPaidAmount(string $billingPlatform): int
    {
        switch ($billingPlatform) {
            case CurrencyConstants::PLATFORM_APPSTORE:
                return $this->getPaidAmountApple() + $this->getPaidAmountShare();
            case CurrencyConstants::PLATFORM_GOOGLEPLAY:
                return $this->getPaidAmountGoogle() + $this->getPaidAmountShare();
            default:
                return 0;
        }
    }
