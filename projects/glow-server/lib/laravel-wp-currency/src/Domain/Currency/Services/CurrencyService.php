...
class CurrencyService
{
    ...
    public function usePaid(
        ...
        //  getPlatformPaidAmount()はWebStore購入の通貨(paid_amount_share)も含んでチェックする
        ...
        // 優先順位1: プラットフォーム固有の有償通貨
        $afterAmount = $this->usePaidInternal($userId, $osPlatform, $billingPlatform, $amount, $trigger);

        if ($afterAmount > 0) {
            // 優先順位2: WebStore購入の有償通貨（iOS/Androidどちらからでも消費可能）
            $afterAmount = $this->usePaidInternal(
                $userId,
                $osPlatform,
        ...
        // summaryを更新する（WebStoreとプラットフォーム固有の両方）
        $this->refreshPaidCurrncySummary($userId, CurrencyConstants::PLATFORM_WEBSTORE);
     ...
     * - WebStoreで購入した有償一次通貨（paid_amount_share）を次に消費
     * - プラットフォーム固有の有償一次通貨は最後に消費され、古いものから消費される
     *
     * 詳細は次のURLを参照してください。
     * https://wonderplanet.atlassian.net/wiki/spaces/wonderplanet/pages/106135583
     *
    ...
    public function useCurrency(
        ...
        $beforeAmount =
            ...
            $beforeSumamry->paid_amount_share +
