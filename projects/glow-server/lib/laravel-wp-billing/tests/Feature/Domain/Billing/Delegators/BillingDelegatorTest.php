...
class BillingDelegatorTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function allowedToPurchase_許可情報を登録する()
    {
        ...
        // Exercise
        $this->billingDelegator->allowedToPurchase(
            '1',
    ...
    public function getStoreAllowance_許可情報を取得する()
    {
        ...
        // Exercise
        $result = $this->billingDelegator->getStoreAllowance('1', CurrencyConstants::PLATFORM_APPSTORE, 'store_product1');

        // Verify
        $this->assertEquals('1', $result->usr_user_id);
        $this->assertEquals('store_product1', $result->product_id);
        $this->assertEquals('mst_product1', $result->mst_store_product_id);
        $this->assertEquals('product1', $result->product_sub_id);
        $this->assertEquals(CurrencyConstants::OS_PLATFORM_IOS, $result->os_platform);
        $this->assertEquals(CurrencyConstants::PLATFORM_APPSTORE, $result->billing_platform);
        $this->assertEquals('device1', $result->device_id);
    }

    #[Test]
    public function getOrCreateStoreAllowance_許可情報が存在しない場合でも自動登録されて取得できる()
    {
        // Setup
        // 通貨管理情報を登録
        $this->currencyService->registerCurrencySummary('1', CurrencyConstants::OS_PLATFORM_IOS, 0);
        ...
        // Exercise
        $result = $this->billingDelegator->getOrCreateStoreAllowance(
            '1',
            CurrencyConstants::PLATFORM_APPSTORE,
            'store_product1',
    ...
    public function purchased_購入する()
    {
        ...
        $allowance = $this->billingDelegator->getStoreAllowance('1', CurrencyConstants::PLATFORM_APPSTORE, 'store_product_id');
    ...
    public function getStoreInfo_ショップ情報を取得()
    {
        ...
        // Exercise
        $usrStoreInfo = $this->billingDelegator->getStoreInfo('1');
    ...
    {
        // Exercise
        $usrStoreInfo = $this->billingDelegator->setStoreInfo('1', 20, '2020-01-01 00:00:00');
    ...
    public function hasStoreProductHistory_ショップ履歴の有無を確認()
    {
        ...
        // Exercise
        $result = $this->billingDelegator->hasStoreProductHistory('1');
