...
class SpreadSheetCsvEntity
{
    ...
    /**
     * 処理済みデータのデータ行のみを既存データに結合する
     * processedDataの最初の行はヘッダー（ENABLE行）のためスキップし、データ行のみマージする
     *
     * @param array $processedData setDataまたはcreateI18nIncludingSheetsで処理済みのデータ
     * @return void
     */
    public function mergeProcessedDataRows(array $processedData): void
    {
        $dataRows = array_slice($processedData, 1);
        $this->data = array_merge($this->data, $dataRows);
    }
