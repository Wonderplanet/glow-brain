...
class SpreadSheetFetchService
{
    ...
    public function fetchValues(int $releaseKey, array $target = [], array $sheets = []): array
    {
        ...
        // 分割シートのベースシート（_なし）が先に処理されるようにソート
        // スプシ上のタブ順序が MstItem_2, MstItem, MstItem_3 のように
        // ベースシートより分割シートが先に来る場合のマージ漏れを防ぐ
        $baseSheets = [];
        $splitSheets = [];
        foreach ($sheets as $sheet) {
            if (strrpos($sheet->getTitle(), '_') === false) {
                $baseSheets[] = $sheet;
            } else {
                $splitSheets[] = $sheet;
            }
        }
        $sheets = array_merge($baseSheets, $splitSheets);
        ...
        foreach ($sheets as $sheet) {
            ...
            if ($pos === false) {
                ...
                // 分割シートもベースシートと同じ処理（setData）でデータを加工
                $sheet->setData($value, $releaseKey);
                if ($sheet->existI18nConvert()) {
                    // i18nデータを含む分割シートの場合、i18n分離してから各エンティティにマージ
                    $splitEntities = $sheet->createI18nIncludingSheets();
                    foreach ($splitEntities as $splitEntity) {
                        $splitTitle = $splitEntity->getTitle();
                        if (isset($spreadSheets[$splitTitle])) {
                            $spreadSheets[$splitTitle]->mergeProcessedDataRows($splitEntity->getData());
                        }
                    }
                } else {
                    // i18nなしの分割シート
                    $spreadSheets[$title]->mergeProcessedDataRows($sheet->getData());
                }
            }
        }
        return $spreadSheets;
