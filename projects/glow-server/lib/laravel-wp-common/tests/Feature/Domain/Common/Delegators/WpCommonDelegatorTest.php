...
namespace WonderPlanet\Tests\Feature\Domain\Common\Delegators;
...
/**
 * 共通ドメインのDelegatorのテスト
 */
class WpCommonDelegatorTest extends TestCase
{
    protected $backupConfigKeys = [
        'wp_common.request_unique_id_header_key',
    ];

    private WpCommonDelegator $wpCommonDelegator;

    protected function setUp(): void
    {
        parent::setUp();
        $this->wpCommonDelegator = app()->make(WpCommonDelegator::class);
    }
    ...
    public function getApiRequestId_リクエストID取得()
    {
        ...
        // Exercise
        $result = $this->wpCommonDelegator->getApiRequestId();
        // 二回呼び出しても同じ値が返ることを確認
        $result2 = $this->wpCommonDelegator->getApiRequestId();
    ...
    public function getClientRequestId_クライアントのrequest_id取得()
    {
        ...
        // Exercise
        $result = $this->wpCommonDelegator->getClientRequestId();
        // 二回呼び出しても同じ値が返ることを確認
        $result2 = $this->wpCommonDelegator->getClientRequestId();
    ...
    public function getFrontRequestId_フロントのrequest_id取得(
        ...
        // Exercise
        $result = $this->wpCommonDelegator->getFrontRequestId();
        // 二回呼び出しても同じ値が返ることを確認
        $result2 = $this->wpCommonDelegator->getFrontRequestId();
