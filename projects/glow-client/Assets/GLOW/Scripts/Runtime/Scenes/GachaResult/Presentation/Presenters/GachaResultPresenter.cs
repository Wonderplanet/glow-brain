...
namespace GLOW.Scenes.GachaResult.Presentation.Presenters
{
    ...
    public class GachaResultPresenter : IGachaResultViewDelegate
    {
        ...
        bool _hasStepRewards;
        IReadOnlyList<GLOW.Modules.CommonReceiveView.Domain.Model.CommonReceiveResourceModel> _stepRewardModels;
        DrawableFlag _originalDrawableFlag;
        ...
        public void OnViewDidLoad()
        {
            ...
            _hasStepRewards = useCaseModel.StepRewardModels.Count > 0;

            // ステップ報酬情報を保存
            if (_hasStepRewards)
            {
                _stepRewardModels = useCaseModel.StepRewardModels;
                _originalDrawableFlag = useCaseModel.DrawableFlag;
            }
            ...
            // ステップ報酬がある場合はガシャ結果画面のボタンを無効化
            if (_hasStepRewards)
            {
                ViewController.SetIsGachaReDrawable(DrawableFlag.False);
            }
            ...
            // ステップ報酬がある場合はアニメーション完了後に表示するようコールバックを設定
            if (_hasStepRewards)
            {
                ViewController.OnAllAnimationCompleted = () => ShowStepRewardReceiveView(_stepRewardModels, _originalDrawableFlag);
            }
        }
        ...
        void ShowStepRewardReceiveView(
            IReadOnlyList<GLOW.Modules.CommonReceiveView.Domain.Model.CommonReceiveResourceModel> stepRewardModels,
            DrawableFlag originalDrawableFlag)
        {
            var stepRewardViewModels = stepRewardModels
                .Select(model => CommonReceiveResourceViewModelTranslator
                    .TranslateToCommonReceiveViewModel(model))
                .ToList();
            ...
            CommonReceiveWireFrame.Show(
                ...
                onClosed: () =>
                {
                    // 汎用受け取り画面を閉じたら、元のガシャ結果画面のボタン状態を復元
                    ViewController.SetIsGachaReDrawable(originalDrawableFlag);
                });
        }
