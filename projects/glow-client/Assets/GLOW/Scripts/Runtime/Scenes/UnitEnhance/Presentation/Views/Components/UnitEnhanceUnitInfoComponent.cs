...
namespace GLOW.Scenes.UnitEnhance.Presentation.Views.Components
{
    public class UnitEnhanceUnitInfoComponent : MonoBehaviour
    {
        ...
        [SerializeField] UIObject _unitArtworkButtonRoot;
        [SerializeField] Animator _unitArtworkButtonAnimator;

        const string _artworkButtonOpenAnimationName = "GengaBtn";
        const string _artworkButtonLcokAnimationName = "GengaLockBtn";
        ...
        public void Setup(UnitEnhanceUnitInfoViewModel viewModel)
        {
            ...
            SetUnitArtworkButton(viewModel.ArtworkButtonState);
        ...
        void SetUnitArtworkButton(UnitEnhanceArtworkButtonStateType stateType)
        {
            if (!_unitArtworkButtonRoot) return;

            var isVisible = stateType != UnitEnhanceArtworkButtonStateType.Hidden;
            var isLocked = stateType == UnitEnhanceArtworkButtonStateType.Locked;

            _unitArtworkButtonRoot.IsVisible = isVisible;
            _unitArtworkButtonAnimator.Play(
                isLocked ? _artworkButtonLcokAnimationName : _artworkButtonOpenAnimationName);
        }
    }
}
