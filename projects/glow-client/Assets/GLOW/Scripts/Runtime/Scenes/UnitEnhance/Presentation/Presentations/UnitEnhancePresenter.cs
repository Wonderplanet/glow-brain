...
namespace GLOW.Scenes.UnitEnhance.Presentation.Presentations
{
    public class UnitEnhancePresenter : IUnitEnhanceViewDelegate
    {
        ...
        [Inject] CheckUnitGradeUpArtworkRewardUseCase CheckUnitGradeUpArtworkRewardUseCase { get; }
        [Inject] ExecuteUnitGradeUpArtworkRewardUseCase ExecuteUnitGradeUpArtworkRewardUseCase { get; }
        ...
        void IUnitEnhanceViewDelegate.ViewDidLoad()
        {
            ...
            CheckAndShowUnitGradeUpArtworkReward();
        }

        void IUnitEnhanceViewDelegate.OnViewWillAppear()
        {
            // 別画面へ遷移し戻ってきた際に正しくAnimationが動かないことがあるためUpdateをかける
            UpdateUnitInfo();
        }
        ...
        void IUnitEnhanceViewDelegate.OnGradeUpArtworkButtonTapped()
        {
            // ArtworkIdがEmptyの場合はボタン自体が非表示になっているはずだが一応確認する
            if (_unitInfoViewModel.ArtworkId.IsEmpty()) return;

            var argument = new ArtworkEnhanceViewController.Argument(
                _unitInfoViewModel.ArtworkId,
                new List<MasterDataId> { _unitInfoViewModel.ArtworkId });
        ...
        void IUnitEnhanceViewDelegate.OnGradeUpButtonTapped()
        {
            ...
            var confirmArgument = new UnitEnhanceGradeUpConfirmDialogViewController.Argument(_currentUnitId, () =>
            {
                DoAsync.Invoke(ViewController.View, ScreenInteractionControl, async cancellationToken =>
                {
                    ...
                    var args = new UnitEnhanceGradeUpDialogViewController.Argument(
                        result.UserUnitId,
                        result.BeforeGrade,
                        result.AfterGrade,
                        result.ArtworkFragmentAcquisitionModel);
        ...
        void CheckAndShowUnitGradeUpArtworkReward()
        {
            // 原画獲得条件を満たしていない場合はreturn
            if (!CheckUnitGradeUpArtworkRewardUseCase.CheckUnitGradeUpArtworkReward(_currentUnitId).Value)
            {
                return;
            }
            ...
            DoAsync.Invoke(ViewController.View, ScreenInteractionControl, async cancellationToken =>
            {
                var model = await ExecuteUnitGradeUpArtworkRewardUseCase.ExecuteUnitGradeUpArtworkReward(cancellationToken, _currentUnitId);

                var viewModel = ArtworkFragmentAcquisitionViewModel.Empty;
                if (!model.IsEmpty())
                {
                    viewModel = new ArtworkFragmentAcquisitionViewModel(
