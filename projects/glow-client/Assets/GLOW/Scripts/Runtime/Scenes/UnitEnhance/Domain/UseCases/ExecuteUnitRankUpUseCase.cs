...
{
    public class ExecuteUnitRankUpUseCase
    {
        [Inject] IUnitService UnitService { get; }
        [Inject] IGameRepository GameRepository { get; }
        [Inject] IGameManagement GameManagement { get; }

        public async UniTask<UnitEnhanceRankUpResultModel> ExecuteRankUp(CancellationToken cancellationToken, UserDataId userUnitId)
        {
            var beforeUnit = GameRepository.GetGameFetchOther().UserUnitModels.Find(unit => unit.UsrUnitId == userUnitId);
            var beforeRank = beforeUnit.Rank;
            var result = await UnitService.RankUp(cancellationToken, userUnitId);
            UpdateGameModel(result.UserUnit, result.UserItems);

            return new UnitEnhanceRankUpResultModel(userUnitId, beforeRank, result.UserUnit.Rank);
        }
