    ...
    public class AdventBattleTopPresenter : IAdventBattleTopViewDelegate
    {
        ...
        async UniTask ShouldShowAdventBattleTutorialDialog(CancellationToken cancellationToken)
        {
            ...
            using (ViewController.ViewTapGuard())
            {
                ...
                {
                    var adventBattleTutorialTcs = new UniTaskCompletionSource();

                    TutorialTipDialogViewWireFrame.ShowTutorialTipDialogs(
                        ViewController,
                        tutorialFunctionName,
                        () => adventBattleTutorialTcs.TrySetResult());

                    // チュートリアルダイアログが閉じられるまで待機
                    await adventBattleTutorialTcs.Task.AttachExternalCancellation(cancellationToken);
                ...
                {
                    var raidAdventBattleTutorialTcs = new UniTaskCompletionSource();

                    TutorialTipDialogViewWireFrame.ShowTutorialTipDialogs(
                        ViewController,
                        useCaseModel.TutorialTipModels,
                        () => raidAdventBattleTutorialTcs.TrySetResult());

                    // チュートリアルダイアログが閉じられるまで待機
                    await raidAdventBattleTutorialTcs.Task.AttachExternalCancellation(cancellationToken);
                    await CompleteFreePartTutorialUseCase.CompleteFreePartTutorial(
                        cancellationToken,
