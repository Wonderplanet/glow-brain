...
namespace GLOW.Scenes.Home.Presentation.Presenters
{
        ...
        ITutorialHomeViewDelegate
    {
        ...
        [Inject] ApplyArtworkFormationUseCase ApplyArtworkFormationUseCase { get; }
        ...
        void IHomeViewControl.OnPvpTopSelected()
        {
            ...
            if (pvpOpeningStatusModel.PvpQuestContentOpeningStatus.OpeningStatusAtUserStatus == QuestContentOpeningStatusAtUserStatus.RankLocked ||
                pvpOpeningStatusModel.PvpQuestContentOpeningStatus.OpeningStatusAtUserStatus == QuestContentOpeningStatusAtUserStatus.StageLocked)
            {
                CommonToastWireFrame.ShowScreenCenterToast(
        ...
        void IHomeViewControl.OnGachaContentSelectedFromHome(MasterDataId oprGachaId)
        {
            ...
            DoAsync.Invoke(CancellationToken, async _ =>
            {
                ...
                factory.ChangeTargetGachaContent(oprGachaId);
                await SwitchContentAsync(HomeContentTypes.Gacha, () => {});
            });
        }
        ...
        void IOutpostArtworkApplier.AsyncApplyOutpostArtwork()
        {
            DoAsync.Invoke(ViewController.View, ScreenInteractionControl, async cancellationToken =>
            {
                await ApplyOutpostArtwork(cancellationToken);
            });
        }

        void IArtworkFormationApplier.AsyncApplyArtworkFormation(IReadOnlyList<MasterDataId> mstArtworkIds)
        {
            DoAsync.Invoke(ViewController.View, ScreenInteractionControl, async cancellationToken =>
            {
                try
                {
                    await ApplyArtworkFormationUseCase.ApplyArtworkFormation(cancellationToken, mstArtworkIds);
                }
                catch (PartyDuplicateArtworkIdException)
                {
                    MessageViewUtil.ShowMessageWithClose(
                        "原画編成",
                        "同じ原画を複数編成することはできません");
                }
                catch (PartyInvalidArtworkIdException)
                {
                    MessageViewUtil.ShowMessageWithClose(
                        "原画編成",
                        "所持していない原画が含まれています");
                }
                catch (PartyOverArtworkCostException)
                {
                    MessageViewUtil.ShowMessageWithClose(
                        "原画編成",
                        "原画のコスト上限を超えています");
                }
                catch (PartyOverArtworkCountException)
                {
                    MessageViewUtil.ShowMessageWithClose(
                        "原画編成",
                        "編成できる原画の枚数を超えています");
