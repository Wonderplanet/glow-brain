...
namespace GLOW.Scenes.Home.Presentation.Views.HomeStageLimitStatusView
{
    public sealed class HomeStageLimitStatusView : UIView
    {
        ...
        [SerializeField] GameObject _unitColorArea;
        [SerializeField] List<GameObject> _unitColors;
        ...
        public void SetupUnitColor(IReadOnlyList<CharacterColor> characterColors)
        {
            _unitColorArea.SetActive(characterColors.Count > 0);
            _unitColors.ForEach(color => color.SetActive(false));
            foreach (var color in characterColors)
            {
                if (color == CharacterColor.None) continue;

                var index = (int)color - 1;
                if (index < 0 || index >= _unitColors.Count) continue;
                _unitColors[index].SetActive(true);
            }
        }
