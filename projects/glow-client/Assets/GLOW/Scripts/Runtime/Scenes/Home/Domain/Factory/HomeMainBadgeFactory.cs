    ...
    public class HomeMainBadgeFactory : IHomeMainBadgeFactory
    {
        ...
        public HomeMainBadgeModel GetHomeMainBadgeModel()
        {
            return new HomeMainBadgeModel(
                ...
                CreateArtworkPanelMissionBadge(),
        ...
        NotificationBadge CreateArtworkPanelMissionBadge()
        {
            var latestMstEventModel = MstEventDataRepository.GetEvents()
                .Where(m => CalculateTimeCalculator.IsValidTime(TimeProvider.Now, m.StartAt, m.EndAt))
                .MaxBy(m =>m.StartAt);
            ...
            var latestMstArtworkPanelMissionModel = MstArtworkPanelMissionDataRepository
                .GetMstArtworkPanelMissionModels(latestMstEventModel.Id)
                .FirstOrDefault(MstArtworkPanelMissionModel.Empty);
            if (latestMstArtworkPanelMissionModel.IsEmpty()) return NotificationBadge.False;

            var unreceivedMissionRewardCount =
                GameRepository.GetGameFetch().BadgeModel.UnreceivedMissionArtworkPanelRewardCountById(
                    latestMstArtworkPanelMissionModel.Id);
