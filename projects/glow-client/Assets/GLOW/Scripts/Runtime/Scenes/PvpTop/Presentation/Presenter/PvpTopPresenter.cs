...
namespace GLOW.Scenes.PvpTop.Presentation
{
    public class PvpTopPresenter : IPvpTopViewDelegate
    {
        ...
        PvpEndAt _pvpEndAt;
        MasterDataId _pvpExchangeShopId = MasterDataId.Empty;
            ...
            DoAsync.Invoke(
                _cancellationToken, async (cancellationToken) =>
                {
                    ...
                    using (ScreenInteractionControl.Lock())
                    {
                        ...
                        _pvpExchangeShopId = useCaseModel.MstPvpExchangeShopId;
                    }
        ...
        void IPvpTopViewDelegate.OnPvpExchangeButtonTapped()
        {
            // IsEmpty時は交換所ボタンを非表示にするため、通常はここに来ることはないが、念のためのガード
            if (_pvpExchangeShopId.IsEmpty()) return;

            var argument = new ExchangeShopTopViewController.Argument(_pvpExchangeShopId);
        ...
        void IPvpTopViewDelegate.OnPartyEditTapped()
        {
            ...
            var controller = ViewFactory.Create<
                HomePartyTabViewController,
                HomePartyTabViewController.Argument>(argument);
            HomeViewNavigation.TryPush(controller, HomeContentDisplayType.BottomOverlap);
        }
