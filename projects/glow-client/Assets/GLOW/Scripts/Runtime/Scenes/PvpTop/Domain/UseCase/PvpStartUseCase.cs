...
namespace GLOW.Scenes.PvpTop.Domain.UseCase
{
    public class PvpStartUseCase
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            SetRandomSeed(gameFetchOther, randomSeed);
        ...
        // 副作用 シード値の保存
        void SetRandomSeed(GameFetchOtherModel gameFetchOther, InGameRandomSeed randomSeed)
        {
            var userInGameStatus = GameRepository.GetGameFetchOther().UserInGameStatusModel;
            var newUserInGameStatus = userInGameStatus with
            {
                InGameRandomSeed = randomSeed
            };
            var newGameFetchOther = gameFetchOther with
            {
                UserInGameStatusModel = newUserInGameStatus
            };
