...
namespace GLOW.Scenes.PvpTop.Domain.ModelFactories
{
    public class PvpTopOpponentModelFactory : IPvpTopOpponentModelFactory
    {
            ...
            MasterDataId mstPvpId)
        {
            ...
            var partyArtworks = opponentSelectStatusModel.OpponentSelectStatus.ArtworkPartyStatuses
                .Select(artworkPartyStatus =>
                {
                    var artworkData = MstArtworkDataRepository.GetArtwork(artworkPartyStatus.MstArtworkId);
                    return new PvpTopOpponentPartyArtworkModel(
                        ArtworkAssetPath.Create(artworkData.AssetKey),
                        artworkData.Rarity,
                        artworkPartyStatus.ArtworkGradeLevel);
                })
                .ToList();
