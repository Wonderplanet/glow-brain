...
namespace GLOW.Scenes.PvpTop.Domain.ModelFactories
{
    public class PvpTopModelFactory : IPvpTopModelFactory
    {
        ...
        public PvpTopUseCaseModel Create(PvpTopResultModel pvpTopResultModel)
        {
            ...
            var mstPvpExchangeShopId = GetMstPvpExchangeShopId();
        ...
        MasterDataId GetMstPvpExchangeShopId()
        {
            var now = TimeProvider.Now;
            var pvpExchangeShop = MstShopDataRepository
                .GetTradeContents()
                .FirstOrDefault(
                    m => m.TradeType == ExchangeTradeType.PvpExchangeTrade
                         && CalculateTimeCalculator.IsValidTime(now, m.StartAt.Value, m.EndAt.Value),
                    MstExchangeModel.Empty);

            return pvpExchangeShop.Id;
        }
    }
}
