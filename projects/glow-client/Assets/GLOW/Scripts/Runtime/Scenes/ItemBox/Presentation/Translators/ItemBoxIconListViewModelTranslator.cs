...
namespace GLOW.Scenes.ItemBox.Presentation.Translators
{
    public class ItemBoxViewModelTranslator
    {
        ...
        static ItemBoxIconListViewModel ToEnhanceItemViewModels(IReadOnlyList<ItemBoxModel> itemModels)
        {
            ...
            // 原画グレードアップアイテム
            var artworkEnhanceItems = itemModels
                .Where(model => model.ItemModel.IsArtworkGradeUpMaterialType())
                .OrderByDescending(model => model.ItemModel.Rarity)
                .Select(model => ItemViewModelTranslator.ToItemIconViewModel(model.ItemModel))
                .ToList();

            // シリーズ原画グレードアップアイテム
            var seriesArtworkEnhanceItems = itemModels
                .Where(model => model.ItemModel.IsArtworkGradeUpSeriesMaterialType())
                .OrderByDescending(model => model.ItemModel.Rarity)
                .Select(model => ItemViewModelTranslator.ToItemIconViewModel(model.ItemModel))
                .ToList();

            return new ItemBoxIconListViewModel(
                memoryFraguments
                    .Concat(colorMemorys)
                    .Concat(artworkEnhanceItems)
                    .Concat(seriesArtworkEnhanceItems)
                    .ToList());
        }
