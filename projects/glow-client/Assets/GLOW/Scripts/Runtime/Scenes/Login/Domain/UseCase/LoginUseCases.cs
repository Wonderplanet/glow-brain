...
using GLOW.Scenes.Login.Domain.Provider;
...
namespace GLOW.Scenes.Login.Domain.UseCase
{
    public sealed class LoginUseCases
    {
        ...
        [Inject] ICountryCodeProvider CountryCodeProvider { get; }
        ...
        public async UniTask Login(CancellationToken cancellationToken, IProgress<float> progress)
        {
            ...
            if (tutorialStatus.ShouldDownloadOnLogin())
            {
        ...
        {
            // チュートリアル中は中断復帰から再開しない
            if (!GameRepository.GetGameFetchOther().TutorialStatus.IsCompleted()) return;
        ...
        async UniTask<GameUpdateAndFetchResultModel> UpdateAndFetch(CancellationToken cancellationToken, IProgress<float> progress)
        {
            ...
            // 国コードの取得
            var countryCode = await CountryCodeProvider.GetCountryCodeAsync(cancellationToken);
            var updateAndFetchResultModel = await GameService.UpdateAndFetch(cancellationToken, countryCode);
