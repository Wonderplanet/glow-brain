...
namespace GLOW.Scenes.GachaContent.Presentation.ViewModels
{
        ...
        IReadOnlyList<MasterDataId> PickupMstUnitIds)
    {
        ...
        public bool ShouldShowSingleGachaFreeText()
        {
            // 単発無料ガシャのテキスト表示 回数上限到達は非表示
            return SingleDrawButtonViewModel.SingleDrawCostType == CostType.Free
                && IsDrawableFlagByHasDrawLimitedCount;
        }
        ...
        {
            // 回数上限到達もしくは無料ガシャの場合、10連コスト表示を消す
            return IsDrawableFlagByHasDrawLimitedCount
                   && MultiDrawButtonViewModel.MultiDrawCostType != CostType.Free;
        }
        
        public bool ShouldShowMultiGachaFreeText()
        {
            // 10連無料ガシャのテキスト表示 回数上限到達は非表示
            return MultiDrawButtonViewModel.MultiDrawCostType == CostType.Free
                && IsDrawableFlagByHasDrawLimitedCount;
        }
