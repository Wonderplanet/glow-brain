...
{
    public interface IGachaUnitAvatarPageListDelegate
    {
    ...
    public class GachaUnitAvatarPageListComponent : MonoBehaviour,
        ...
        IUIPageViewControllerPageControlDataSource
    {
        ...
        [Header("offだと左、onだと右向きに表示される")] [SerializeField]
        bool _defaultFlip;
        ...
        public IGachaUnitAvatarPageListDelegate Delegate { get; set; }

        void SetFlip(bool isFlip)
        {
            foreach (var vc in _contentViewControllers)
            {
                var avatarPage = vc as GachaUnitAvatarPageViewController;
                avatarPage?.SetFlip(isFlip);
            }
        }

        public void ResetUnitAnimation()
        {
            var viewController = _pageViewController.ViewControllers.First() as GachaUnitAvatarPageViewController;
            viewController?.PlayWaitAnimation();
        }
            ...
            MasterDataId presentationMstUnitId)
        {
            ...
            // TODO: UIPageViewは子要素が3つ未満だと挙動がおかしくなるので、それ以上になるようリストを調整
            ...
            foreach (var id in tempList)
            {
                ...
                var controller =
                    viewFactory.Create<GachaUnitAvatarPageViewController, IUnitAvatarPageViewController.Argument>(
                        argument);
        ...
        void IUIPageViewControllerDelegate.WillTransitionTo(UIPageViewController pageViewController,
            UIViewController[] pendingViewControllers)
        {
            ...
            {
                var avatarPage = controller as GachaUnitAvatarPageViewController;
        ...
        void IUIPageViewControllerDelegate.DidFinishAnimating(UIPageViewController pageViewController,
            bool finished, UIViewController[] previousViewControllers, bool transitionCompleted)
        {
            ...
            // デフォ位置に戻った際にすぐ元の向きに戻ると見た目上違和感があるので少し遅延させる
            SetFlip(_defaultFlip);
            ...
            {
                // ページ切り替えがキャンセルされた場合
                Delegate.DidCancelTransition();
            }
        }

        UIViewController IUIPageViewControllerDataSource.ViewControllerAfter(
            UIPageViewController pageViewController, UIViewController viewController)
        {
