...
namespace GLOW.Scenes.BattleResult.Domain.UseCases
{
    public class VictoryUseCase
    {
        ...
        public async UniTask<VictoryResultModel> Victory(CancellationToken cancellationToken)
        {
            ...
            {
                // チュートリアル中のリザルトを上書きする
                var result = await UpdateTutorialStatusAndGetVictoryResult(cancellationToken, tutorialStatus, selectedStage.SelectedStageId);
                return result;
            }
        ...
        async UniTask<VictoryResultModel> UpdateTutorialStatusAndGetVictoryResult(
            ...
            MasterDataId mstStageId)
        {
            ...
            // チュートリアル進行状態含むユーザー情報の更新
            TutorialStatusApplier.UpdateTutorialStageEndResult(result);
            
            // 経験値を受け取るので、キャッシュをリセットする
