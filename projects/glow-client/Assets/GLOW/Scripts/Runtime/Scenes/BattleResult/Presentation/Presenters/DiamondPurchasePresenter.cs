...
namespace GLOW.Scenes.BattleResult.Presentation.Presenters
{
    ...
    public class DiamondPurchasePresenter : IDiamondPurchaseViewDelegate
    {
        ...
        [Inject] DiamondPurchaseWireframe DiamondPurchaseWireFrame { get; }
            ...
            UIIndexPath indexPath)
        {
            ...
            if (confirmationModel.IsEmpty())
            {
                DiamondPurchaseWireFrame.ShowOutOfTimeDiamondPurchaseView(OnCloseSelected);
                return;
            }

            ShowConfirmView(oprProductId, confirmationModel, indexPath);
        }

        void ShowConfirmView(
            MasterDataId oprProductId,
            ProductBuyWithCashConfirmationModel confirmationModel,
            UIIndexPath indexPath)
        {
