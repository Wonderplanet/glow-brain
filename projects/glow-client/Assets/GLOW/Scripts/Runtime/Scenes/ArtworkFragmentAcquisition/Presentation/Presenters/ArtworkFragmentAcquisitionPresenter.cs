...
namespace GLOW.Scenes.ArtworkFragmentAcquisition.Presentation.Presenters
{
    ...
    public class ArtworkFragmentAcquisitionPresenter : IArtworkFragmentAcquisitionViewDelegate
    {
        ...
        ArtworkFragmentAcquisitionViewModel _viewModel;

        public void OnViewDidLoad()
        {
            ApplicationLog.Log(nameof(ArtworkFragmentAcquisitionPresenter), nameof(OnViewDidLoad));

            _viewModel = Argument.ViewModel;

            OverrideViewModelIfAllUnlocked();

            ViewController.Setup(_viewModel);
        ...
        async UniTask PlayArtworkFragmentAnimation(CancellationToken cancellationToken)
        {
            ...
            var fragmentAnimationCanceled = await
                ViewController.PlayArtworkFragmentAnimation(_viewModel.AcquiredArtworkFragmentIds, fragmentAnimationCancellationToken)
            ...
            if (fragmentAnimationCanceled)
            {
                ...
                ViewController.SkipArtworkFragmentAnimation(_viewModel.AcquiredArtworkFragmentIds);
            }
        ...
        async UniTask PlayArtworkCompleteAnimation(CancellationToken cancellationToken)
        {
            ...
            var completeAnimationCanceled = await ViewController.PlayArtworkCompleteAnimation(_viewModel.AddHp, completeAnimationCancellationToken)
                .SuppressCancellationThrow();
            cancellationToken.ThrowIfCancellationRequested();
        ...
        void OverrideViewModelIfAllUnlocked()
        {
            if (!_viewModel.IsCompletedWithoutFragmentData()) return;

            var numList = new List<ArtworkFragmentPositionNum>();
            for (int i = 0; i < 16; i++)
            {
                numList.Add(new ArtworkFragmentPositionNum(i + 1));
            }

            _viewModel = _viewModel with { AcquiredArtworkFragmentIds = numList };
        }
    }
}
