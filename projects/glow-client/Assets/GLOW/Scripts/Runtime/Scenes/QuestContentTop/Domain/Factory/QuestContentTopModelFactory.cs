...
namespace GLOW.Scenes.QuestContentTop.Domain
{
    public class QuestContentTopModelFactory : IQuestContentTopModelFactory
    {
        ...
        QuestContentTopElementUseCaseModel CreateEventQuestModel(MstEventModel mstEventModel)
        {
            ...
            var missionBadge = CheckExistNotificationBadge(mstEventModel.Id, badgeModel);
        ...
        IReadOnlyDictionary<MasterDataId, NotificationBadge> IQuestContentTopModelFactory.CreateEventMissionBadgeDictionary()
        {
            var badgeModel = GameRepository.GetGameFetch().BadgeModel;
            ...
            return MstEventDataRepository.GetEvents()
                ...
                .ToDictionary(
                    m => m.Id,
                    m => CheckExistNotificationBadge(m.Id, badgeModel));
        }
        ...
        NotificationBadge CheckExistNotificationBadge(
            MasterDataId mstEventId,
            BadgeModel badgeModel)
        {
            // イベントミッションの報酬が未受取のものがある場合はバッジ表示
            var unreceivedEventMission = badgeModel.UnreceivedMissionEventRewardCountById(mstEventId);
            if (!unreceivedEventMission.IsZero())
            {
                return NotificationBadge.True;
            }
                    
            var mstArtworkPanelMissionModels = MstArtworkPanelMissionDataRepository
                .GetMstArtworkPanelMissionModels(mstEventId);
            if (mstArtworkPanelMissionModels.IsEmpty())
            {
                // 原画パネルミッションが存在しない、かつイベントミッションの報酬が未受取のものもない場合はバッジ非表示
                return NotificationBadge.False;
            }
                    
            var mstArtworkPanelIds = mstArtworkPanelMissionModels
                .Select(model => model.Id)
                .ToList();
            var isExistUnreceivedArtworkPanelMissionCount = mstArtworkPanelIds
                .Select(badgeModel.UnreceivedMissionArtworkPanelRewardCountById)
                .Any(count => !count.IsZero());
                    
            return new NotificationBadge(isExistUnreceivedArtworkPanelMissionCount);
        }
    }
}
