...
namespace GLOW.Scenes.GachaList.Domain.UseCases
{
    public class TutorialGachaDrawUseCase
    {
        ...
        public async UniTask GachaDraw(CancellationToken cancellationToken)
        {
            ...
            var drawInfo = GachaDrawInfoModel.CreateTutorialDrawInfoModel(tutorialGachaModel.GachaId);
            GachaCacheRepository.SaveGachaDrawType(drawInfo);

            // 必ずGachaListViewから引くためFalseに設定
            GachaCacheRepository.SetGachaDrawFromContentViewFlag(GachaDrawFromContentViewFlag.False);
        }
    }
}
