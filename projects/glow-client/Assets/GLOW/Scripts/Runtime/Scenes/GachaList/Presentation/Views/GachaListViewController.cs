...
namespace GLOW.Scenes.GachaList.Presentation.Views
{
        ...
        IGachaContentViewFactory
    {
        ...
        // _currentTappedOprGachaId更新タイミング
        // 1. 初回表示
        // 2. バナー・左右ボタンタップ
        // 3. HomeBannerなどHomeFooterBanner以外からの直接遷移(IGachaContentViewFactory.ChangeTargetGachaContent)
        MasterDataId _currentTappedOprGachaId = MasterDataId.Empty;
        MasterDataId CurrentTappedOprGachaId => _currentTappedOprGachaId;
        ...
        public override void ViewWillAppear(bool animated)
        {
            ...
            // 場所依存: InitializeView()があるが、そのタイミングでは遅いのでここで行う
            PrepareForView();


            GachaListViewDelegate.OnViewWillAppear(_currentTappedOprGachaId);
        ...
        {
            _currentTappedOprGachaId = oprGachaId;
        }
        #endregion
        ...
        {
            // ガシャコンテンツビューの初期化
            ActualView.GachaContentView.InitializeView();

            // ガシャコンテンツアセットのトランジション初期化
            ActualView.GachaContentView.InitializeGachaContentAssetTransition(() => { });
            // 画面が埋まった状態からスタート
            ActualView.GachaContentView.StartGachaContentAssetTransitAnimation(0f);
        }
            ...
            if (!GachaContentAssetContainer.Exists(gachaContentAssetPath))
            {
                GachaListViewDelegate.LoadGachaAsset(
                    ...
                    () =>
                    {
                        ...
                        SwitchContent(contentViewModel, stepUpGachaViewModel, asset);
                    });
                return;
            }
        ...
        {
            UpdateContent(contentViewModel, stepUpGachaViewModel, asset);
        }
        ...
        {
            ShowGachaConfirmDialog(CurrentTappedOprGachaId, GachaDrawType.Ad);
        }

        [UIAction]
        public void OnSingleDrawButtonTapped()
        {
            ShowGachaConfirmDialog(CurrentTappedOprGachaId, GachaDrawType.Single);
        }

        [UIAction]
        public void OnMultiDrawButtonTapped()
        {
            if (_gachaListViewModel.IsTutorialTarget(CurrentTappedOprGachaId))
            {
                GachaListViewDelegate.OnTutorialGachaDrawButtonTapped();
                return;
            }

            ShowGachaConfirmDialog(CurrentTappedOprGachaId, GachaDrawType.Multi);
        }
        ...
        {
            var currentElementViewModel = _gachaListViewModel
                .GetListElementViewModels()
                .FirstOrDefault(vm => vm.GachaFooterBannerViewModel.OprGachaId == CurrentTappedOprGachaId);

            if (currentElementViewModel != null && currentElementViewModel.IsStepUpGacha())
            {
                // ステップアップガシャ提供割合ダイアログの表示
                GachaListViewDelegate.ShowStepupGachaRatioDialogView(CurrentTappedOprGachaId);
            }
            else
            {
                // ガシャ提供割合ダイアログの表示
                GachaListViewDelegate.ShowGachaRatioDialogView(CurrentTappedOprGachaId);
            }
        }
        ...
        {
            // ガシャラインナップダイアログの表示
            GachaListViewDelegate.ShowGachaLineUpDialogView(CurrentTappedOprGachaId);
        }
        ...
        {
            // ガシャ詳細ダイアログの表示
            GachaListViewDelegate.ShowGachaDetailDialogView(CurrentTappedOprGachaId);
        }
