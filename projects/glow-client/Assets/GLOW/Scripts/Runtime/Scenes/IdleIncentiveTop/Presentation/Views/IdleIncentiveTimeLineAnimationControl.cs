...
namespace GLOW.Scenes.IdleIncentiveTop.Presentation.Views
{
    public class IdleIncentiveTimeLineAnimationControl : UIObject, IIdleIncentiveBattleUnitTrackClipDelegate
    {
        ...
        [SerializeField] UIAnimator _playerAnimator;
        [Header("敵Spineデータ")]
            ...
            PhantomizedFlag enemyIsPhantomized)
        {
            ...
            // 味方キャラが背景枠外から登場するアニメーションの再生が終わった後に実行する
            _playerAnimator.OnAnimationCompleted = () =>
            {
                var updatedPosition = _playerCharacterRoot.transform.localPosition;
                updatedPosition.z = -10f;  // 味方キャラのz座標を変更して、エフェクトが背景の上に表示されるようにする
                _playerCharacterRoot.transform.localPosition = updatedPosition;
            };
