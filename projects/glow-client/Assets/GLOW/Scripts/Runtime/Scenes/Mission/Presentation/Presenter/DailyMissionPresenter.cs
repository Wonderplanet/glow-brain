...
{
    public class DailyMissionPresenter : IDailyMissionViewDelegate
    {
        [Inject] IMissionMainControl MissionMainViewControl { get; }
        [Inject] DailyMissionViewController ViewController { get; }
        ...
        CancellationToken DailyMissionCancellationToken => ViewController.ActualView.GetCancellationTokenOnDestroy();

        public void OnViewDidLoad()
        {
            ApplicationLog.Log(nameof(DailyMissionPresenter), nameof(OnViewDidLoad));
        ...
        void IDailyMissionViewDelegate.ReceiveAchievedMissionRewards(MasterDataId dailyMissionId)
        {
            CommonReceiveWireFrame.AsyncShowReceived(
                cancellationToken => ReceiveRewardSingle(dailyMissionId, cancellationToken),
        ...
        {
            // 一括受取りタイミングでのポイント総計
            var bonusPoint = Argument.ViewModel.DailyMissionCellViewModels
        ...
        void IDailyMissionViewDelegate.ShowRewardListWindow(
            IReadOnlyList<PlayerResourceIconViewModel> viewModels,
            RectTransform windowPosition)
        {
            MissionMainViewControl
                .ShowRewardListWindow(viewModels, windowPosition, DailyMissionCancellationToken)
                .Forget();
        }

        void IDailyMissionViewDelegate.OnMissionBonusPointSelected()
        {
            ItemDetailWireFrame.ShowMissionBonusPointDetail(PlayerResourceConst.DailyBonusPointMasterDataId, ViewController);
        }
            ...
            CancellationToken cancellationToken)
        {
            var receiveRewardTask = UniTask.Create(async () =>
            {
                ...
                var missionViewModel = DailyMissionViewModelTranslator.ToDailyMissionViewModel(
                    model.MissionFetchResultModel.DailyResultModel,
        ...
        {
            DoAsync.Invoke(DailyMissionCancellationToken, async cancellationToken =>
            {
        ...
        {
            var nextUpdateTime = GetMissionNextUpdateTimeUseCase.GetNextUpdateTime(MissionType.Daily);
