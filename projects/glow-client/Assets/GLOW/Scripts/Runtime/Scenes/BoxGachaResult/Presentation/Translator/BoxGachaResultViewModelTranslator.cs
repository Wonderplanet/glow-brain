...
namespace GLOW.Scenes.BoxGachaResult.Presentation.Translator
{
    public static class BoxGachaResultViewModelTranslator
    {
        public static BoxGachaResultViewModel Translate(BoxGachaDrawModel drawModel)
        {
            ...
            var convertedResourceTypeList = drawModel.DrawnBoxGachaResultRewards
                .Where(model => !model.BoxGachaReward.PreConversionPlayerResourceModel.IsEmpty())
                .Select(model => model.BoxGachaReward.PreConversionPlayerResourceModel.Type)
                .Distinct()
                .ToList();
