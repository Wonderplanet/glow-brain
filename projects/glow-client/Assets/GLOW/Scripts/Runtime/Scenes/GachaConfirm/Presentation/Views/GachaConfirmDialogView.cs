...
namespace GLOW.Scenes.GachaConfirm.Presentation.Views
{
    ...
    public class GachaConfirmDialogView : UIView
    {
        ...
        public void SetViewModel(GachaConfirmDialogViewModel viewModel)
        {
            ...
            switch (viewModel.CostType)
            {
                ...
                case CostType.Diamond:
                    ...
                    SetDiamondDrawText(viewModel, false, viewModel.GachaType == GachaType.Stepup);
                ...
                case CostType.PaidDiamond:
                    ...
                    SetDiamondDrawText(viewModel, true, viewModel.GachaType == GachaType.Stepup);
                ...
                case CostType.Free:
                    _resourceArea.SetActive(false); // 無料ガシャのためリソースのエリアを非表示にする
                    _gachaAdDrawText.Hidden = false;
                    _cancelButton.gameObject.SetActive(true);
                    _drawButton.gameObject.SetActive(viewModel.DrawableFlag.Value);
                    _gachaDrawCount = viewModel.GachaDrawCount;
                    
                    SetFreeDrawText(viewModel);
                    break;
                ...
                case CostType.Item: // 消費アイテム不足時は「ひく」ボタングレイアウトでここまで遷移しない
                    ...
                    // ステップアップガチャの場合は回数選択UIを非表示
                    var isStepUpGacha = viewModel.GachaType == GachaType.Stepup;
                    _amountSelectionComponent.Hidden = isStepUpGacha || !viewModel.GachaDrawCount.IsSingleDraw();

                    if (isStepUpGacha)
                    {
                        SetItemDrawText(viewModel, costItemAmount);
                    }
                    else if (viewModel.GachaDrawCount.IsSingleDraw())
                    {
                        // 所持数で引ける回数(切り捨て)
        ...
        void SetDiamondDrawText(GachaConfirmDialogViewModel viewModel, bool isPaidDiamond, bool isStepup)
        {
            var diamondType = isPaidDiamond ? "有償プリズム" : "プリズム";
            
            if (isStepup)
            {
                _gachaDiamondDrawText.SetText(
                    "{0}を<color=#EE3628>{1}</color>個使用して\n{2}、{3}ステップ目を引きますか？",
                    diamondType,
                    viewModel.CostAmount.ToString(),
                    _title.Value,
                    viewModel.CurrentStepNumber.Value);
            }
            else
            {
                _gachaDiamondDrawText.SetText(
        ...
        void SetItemDrawText(GachaConfirmDialogViewModel viewModel, ItemAmount costItemAmount)
        {
            _gachaTicketDrawText.SetText(
                "{0}を<color=#EE3628>{1}</color>枚使用して\n{2}、{3}ステップ目を引きますか？",
                GetConsumeItemName(viewModel.CostName),
                costItemAmount.ToStringSeparated(),
                _title.Value,
                viewModel.CurrentStepNumber.Value);
        }
        ...
        void SetFreeDrawText(GachaConfirmDialogViewModel viewModel)
        {
            if (viewModel.GachaType == GachaType.Stepup)
            {
                _gachaAdDrawText.SetText(
                    "無料で\n{0}、{1}ステップ目を引きますか？",
                    _title.Value,
                    viewModel.CurrentStepNumber.Value);
            }
