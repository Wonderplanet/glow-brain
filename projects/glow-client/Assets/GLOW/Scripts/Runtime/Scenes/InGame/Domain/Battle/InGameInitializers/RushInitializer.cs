...
namespace GLOW.Scenes.InGame.Domain.Battle.InGameInitializers
{
    public class RushInitializer : IRushInitializer
    {
        ...
        {
            var artworkEffectRushChargeSpeedUp = TickCount.Empty;
            foreach (var effectElement in artworkEffect.EffectElements)
            {
                if (effectElement.EffectType == ArtworkEffectType.JumbleRushChargeSpeedUp &&
                    ArtworkEffectActivationEvaluator.EvaluateActivation(
                        effectElement.ActivationRules,
                        deckUnits))
                {
                    artworkEffectRushChargeSpeedUp += effectElement.EffectValue.ToTickCount();
                }
            }

            // 原画効果ダメージボーナス
            var artworkEffectRushDamageUpBonus = PercentageM.Hundred;
            foreach (var effectElement in artworkEffect.EffectElements)
            {
                if (effectElement.EffectType == ArtworkEffectType.JumbleRushDamageUp &&
                    ArtworkEffectActivationEvaluator.EvaluateActivation(
                        effectElement.ActivationRules,
                        deckUnits))
                {
                    artworkEffectRushDamageUpBonus += effectElement.EffectValue.ToPercentageM();
                }
            }

            var chargeTime = RushChargeTimeCalculator.Calculate(outpostEnhancement, artworkEffectRushChargeSpeedUp, MstConfigRepository);
            ...
            var rushModel = new RushModel(
                ...
                artworkEffectRushChargeSpeedUp,
                artworkEffectRushDamageUpBonus,
                attackData,
                rushCoefficient,
                damageUpper);
