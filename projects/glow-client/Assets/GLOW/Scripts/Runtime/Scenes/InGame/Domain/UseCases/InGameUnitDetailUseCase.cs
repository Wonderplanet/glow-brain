...
namespace GLOW.Scenes.InGame.Domain.UseCases
{
    public class InGameUnitDetailUseCase
    {
        ...
        InGameUnitDetailInfoModel CreateInfoModel(MstCharacterModel mstUnit, UserUnitModel userUnit)
        {
            ...
            var fieldUnit = InGameScene.CharacterUnits
                .Where(unit => unit.BattleSide == BattleSide.Player)
                .FirstOrDefault(unit => unit.CharacterId == mstUnit.Id);
            ...
            {
                // 原画効果のStateEffectTypeを取得して追加
                // フィールド上にすでにユニットが存在する場合は、2重で表示されるため
                // 未召喚時のみ追加する
                var artworkEffectStateEffectList = GetArtworkEffectStateEffectTypeList(mstUnit);
                effectList.AddRange(artworkEffectStateEffectList);
            }
        ...
        InGameUnitDetailStatusModel CreateStatusModel(MstCharacterModel mstUnit, UserUnitModel userUnit)
        {
            ...
            var deckUnits = InGameScene.DeckUnits;
            var deckUnit = deckUnits.FirstOrDefault(unit => unit.CharacterId == mstUnit.Id, DeckUnitModel.Empty);
            var artworkEffectHpUp = !deckUnit.IsEmpty() ? deckUnit.ArtworkEffectHpBonus : PercentageM.Hundred;
            var artworkEffectAttackPowerUp = !deckUnit.IsEmpty() ? deckUnit.ArtworkEffectAttackPowerBonus : PercentageM.Hundred;
            ...
            // ランクマッチ以外は特別ルールではなく、ステージギミック扱いになるため除外
            if (InGameScene.Type == InGameType.Pvp && InGameSpecialRuleUnitStatusEvaluator.EvaluateTarget(mstUnit, InGameScene.SpecialRuleUnitStatusModels))
            {
            ...
            return new InGameUnitDetailStatusModel(
                ...
                balloonMessageList,
                new InGameTutorialIntroductionFlag(GameRepository.GetGameFetchOther().TutorialStatus.IsIntroduction()));
        }

        List<StateEffectType> GetArtworkEffectStateEffectTypeList(MstCharacterModel mstUnit)
        {
            var artworkEffectStateEffectTypeList = new List<StateEffectType>();

            var deckUnits = InGameScene.DeckUnits;
            var deckUnit = deckUnits.FirstOrDefault(unit => unit.CharacterId == mstUnit.Id, DeckUnitModel.Empty);
            if (!deckUnit.IsEmpty())
            {
                if (deckUnit.ArtworkEffectHpBonus > PercentageM.Hundred)
                {
                    artworkEffectStateEffectTypeList.Add(StateEffectType.HpUp);
                }

                if (deckUnit.ArtworkEffectAttackPowerBonus > PercentageM.Hundred)
                {
                    artworkEffectStateEffectTypeList.Add(StateEffectType.AttackPowerUp);
                }

                if (deckUnit.ArtworkEffectSpecialAttackCoolTimeBonus > TickCount.Zero)
                {
                    artworkEffectStateEffectTypeList.Add(StateEffectType.SpecialAttackCoolTimeShorten);
                }

                if (deckUnit.ArtworkEffectSummonCoolTimeBonus > TickCount.Zero)
                {
                    artworkEffectStateEffectTypeList.Add(StateEffectType.SummonCoolTimeShorten);
                }
            }

            return artworkEffectStateEffectTypeList;
        }
    }
}
