...
namespace GLOW.Scenes.InGame.Domain.Battle
{
    public class HPCalculator : IHPCalculator
    {
        record DamageCalculationResult(
            ...
            BlockedDamageByUnbeatableFlag IsBlockedDamageByUnbeatable);
            ...
            UndeadFlag isUndead)
        {
            checked
            {
                ...
                return new HPCalculatorResultModel(
                    ...
                    damageResult.IsBlockedDamageByUnbeatable);
            }
        }
            ...
            UndeadFlag isUndead)
        {
            ...
            // 無敵によるダメージ無効化か
            var isBlockedByUnbeatable = BlockedDamageByUnbeatableFlag.False;
            ...
            foreach (var damageType in DamageTypes)
            {
                ...
                if (result.IsBlockedDamageByUnbeatable)
                {
                    isBlockedByUnbeatable = BlockedDamageByUnbeatableFlag.True;
                }
            }
            ...
            UndeadFlag isUndead)
        {
            ...
            foreach (var attackResult in attackResults)
            {
                ...
                // 状態変化によるダメージ軽減(無敵 -> ダメージカットの順で適用)
                (power, updatedStateEffects, isBlockedByUnbeatable) = ApplyDamageReductionByStateEffect(
                    calculationDamageType, 
                    power, 
                ...
                var detail = new HPCalculatorResultDetailModel(
                    ...
                    new DamageMotionUnplayableFlag(isBlockedByUnbeatable));
        ...
        (AttackPower, IReadOnlyList<IStateEffectModel>, BlockedDamageByUnbeatableFlag) ApplyDamageReductionByStateEffect(
            ...
            IReadOnlyList<IStateEffectModel> stateEffects)
        {
            ...
            // 無敵
            if (damageType == AttackDamageType.Damage || damageType == AttackDamageType.SlipDamage)
            {
                var unbeatableResult = StateEffectChecker.CheckAndReduceCount(
                    StateEffectType.Unbeatable, 
                    updatedStateEffects, 
                    new StateEffectAttackHitConditionContext(attackerColor, attackerRoleType));
                
                if (unbeatableResult.IsEffectActivated)
                {
                    return (AttackPower.Zero, unbeatableResult.UpdatedStateEffects, BlockedDamageByUnbeatableFlag.True);
                }
            }
            ...
            return (updatedPower, updatedStateEffects, BlockedDamageByUnbeatableFlag.False);
        }
