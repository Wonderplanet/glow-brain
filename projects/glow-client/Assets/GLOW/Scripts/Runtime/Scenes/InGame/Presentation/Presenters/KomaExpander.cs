...
namespace GLOW.Scenes.InGame.Presentation.Presenters
{
    ...
    public class KomaExpander
    {
        ...
        public void ExpandKomaIfNeeded(IReadOnlyList<CharacterUnitModel> unitModels)
        {
            ...
            // 画面に存在しないユニットのExpandingハンドラを破棄(必殺技中にキャラが退場すると戻す処理が呼ばれないため)
            var handlers = _specialAttackKomaExpandingHandlerDictionary
                .Where(handler => !unitModelIds.Contains(handler.Key))
                .ToList();
            foreach (var handler in handlers)
            {
                handler.Value?.Dispose();
                _specialAttackKomaExpandingHandlerDictionary.Remove(handler.Key);
            }
        }
