    ...
    public static class AttackTargetSelector
    {
            ...
            AttackTargetSelectionData selectionData)
        {
            ...
            return targetBattleSide == candidate.BattleSide
                   ...
                   && IsTargetSeriesId(selectionData.TargetSeriesIds, candidate.MstSeriesId)
                   && IsTargetCharacterId(selectionData.TargetCharacterIds, candidate.CharacterId);
        }
        static bool IsTargetSeriesId(IReadOnlyList<MasterDataId> targetSeriesIds, MasterDataId seriesId)
        {
            // 空リストの場合は絞り込みなし（全対象）
            return targetSeriesIds.Count == 0 || targetSeriesIds.Contains(seriesId);
        }

        static bool IsTargetCharacterId(IReadOnlyList<MasterDataId> targetCharacterIds, MasterDataId characterId)
        {
            // 空リストの場合は絞り込みなし（全対象）
