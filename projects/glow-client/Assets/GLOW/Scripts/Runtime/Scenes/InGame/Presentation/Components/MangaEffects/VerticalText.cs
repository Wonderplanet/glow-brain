...
namespace GLOW.Scenes.InGame.Presentation.Components
{
    ...
    {
        static readonly char[] Rotation90DegreeCharacters =
        {
            'ー',
            '…',
            '〜',
        ...
        static readonly char[] Rotation180DegreeCharacters =
        {
            '“',
            '”'
        };

        static readonly char[] OffsetCharacters =
        {
            '。',
            '、',
        };
        ...
        void RotateSpecificCharacters()
        {
            ...
            for (int index = 0; index < textInfo.characterCount; index++)
            {
                ...
                if (Rotation90DegreeCharacters.Contains(charaInfo.character))
                {
                    RotateCharacter(textInfo, charaInfo, -90);
                }

                if (Rotation180DegreeCharacters.Contains(charaInfo.character))
                {
                    RotateCharacter(textInfo, charaInfo, 180);
                }

                if (OffsetCharacters.Contains(charaInfo.character))
                {
                    OffsetCharacter(textInfo, charaInfo, new Vector3(10f, 10f, 0f));
                }
            }
        ...
        static void RotateCharacter(TMP_TextInfo textInfo, TMP_CharacterInfo charaInfo, float angle)
        {
        ...
        static void OffsetCharacter(TMP_TextInfo textInfo, TMP_CharacterInfo charaInfo, Vector3 offset)
        {
            int materialIndex = charaInfo.materialReferenceIndex;
            int vertexIndex = charaInfo.vertexIndex;
            Vector3[] destVertices = textInfo.meshInfo[materialIndex].vertices;

            // 4つの頂点（左下、左上、右上、右下）すべてにオフセットを加算
            destVertices[vertexIndex + 0] += offset;
            destVertices[vertexIndex + 1] += offset;
            destVertices[vertexIndex + 2] += offset;
            destVertices[vertexIndex + 3] += offset;
