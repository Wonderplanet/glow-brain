...
using GLOW.Scenes.InGame.Domain.Battle.UpdateProcess.ImmediateEffectHandler;
...
namespace GLOW.Scenes.InGame.Domain.Battle.UpdateProcess
{
    public class AttackProcess : IAttackProcess
    {
            ...
            MstPageModel mstPage)
        {
            ...
            updatedUnits = immediateEffectResult.updatedCharacterUnits.ToList();
            ...
            return new AttackProcessResult(
                ...
                immediateEffectResult.appliedCharacterImmediateEffectResultModels,
            ...
            IReadOnlyList<HitAttackResultModel> attackResults)
        {
            ...
            foreach (var unit in characterUnits)
            {
                ...
                if (isStateEffectBlocked || hpCalculatorResult.IsBlockedByUnbeatable)
                {
            ...
            StageTimeModel stageTime)
        {
            ...
            for (var i = 0; i < orderedHitInfos.Count; i++)
            {
                ...
                // 無敵判定
                var isUnbeatableBlockType = hitInfo.HitData.HitType.IsBlockableHitTypeByUnbeatable();
                if (isUnbeatableBlockType)
                {
                    (updatedStateEffects, hitActionBlocked) = BlockHitEffect(
                        StateEffectType.Unbeatable, 
                        updatedStateEffects, 
                        context);
                    
                    if (hitActionBlocked) continue;  // 無効化されたのでスキップ
                }
        ...
        (IReadOnlyList<IStateEffectModel> updatedEffects, bool stateEffectBlocked) BlockHitEffect(
            StateEffectType blockingStateEffectType,
            IReadOnlyList<IStateEffectModel> currentEffects,
            IStateEffectConditionContext context)
        {
            var unbeatableResult = StateEffectChecker.CheckAndReduceCount(
                blockingStateEffectType,
                currentEffects,
                context);
            
            return (unbeatableResult.UpdatedStateEffects, unbeatableResult.IsEffectActivated);
        }
            ...
            IReadOnlyList<HitAttackResultModel> attackResults)
        {
            ...
            var filterByOverwritable = new List<HitAttackResultModel>();
            var updatedEffects = currentEffects;
            var stateEffectBlocked = false;
            foreach (var result in filterByHasNotMulti)
            {
                // 上書き可能な状態変化は一旦保留
                if (result.StateEffect.Type.IsOverwritable())
                {
                    filterByOverwritable.Add(result);
                    continue;
                }
                ...
                // 無敵判定
                var isUnbeatableBlockType = result.StateEffect.Type.IsBlockableByUnbeatable();
                if (isUnbeatableBlockType)
                {
                    // CheckAndReduceCount()による更新結果を反映
                    (updatedEffects, stateEffectBlocked) = BlockStateEffect(
                        StateEffectType.Unbeatable, 
                        result, 
                        updatedEffects, 
                        context);
                    
                    if (stateEffectBlocked) continue;  // 無効化されたのでスキップ
                }
            ...
            // 上書き可能な状態変化を処理
            foreach (var result in filterByOverwritable)
            {
                updatedStateEffectList = updatedStateEffectList.ReplaceOrAdd(
                    effect => effect.Type == result.StateEffect.Type, 
                    StateEffectModelFactory.Create(result.StateEffectSourceId, result.StateEffect, true))
                    .ToList();
            }
        ...
        (IReadOnlyList<IStateEffectModel> updatedEffects, bool stateEffectBlocked) BlockStateEffect(
            StateEffectType blockingStateEffectType,
            HitAttackResultModel result,
            IReadOnlyList<IStateEffectModel> currentEffects,
            IStateEffectConditionContext context)
        {
            var unbeatableResult = StateEffectChecker.CheckAndReduceCount(
        ...
        (IReadOnlyList<DeckUnitModel> updatedPlayerDeckUnits,
            ...
            IReadOnlyList<AppliedCharacterImmediateEffectResultModel> appliedCharacterImmediateEffectResultModels)
            ...
            IReadOnlyList<IAttackResultModel> attackResults)
        {
            ...
            var updatedCharacterUnits = characterUnits.ToList();
            var appliedDeckStateEffectResultModels = new List<AppliedDeckStateEffectResultModel>();
            var appliedCharacterImmediateEffectResultModels = new List<AppliedCharacterImmediateEffectResultModel>();
            ...
            foreach (var attackResult in attackResults)
            {
                ...
                if (handler != null)
                {
                    var result = handler.Handle(attackResult, updatedCharacterUnits, updatedPlayerDeckUnits, updatedPvpOpponentDeckUnits);
                    updatedPlayerDeckUnits = result.UpdatedPlayerDeckUnits.ToList();
                    updatedPvpOpponentDeckUnits = result.UpdatedPvpOpponentDeckUnits.ToList();
                    updatedCharacterUnits = result.UpdatedCharacterUnits.ToList();

                    if (!result.AppliedDeckResult.IsEmpty())
                    {
                        appliedDeckStateEffectResultModels.Add(result.AppliedDeckResult);
                    }
                    if (!result.AppliedCharacterResult.IsEmpty())
                    {
                        appliedCharacterImmediateEffectResultModels.Add(result.AppliedCharacterResult);
                    }
                }
            }

            return (updatedPlayerDeckUnits, updatedPvpOpponentDeckUnits, updatedCharacterUnits,
                    appliedDeckStateEffectResultModels, appliedCharacterImmediateEffectResultModels);
        }
    }
}
