...
namespace GLOW.Scenes.InGame.Presentation.Views
{
    public class InGameView : UIView, IScreenFlashTrackClipDelegate
    {
        ...
        public void OnFieldUnitImmediateEffect(AppliedCharacterImmediateEffectResultModel fieldUnitImmediateEffect)
        {
            var characterUnitView = BattleFieldView.GetCharacterUnitView(fieldUnitImmediateEffect.TargetId);
            if (characterUnitView == null) return;

            characterUnitView.OnImmediateEffect(fieldUnitImmediateEffect.StateEffectType);
        }
        ...
        public MultipleSwitchHandler PauseWithout(FieldObjectId id)
        {
            ...
            _pageComponent.PauseWithout(handler, id);
            _mangaAnimationPlayer.Pause(handler);
            _cutInPlayer.Pause(handler);
            _rushPlayer.Pause(handler);
            UIEffectManager.PauseAllEffects(handler);
