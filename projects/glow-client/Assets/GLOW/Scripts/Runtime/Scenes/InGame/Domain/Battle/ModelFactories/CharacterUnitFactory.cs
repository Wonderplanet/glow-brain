...
namespace GLOW.Scenes.InGame.Domain.Battle
{
    public class CharacterUnitFactory : ICharacterUnitFactory
    {
        ...
        [Inject] GLOW.Debugs.InGame.Domain.UseCases.DebugApplySpecialAttackTemplateUseCase DebugApplySpecialAttackTemplateUseCase { get; }
#endif
            ...
            MstPageModel page)
        {
            ...
            if(fetchOther.TutorialStatus.IsIntroduction())
            {
                // 導入チュートリアル中はユーザーユニットを上書きする
                // NOTE: 実際にチュートリアル中キャラ詳細はLevel80 Rank5 Grade5で固定されているため齟齬がある状態だが、
                // ここも変えてしまうと導入チュートリアルの流れに影響が出るため、ここは変更しない
        ...
        {
            var currentTickCount = InGameScene.StageTimeModel.CurrentTickCount;
            var deckUnit = GetDeckUnit(mstCharacter, side);
            ...
            var statusWithBonus = UnitStatusCalculator.CalculateStatus(
                ...
                !deckUnit.IsEmptyUnit() ? deckUnit.ArtworkEffectHpBonus : PercentageM.Hundred,
                !deckUnit.IsEmptyUnit() ? deckUnit.ArtworkEffectAttackPowerBonus : PercentageM.Hundred);
...
#if GLOW_INGAME_DEBUG
            // デバッグ設定があれば上書き
            specialAttackData = DebugApplySpecialAttackTemplateUseCase.Execute(mstCharacter.Id, side, specialAttackData);
#endif
            ...
            // 常時発動する原画効果の状態変化を付与する
            var artworkEffectDisplayOnlyStateEffects = GetArtworkEffectDisplayOnlyStateEffects(deckUnit);
            stateEffects.AddRange(artworkEffectDisplayOnlyStateEffects);
        ...
        List<IStateEffectModel> GetArtworkEffectDisplayOnlyStateEffects(DeckUnitModel deckUnit)
        {
            var artworkEffectDisplayOnlyStateEffects = new List<IStateEffectModel>();

            if (deckUnit.IsEmpty())
            {
                return artworkEffectDisplayOnlyStateEffects;
            }

            if (deckUnit.ArtworkEffectHpBonus > PercentageM.Hundred)
            {
                var stateEffectSourceId = StateEffectSourceIdProvider.GenerateNewId();
                var displayOnlyEffect = StateEffectModelFactory.CreateDisplayOnly(
                    stateEffectSourceId,
                    StateEffectType.HpUp);
                artworkEffectDisplayOnlyStateEffects.Add(displayOnlyEffect);
            }
        ...
        DeckUnitModel GetDeckUnit(MstCharacterModel mstCharacter, BattleSide side)
        {
            if (side == BattleSide.Player)
            {
                return InGameScene.DeckUnits.FirstOrDefault(unit => unit.CharacterId == mstCharacter.Id,
                    DeckUnitModel.Empty);
            }
            else
            {
                return InGameScene.PvpOpponentDeckUnits.FirstOrDefault(unit => unit.CharacterId == mstCharacter.Id,
                    DeckUnitModel.Empty);
            }
        }
    }
