...
{
    public class CharacterUnitFreezeAction : ICharacterUnitAction
    {
        static readonly UnitActionState[] NotChangeToStates = new[] { UnitActionState.Stun, UnitActionState.Freeze };
        ...
        public UnitActionState ActionState => UnitActionState.Freeze;
        ...
        public bool CanForceChangeTo(UnitActionState actionState) => !NotChangeToStates.Contains(actionState);

        public CharacterUnitFreezeAction(
        ...
        {
            Profiler.BeginSample("CharacterUnitFreezeAction.Update");
            ...
            if (_isActionStart)
            {
                var freezeStateEffectModel = CreateFreezeStateEffectModel(
                    context.StateEffectModelFactory,
                    _unitEffectSourceId,
                    _duration);

                updatedStateEffects = updatedStateEffects.ToList().ChainAdd(freezeStateEffectModel);
            }

            // StateEffectが解除されている場合、UnitActionも解除(RemoveDebuff考慮)
            if (!updatedStateEffects.Any(effect =>
                effect.SourceId == _unitEffectSourceId &&
                effect.Type == StateEffectType.Freeze))
            {
                Profiler.EndSample();
                return ReturnResult(myUnit,
                    remainingAttackInterval,
                    UnitMoveActionFactory.Create(myUnit.IsMoveStarted),
                    updatedStateEffects);
            }

            // Action終了時に氷結の状態変化を解除
        ...
        ICharacterUnitAction CreateNextAction(TickCount remainingDuration, bool isMoveStarted)
        {
            if (!remainingDuration.IsZero())
            {
                return new CharacterUnitFreezeAction(
                    _startStateTime,
                    _duration,
                    _unitEffectSourceId,
