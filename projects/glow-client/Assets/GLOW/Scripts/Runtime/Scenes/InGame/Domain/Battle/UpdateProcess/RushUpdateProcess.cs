    ...
    public class RushUpdateProcess : IRushUpdateProcess
    {
            ...
            ExecuteRushFlag canExecuteRush)
        {
            ...
            if (rushModel.ExecuteRushFlag && canExecuteRush)
            {
                ...
                // 原画効果ダメージボーナス
                var artworkEffectBonus = PercentageM.Hundred;
                foreach (var effectElement in artworkEffect.EffectElements)
                {
                    if (effectElement.EffectType == ArtworkEffectType.JumbleRushDamageUp &&
                        ArtworkEffectActivationEvaluator.EvaluateActivation(
                            ...
                            deckUnits))
                    {
                        artworkEffectBonus += effectElement.EffectValue.ToPercentageM();
                    }
                }

                // 計算式
                // バフ・デバフ込みの攻撃力合計 × ボーナス合計 × 総攻撃の威力アップ効果合計 × 原画効果 × 総攻撃ダメージ係数
                var calculatedAttackPower =
                    totalFieldUnitAttackPower * totalBonus * stateEffectBonus * artworkEffectBonus * rushModel.Coefficient;
