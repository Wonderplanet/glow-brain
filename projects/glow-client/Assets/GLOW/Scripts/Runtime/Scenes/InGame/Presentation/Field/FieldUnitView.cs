...
namespace GLOW.Scenes.InGame.Presentation.Field
{
    public class FieldUnitView : MonoBehaviour, IFieldViewPagePositionTrackerTarget, IKomaShakeTrackClipDelegate
    {
        ...
        BaseBattleEffectView _unbeatableEffect;
        ...
        public async UniTask OnDead(bool isAnimation, UnitDeathType deathType, CancellationToken cancellationToken)
        {
            ...
            if (_unbeatableEffect != null)
            {
                _unbeatableEffect.Destroy();
                _unbeatableEffect = null;
            }
        ...
        // 即時効果のエフェクト・SE再生
        // RemoveBuff/RemoveDebuffなど、StateEffectを付与せずに盤面上キャラにエフェクトを出す場合に使用
        public void OnImmediateEffect(StateEffectType stateEffectType)
        {
            switch (stateEffectType)
            {
                case StateEffectType.RemoveBuff:
                    BattleEffectManager
                        .Generate(BattleEffectId.RemoveBuff, transform)
                        ?.BindCharacterUnit(this)
                        ?.Play();
                ...
                case StateEffectType.RemoveDebuff:
                    BattleEffectManager
                        .Generate(BattleEffectId.RemoveDebuff, transform)
                        ?.BindCharacterUnit(this)
                        ?.Play();
        ...
        {
            // 全ての攻撃がダメージモーションを再生しない場合は処理を抜ける
            if (appliedAttackResultModels.All(res => res.IsDamageMotionUnplayable))
            {
                return;
            }
            ...
            IReadOnlyList<IStateEffectModel> newStateEffects)
        {
            ...
            var existsUnbeatable = additions.Any(stateEffect => stateEffect.Type == StateEffectType.Unbeatable);
            if (existsUnbeatable)
            {
                // 無敵
                if (_unbeatableEffect == null)
                {
                    _unbeatableEffect = BattleEffectManager
                        .Generate(BattleEffectId.Unbeatable, transform)
                        ?.BindCharacterUnit(this)
                        .Play();
                }
            }
            else if (_unbeatableEffect != null)
            {
                if (newStateEffects.All(stateEffect => stateEffect.Type != StateEffectType.Unbeatable))
                {
                    _unbeatableEffect.Destroy();
                    _unbeatableEffect = null;
                }
        ...
        void GenerateAttackEffect(UnitAttackViewInfo attackViewInfo)
        {
            ...
            if (mangaEffect != null)
            {
                _attackMangaEffect = _pageComponent
                    .GenerateMangaEffect(mangaEffect, FieldViewPos, Id,false)
                    ?.Play();
            }
        }
