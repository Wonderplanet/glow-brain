...
namespace GLOW.Scenes.InGame.Presentation.Field
{
    public class FieldUnitView : MonoBehaviour, IFieldViewPagePositionTrackerTarget, IKomaShakeTrackClipDelegate
    {
            ...
            IReadOnlyList<IStateEffectModel> newStateEffects)
        {
            ...
            var existsNonConditionalBuff = additions.Any(
                stateEffect => stateEffect.Type.IsBuff() && 
                               !stateEffect.Type.IsUnitConditionBuff());
            if (existsNonConditionalBuff)
            {
                BattleEffectManager
            ...
            var existsDebuff = additions.Any(stateEffect => stateEffect.Type.IsDebuff());
            if (existsDebuff)
            {
                BattleEffectManager
