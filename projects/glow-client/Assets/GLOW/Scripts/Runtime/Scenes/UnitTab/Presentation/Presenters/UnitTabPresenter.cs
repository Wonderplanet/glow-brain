...
namespace GLOW.Scenes.UnitTab.Presentation.Presenters
{
    public class UnitTabPresenter : IUnitTabViewDelegate, IUnitTabDelegate
    {
        ...
        [Inject] IArtworkEffectTutorialContext ArtworkEffectTutorialContext { get; }
        ...
        void IUnitTabViewDelegate.ViewWillAppear()
        {
            DoAsync.Invoke(
                ViewController.ActualView.GetCancellationTokenOnDestroy(),
                async (_) =>
                {
                    await ArtworkEffectTutorialContext.DoIfTutorial(() => UniTask.CompletedTask);
                });
        }
        ...
        void IUnitTabViewDelegate.ArtworkFormationTabSelect()
        {
            SwitchContent(UnitTabType.ArtworkFormation);
        }

        void IUnitTabViewDelegate.ArtworkListTabSelect()
        {
            SwitchContent(UnitTabType.ArtworkList);
        }
        ...
        void SetBackgroundRectTop(UnitTabType type)
        {
            switch (type)
            {
                ...
                case UnitTabType.UnitList:
                    ...
                    break;
                case UnitTabType.ArtworkList:
                    ViewController.SetBackgroundRectTop(40);
                    break;
                case UnitTabType.ArtworkFormation:
                    ViewController.SetBackgroundRectTop(40);
                    break;
            }
        }
        ...
        UIViewController CreateContentViewController(UnitTabType tabType)
        {
            return tabType switch
            {
                ...
                UnitTabType.ArtworkList => ViewFactory.Create<ArtworkListViewController>(),
                UnitTabType.ArtworkFormation => ViewFactory.Create<ArtworkFormationViewController>(),
