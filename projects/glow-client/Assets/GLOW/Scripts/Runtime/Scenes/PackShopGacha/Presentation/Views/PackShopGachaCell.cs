...
namespace GLOW.Scenes.PackShopGacha.Presentation.Views
{
    public class PackShopGachaCell : UICollectionViewCell
    {
        ...
        public void Setup(PackShopGachaCellViewModel model, Action<MasterDataId, GachaType> onTappedBannerAction)
        {
            UIBannerLoaderEx.Main.LoadBannerWithFadeIfNotLoaded(_bannerRawImage, model.GachaBannerAssetPath.Value);
            _bannerButton.onClick.AddListener(() => onTappedBannerAction?.Invoke(model.GachaId, model.GachaType));
            _infoButton.onClick.AddListener(() => onTappedBannerAction?.Invoke(model.GachaId, model.GachaType));
        }
    }
}