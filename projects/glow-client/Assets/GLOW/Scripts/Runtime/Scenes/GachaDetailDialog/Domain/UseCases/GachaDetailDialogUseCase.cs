...
namespace GLOW.Scenes.GachaDetailDialog.Domain.UseCases
{
    public class GachaDetailDialogUseCase
    {
            ...
            MasterDataId oprGachaId)
        {
            ...
            // キャッシュ無効化のためにクエリパラメータを追加（Emptyの場合はそのまま返す）
            var arrangedAnnouncementContentsUrl = announcementContentsUrl.IsEmpty()
                ? AnnouncementContentsUrl.Empty
                : new AnnouncementContentsUrl(ZString.Format("{0}?v={1}", announcementContentsUrl.Value, cacheBusterValue));
            var arrangedGachaAttentionContentsUrl = gachaAttentionContentsUrl.IsEmpty()
                ? GachaCautionContentsUrl.Empty
                : new GachaCautionContentsUrl(ZString.Format("{0}?v={1}", gachaAttentionContentsUrl.Value, cacheBusterValue));
            
            return new GachaDetailDialogUseCaseModel(
                arrangedAnnouncementContentsUrl,
                arrangedGachaAttentionContentsUrl);
        }
