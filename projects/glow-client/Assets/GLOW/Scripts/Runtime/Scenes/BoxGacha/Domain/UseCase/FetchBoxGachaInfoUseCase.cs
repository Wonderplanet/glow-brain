...
namespace GLOW.Scenes.BoxGacha.Domain.UseCase
{
    public class FetchBoxGachaInfoUseCase
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var decoUnitFirstAssetPath = UnitImageAssetPath.Empty;
            if (!mstBoxGachaModel.MstDisplayUnitIdFirst.IsEmpty())
            {
                var decoUnitFirst = MstCharacterDataRepository.GetCharacter(mstBoxGachaModel.MstDisplayUnitIdFirst);
                decoUnitFirstAssetPath = UnitImageAssetPath.FromAssetKey(decoUnitFirst.AssetKey);
            }
            
            var decoUnitSecondAssetPath = UnitImageAssetPath.Empty;
            if (!mstBoxGachaModel.MstDisplayUnitIdSecond.IsEmpty())
            {
                var decoUnitSecond = MstCharacterDataRepository.GetCharacter(mstBoxGachaModel.MstDisplayUnitIdSecond);
                decoUnitSecondAssetPath = UnitImageAssetPath.FromAssetKey(decoUnitSecond.AssetKey);
            }

            var model = new BoxGachaTopModel(
                mstBoxGachaModel.Id,
                mstBoxGachaModel.Name,
                decoUnitFirstAssetPath, 
                decoUnitSecondAssetPath,
