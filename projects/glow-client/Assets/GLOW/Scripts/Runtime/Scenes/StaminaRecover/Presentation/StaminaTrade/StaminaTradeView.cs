...
namespace GLOW.Scenes.StaminaRecover.Presentation.StaminaTrade
{
    public class StaminaTradeView : UIView
    {
        ...
        const string _confirmTextFormat = "{0}を{1}個使用してスタミナを{2}回復しますか？";
        ...
        public void Setup(StaminaTradeViewModel viewModel, Action updateAction)
        {
            ...
            SetConfirmText(viewModel.Name,
                _amountSelectionComponent.Amount,
                viewModel.EffectValue,
        ...
        public void SetConfirmText(ItemName itemName, ItemAmount selectedAmount, Stamina effectValue, Stamina maxStamina)
        {
            var staminaRecoveryValue = effectValue.Value * selectedAmount.Value;
            if(staminaRecoveryValue > maxStamina.Value) staminaRecoveryValue = maxStamina.Value;

            _confirmText.SetText(
                _confirmTextFormat,
                itemName.Value,
