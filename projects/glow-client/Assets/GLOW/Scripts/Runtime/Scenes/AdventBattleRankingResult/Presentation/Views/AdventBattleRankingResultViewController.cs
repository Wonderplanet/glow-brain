...
namespace GLOW.Scenes.AdventBattleRankingResult.Presentation.Views
{
        ...
        IAsyncActivityControl
    {
        ...
        {
            await ActualView.PlayRankAnimation(viewModel.Rank, isRaidBattle, cancellationToken);
            if (!isRaidBattle)
            {
                // スコアチャレンジのランキング結果の場合は、報酬を表示させる
                // 協力バトルの場合、次に協力バトルの結果画面を表示させる時に報酬を表示するためここでは報酬を表示しない
