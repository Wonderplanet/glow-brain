...
namespace GLOW.Scenes.AdventBattleRankingResult.Presentation.Views
{
    ...
    public class AdventBattleRankingResultView : UIView
    {
        ...
        static readonly int BoolIsRewardDisplay = Animator.StringToHash("IsRewardDisplay");
        ...
        public async UniTask PlayRankAnimation(AdventBattleRankingRank ranking, bool isRaidBattle, CancellationToken cancellationToken)
        {
            ...
            _animator.SetBool(BoolIsRewardDisplay, !isRaidBattle);
        ...
        async UniTask WaitForAnimationEnd(Animator animator, int layerIndex, CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                var isInLastState = stateInfo.IsName("RankingResultPanel@Loop") ||
                                    stateInfo.IsName("RankingResultPanel@Loop_NoExpandReward");
                if (isInLastState && !animator.IsInTransition(layerIndex))
                {
                    break;
                }
