...
namespace GLOW.Scenes.EventQuestSelect.Presentation
{
    ...
    public class EventQuestSelectPresenter : IEventQuestSelectViewDelegate
    {
        ...
        [Inject] EventQuestBadgeUseCase EventQuestBadgeUseCase { get; }
        ...
        [Inject] ShowArtworkPanelMissionListUseCase ShowArtworkPanelMissionListUseCase { get; }
        [Inject] IArtworkPanelMissionWireFrame ArtworkPanelMissionWireFrame { get; }
        [Inject] IEventQuestTutorialContext EventQuestTutorialContext { get; }
        ...
        void IEventQuestSelectViewDelegate.UpdateEventQuestSelectBadge()
        {
            UpdateEventQuestSelectBadge();
        }
        ...
        void IEventQuestSelectViewDelegate.OnMissionButtonTapped()
        {
            ...
            EventMissionWireFrame.ShowEventMissionViewInEvent(ViewController, Argument.MstEventId, UpdateEventQuestSelectBadge);
        }
        ...
        void IEventQuestSelectViewDelegate.OnArtworkPanelMissionButtonTapped()
        {
            if (!EventOpenCheckUseCase.IsOpenEvent(Argument.MstEventId))
            {
                EventQuestWireFrame.BackToHomeTop();
            ...
            DoAsync.Invoke(ViewController.ActualView, ScreenInteractionControl, async cancellationToken =>
            {
                var model = await ShowArtworkPanelMissionListUseCase.FetchAndUpdateArtworkPanelMissionModel(
                    Argument.MstEventId,
                    cancellationToken);
                var viewModel = ArtworkPanelMissionViewModelTranslator.ToViewModel(model);
                ArtworkPanelMissionWireFrame.ShowArtworkPanelMissionView(viewModel);
            });
        }

        void UpdateEventQuestSelectBadge()
        {
            var badgeModel = EventQuestBadgeUseCase.GetEventQuestSelectBadge(Argument.MstEventId); 
            var badgeViewModel = EventQuestSelectBadgeViewModelTranslator.ToViewModel(badgeModel);
            ViewController.SetEventQuestSelectBadge(badgeViewModel);
        }
        
        void EventQuestTutorialIfNeeded()
        {
            DoAsync.Invoke(
                ViewController.ActualView,
                async cancellationToken =>
                {
                    await EventQuestTutorialContext.DoIfTutorial(() => UniTask.CompletedTask);
                });
        }
    }
}
