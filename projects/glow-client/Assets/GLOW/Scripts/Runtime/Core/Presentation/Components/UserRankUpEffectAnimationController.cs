...
{
    public class UserRankUpEffectAnimationController : UIBehaviour
    {
        ...
        public async UniTask PlayAnimation(CancellationToken cancellationToken)
        {
            ...
            await WaitLogoAnimationPlaying(cancellationToken);
        }

        async UniTask WaitLogoAnimationPlaying(CancellationToken cancellationToken)
        {
            await UniTask.WaitUntil(() =>  _logoAnimator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 0.03f, 
