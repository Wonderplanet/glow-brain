...
namespace GLOW.Core.Data.Services
{
    public class PartyService : IPartyService
    {
        ...
        public async UniTask<ArtworkPartySaveResultModel> ArtworkSave(CancellationToken cancellationToken, IReadOnlyList<MasterDataId> mstArtworkIds)
        {
            try
            {
                var artworkIdStrings = mstArtworkIds
                    .Where(id => !id.IsEmpty())
                    .Select(id => id.ToString())
                    .ToArray();
                var artworkPartySaveResultData = await PartyApi.ArtworkSave(cancellationToken, artworkIdStrings);
                return TranslateToArtworkResultModel(artworkPartySaveResultData);
            }
            catch (ServerErrorException se)
            {
                throw ServerErrorExceptionMapper.Map(se);
        ...
        ArtworkPartySaveResultModel TranslateToArtworkResultModel(ArtworkPartySaveResultData data)
        {
            var artworkParty = UserArtworkPartyDataTranslator.ToModel(data.UsrArtworkParty);
            return new ArtworkPartySaveResultModel(artworkParty);
        }
    }
}
