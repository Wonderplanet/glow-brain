...
namespace GLOW.Core.Data.Services
{
    public class EncyclopediaService : IEncyclopediaService
    {
        ...
        public async UniTask<ArtworkGradeUpRewardResultModel> ArtworkGradeUp(
            CancellationToken cancellationToken,
            MasterDataId mstArtworkId)
        {
            var result = await EncyclopediaApi.ArtworkGradeUp(
                cancellationToken,
                mstArtworkId.Value);
        ...
        ArtworkGradeUpRewardResultModel Translate(ArtworkGradeUpResultData data)
        {
            var userArtwork = UserArtworkDataTranslator.ToUserArtworkModel(data.UsrArtwork);

            var userItems = data.UsrItems
                .Select(ItemDataTranslator.ToUserItemModel)
                .ToList();

            return new ArtworkGradeUpRewardResultModel(userArtwork, userItems);
        }
