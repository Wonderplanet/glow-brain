    ...
    public class GachaPrizeResultDataTranslator
    {
        public static GachaPrizeResultModel ToGachaDrawResultModel(GachaPrizeResultData data)
        {
            GachaPrizeResultModel gachaDrawResultModel = new GachaPrizeResultModel(
                ...
                ToStepupGachaPrizeInfo(data.StepupGachaPrizes)
            );
        ...
        {
            if (data.UpperProbabilities.Length == 0
                || data.UpperProbabilities.All(upper => upper.UpperType != UpperType.MaxRarity))
            {
                return GachaPrizePageModel.Empty;
            }
        ...
        {
            if (data.UpperProbabilities.Length == 0
                || data.UpperProbabilities.All(upper => upper.UpperType != UpperType.Pickup))
            {
                return GachaPrizePageModel.Empty;
            }
        ...
        // ステップアップガチャ
        static StepupGachaPrizeResultModel ToStepupGachaPrizeInfo(StepupGachaPrizeInfoData[]? data)
        {
            if (data == null || data.Length == 0) return StepupGachaPrizeResultModel.Empty;

            var stepModels = data
                .OrderBy(step => step.StepNumber)
                .Select(ToStepupGachaPrizeStepModel)
                .ToList();

            return new StepupGachaPrizeResultModel(stepModels);
        }

        static StepupGachaPrizeStepModel ToStepupGachaPrizeStepModel(StepupGachaPrizeInfoData data)
        {
            var normalPrizePageModel = new GachaPrizePageModel(
                data.RarityProbabilities.Select(ToGachaRarityProbabilityModel).ToArray(),
                data.ProbabilityGroups.Select(ToGachaProbabilityGroupModel).ToArray());

            var fixedPrizePageModel = new GachaPrizePageModel(
                data.FixedProbabilities.RarityProbabilities.Select(ToGachaRarityProbabilityModel).ToArray(),
                data.FixedProbabilities.ProbabilityGroups.Select(ToGachaProbabilityGroupModel).ToArray());

            return new StepupGachaPrizeStepModel(
                new StepUpGachaStepNumber(data.StepNumber),
                normalPrizePageModel,
                fixedPrizePageModel);
        }
