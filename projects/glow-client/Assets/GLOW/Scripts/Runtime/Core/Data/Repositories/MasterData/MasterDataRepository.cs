...
namespace GLOW.Core.Data.Repositories
{
    public class MasterDataRepository : BaseMstRepository,
        ...
        IMstUnitGradeUpRewardRepository,
        ...
        IOprStepUpGachaRepository,
        IOprStepUpGachaStepRepository,
        IOprStepUpGachaStepRewardRepository,
        ...
        IMstArtworkAcquisitionRouteRepository,
        IMstBoxGachaDataRepository,
        IMstArtworkGradeUpDataRepository,
        IMstArtworkEffectDescriptionDataRepository,
        IMstHomeKomaPatternRepository
    {
        ...
        #region IMstUnitGradeUpRewardRepository
        MstUnitGradeUpRewardModel IMstUnitGradeUpRewardRepository.GetUnitGradeUpRewardFirstOrDefault(
            MasterDataId mstUnitId)
        {
            return GetOrCreateModelCache(CreateMstUnitGradeUpRewardModels)
                .FirstOrDefault(m => m.MstUnitId == mstUnitId, MstUnitGradeUpRewardModel.Empty);
        }

        IReadOnlyList<MstUnitGradeUpRewardModel> CreateMstUnitGradeUpRewardModels()
        {
            return DataStore.Get<MstUnitGradeUpRewardData>()
                .Select(MstUnitDataTranslator.TranslateGradeUpReward)
                .ToList();
        }
        ...
        IReadOnlyList<MstArtworkPanelMissionModel> IMstArtworkPanelMissionDataRepository.GetMstArtworkPanelMissionModels(
            MasterDataId mstEventId)
        {
            return GetOrCreateModelCache(CreateMissionArtworkPanelModels)
                .Where(model => model.MstEventId == mstEventId)
                .ToList();
        }

        IReadOnlyList<MstArtworkPanelMissionModel> CreateMissionArtworkPanelModels()
        {
            return DataStore.Get<MstArtworkPanelMissionData>()
                .Select(MstArtworkPanelMissionDataTranslator.ToModel)
                .ToList();
        }
        ...
        MstInGameArtworkEffectModel IMstArtworkEffectRepository.GetMstInGameArtworkEffectModelFirstOrDefault(MasterDataId artworkId)
        {
            return GetOrCreateModelCache(CreateMstInGameArtworkEffectModels)
                .FirstOrDefault(mst => mst.MstArtworkId == artworkId, MstInGameArtworkEffectModel.Empty);
        }

        IReadOnlyList<MstInGameArtworkEffectModel> CreateMstInGameArtworkEffectModels()
        {
            var targetRulesGrouped = DataStore.Get<MstArtworkEffectTargetRuleData>()
                .GroupBy(data => data.MstArtworkEffectId, data => data);

            var activationRulesGrouped = DataStore.Get<MstArtworkEffectActivationRuleData>()
                .GroupBy(data => data.MstArtworkEffectId, data => data);

            var effectDataGrouped = DataStore.Get<MstArtworkEffectData>()
                .GroupBy(data => data.MstArtworkId, data => data);

            return effectDataGrouped
                .Select(
                    artworkEffectGroup =>
                    {
                        var effectModels = artworkEffectGroup
                            .GroupJoin(
                                targetRulesGrouped,
                                effect => effect.Id,
                                targetGroup => targetGroup.Key,
                                (effect, targetRules) => new { effect, targetRules = targetRules.FirstOrDefault() })
                            .GroupJoin(
                                activationRulesGrouped,
                                data => data.effect.Id,
                                activationGroup => activationGroup.Key,
                                (data, activationRules) => new
                                {
                                    data.effect,
                                    data.targetRules,
                                    activationRules = activationRules.FirstOrDefault()
                                })
                            .Select(
                                data =>
                                {
                                    var targetRulesList = data.targetRules != null
                                        ? data.targetRules.ToList()
                                        : new List<MstArtworkEffectTargetRuleData>();
                                    var activationRulesList = data.activationRules != null
                                        ? data.activationRules.ToList()
                                        : new List<MstArtworkEffectActivationRuleData>();

                                    return MstArtworkEffectDataTranslator.TranslateEffect(
                                        data.effect,
                                        targetRulesList,
                                        activationRulesList);
                                })
                        ...
                        return MstArtworkEffectDataTranslator.TranslateInGameModel(
                            new MasterDataId(artworkEffectGroup.Key),
                            effectModels);
                    })
                .ToList();
        }
        ...
        {
            return GetOrCreateModelCache(CreateOprGachaModels)
                .FirstOrDefault(m => m.Id == gachaId, OprGachaModel.Empty);
        }
        ...
        #region IMstBoxGachaDataRepository
        MstBoxGachaModel IMstBoxGachaDataRepository.GetMstBoxGachaModelByMstEventIdFirstOrDefault(MasterDataId mstEventId)
        {
            return GetOrCreateModelCache(CreateMstBoxGachaModels)
                .FirstOrDefault(model => model.MstEventId == mstEventId, MstBoxGachaModel.Empty);
        }

        MstBoxGachaModel IMstBoxGachaDataRepository.GetMstBoxGachaModelFirstOrDefault(MasterDataId mstBoxGachaId)
        {
            return GetOrCreateModelCache(CreateMstBoxGachaModels)
                .FirstOrDefault(model => model.Id == mstBoxGachaId, MstBoxGachaModel.Empty);
        }

        IReadOnlyList<MstBoxGachaModel> CreateMstBoxGachaModels()
        {
            var mstBoxGacha = DataStore.Get<MstBoxGachaData>();
            var mstBoxGachaI18n = DataStore.Get<MstBoxGachaI18nData>();

            var mstBoxGachaModels = mstBoxGacha
                .Join(
                    mstBoxGachaI18n,
                    boxGacha => boxGacha.Id,
                    i18n => i18n.MstBoxGachaId,
                    (boxGacha, i18n) => (boxGacha, i18n))
                .Select(dataAndI18n => MstBoxGachaDataTranslator.Translate(dataAndI18n.boxGacha, dataAndI18n.i18n))
                .ToList();

            return mstBoxGachaModels;
        }

        MstBoxGachaGroupModel IMstBoxGachaDataRepository.GetMstBoxGachaGroupModelFirstOrDefault(
            MasterDataId mstBoxGachaId,
            BoxLevel boxLevel)
        {
            return GetOrCreateModelCache(CreateMstBoxGachaGroupModels)
                .Where(model => model.MstBoxGachaId == mstBoxGachaId)
                .FirstOrDefault(model => model.BoxLevel == boxLevel, MstBoxGachaGroupModel.Empty);
        }

        IReadOnlyList<MstBoxGachaGroupModel> CreateMstBoxGachaGroupModels()
        {
            var mstBoxGachaGroup = DataStore.Get<MstBoxGachaGroupData>();
            var mstBoxGachaPrize = DataStore.Get<MstBoxGachaPrizeData>();

            var mstBoxGachaGroupModels = mstBoxGachaGroup
                .GroupJoin(
                    mstBoxGachaPrize,
                    boxGachaGroup => boxGachaGroup.Id,
                    boxGachaPrize => boxGachaPrize.MstBoxGachaGroupId,
                    (boxGachaGroup, boxGachaPrize) => (boxGachaGroup, boxGachaPrize))
                .Select(data => MstBoxGachaGroupDataTranslator.Translate(
                    data.boxGachaGroup,
                    data.boxGachaPrize.ToList()))
                .ToList();

            return mstBoxGachaGroupModels;
        }

        #endregion


        #region IMstHomeKomaPatternRepository

        IReadOnlyList<MstHomeKomaPatternModel> IMstHomeKomaPatternRepository.GetHomeKomaPatterns()
        {

            return GetOrCreateModelCache(CreateGetHomeKomaPatterns)
                .ToList();

        }

        IReadOnlyList<MstHomeKomaPatternModel> CreateGetHomeKomaPatterns()
        {
            var i18ns = DataStore.Get<MstHomeKomaPatternI18nData>();

            return DataStore.Get<MstHomeKomaPatternData>()
                .Join(
                    i18ns,
                    data => data.Id,
                    i18n => i18n.MstHomeKomaPatternId,
                    (data, i18n) => (data, i18n))
                .Select(dataAndI18n => MstHomeKomaPatternDataTranslator.Translate(dataAndI18n.data, dataAndI18n.i18n))
                .ToList();
        }

        #region OprStepUpGacha

        OprStepUpGachaModel IOprStepUpGachaRepository.GetOprStepUpGachaModelFirstOrDefault(MasterDataId oprGachaId)
        {
            return GetOrCreateModelCache(CreateOprStepUpGachaModels)
                .FirstOrDefault(m => m.OprGachaId == oprGachaId, OprStepUpGachaModel.Empty);
        }

        IReadOnlyList<OprStepUpGachaModel> CreateOprStepUpGachaModels()
        {
            return DataStore.Get<OprStepupGachaData>()
                .Select(OprStepUpGachaDataTranslator.Translate)
                .ToList();
        }

        #endregion

        #region OprStepUpGachaStep

        IReadOnlyList<OprStepUpGachaStepModel> IOprStepUpGachaStepRepository.GetOprStepUpGachaModels(MasterDataId oprGachaId)
        {
            return GetOrCreateModelCache(CreateOprStepUpGachaStepModels)
                .Where(m => m.OprGachaId == oprGachaId)
                .OrderBy(m => m.StepNumber.Value)
                .ToList();
        }

        OprStepUpGachaStepModel IOprStepUpGachaStepRepository.GetOprStepUpGachaStepModelFirstOrDefault(
            MasterDataId oprGachaId,
            StepUpGachaStepNumber stepNumber)
        {
            return GetOrCreateModelCache(CreateOprStepUpGachaStepModels)
                .FirstOrDefault(
                    m => m.OprGachaId == oprGachaId && m.StepNumber.Value == stepNumber.Value,
                    OprStepUpGachaStepModel.Empty);
        }


        IReadOnlyList<OprStepUpGachaStepModel> CreateOprStepUpGachaStepModels()
        {
            var stepDatas = DataStore.Get<OprStepupGachaStepData>();
            var i18nDatas = DataStore.Get<OprStepupGachaStepI18nData>();

            return stepDatas
                .Join(
                    i18nDatas,
                    data => data.Id,
                    i18n => i18n.OprStepupGachaStepId,
                    (data, i18n) => OprStepUpGachaStepDataTranslator.Translate(data, i18n))
                .ToList();
        }

        #endregion

        #region OprStepUpGachaStepReward

        IReadOnlyList<OprStepUpGachaStepRewardModel> IOprStepUpGachaStepRewardRepository.GetOprStepUpGachaStepRewardModels(
            MasterDataId oprGachaId)
        {
            return GetOrCreateModelCache(CreateOprStepUpGachaStepRewardModels)
                .Where(m => m.OprGachaId == oprGachaId)
                .ToList();
        }


        IReadOnlyList<OprStepUpGachaStepRewardModel> CreateOprStepUpGachaStepRewardModels()
        {
            return DataStore.Get<OprStepupGachaStepRewardData>()
                .Select(OprStepUpGachaStepRewardDataTranslator.Translate)
                .ToList();
        }
        ...
        IReadOnlyList<MstArtworkAcquisitionRouteModel> IMstArtworkAcquisitionRouteRepository.GetArtworkAcquisitionRoutes()
        {
            return GetOrCreateModelCache(CreateArtworkAcquisitionRouteModels).ToList();
        }

        MstArtworkAcquisitionRouteModel IMstArtworkAcquisitionRouteRepository.GetArtworkAcquisitionRouteFirstOrDefault(MasterDataId mstArtworkId)
        {
            return GetOrCreateModelCache(CreateArtworkAcquisitionRouteModels)
                .FirstOrDefault(model => model.MstArtworkId == mstArtworkId, MstArtworkAcquisitionRouteModel.Empty);
        }

        IReadOnlyList<MstArtworkAcquisitionRouteModel> CreateArtworkAcquisitionRouteModels()
        {
            var data = DataStore.Get<MstArtworkAcquisitionRouteData>();
            var mstArtworkIds = data.Select(data => data.MstArtworkId).Distinct().ToList();

            return mstArtworkIds
                .GroupJoin(
                    data,
                    artworkId => artworkId,
                    routeData => routeData.MstArtworkId,
                    (artworkId, routes) => (artworkId, routes.ToList()))
                .Select(model =>
                    MstArtworkAcquisitionRouteDataTranslator.Translate(model.artworkId, model.Item2))
                .ToList();
        }

        MstArtworkGradeUpModel IMstArtworkGradeUpDataRepository.GetArtworkGradeUp(
            Rarity rarity,
            ArtworkGradeLevel gradeLevel,
            MasterDataId mstSeriesId,
            MasterDataId mstArtworkId)
        {
            var models = GetOrCreateModelCache(CreateArtworkGradeUpModels);

            var gradeUpModel = models
                .Where(model => model.MstArtworkId == mstArtworkId)
                .Where(model => model.MstSeriesId == mstSeriesId)    // 念の為シリーズIdでも絞る
                .FirstOrDefault(model => model.GradeLevel == gradeLevel.GetNextGradeLevel() && model.Rarity == rarity,
                    MstArtworkGradeUpModel.Empty);

            if (!gradeUpModel.IsEmpty()) return gradeUpModel;

            return models
                .Where(model => model.MstSeriesId == mstSeriesId)
                .FirstOrDefault(model => model.Rarity == rarity && model.GradeLevel == gradeLevel.GetNextGradeLevel(),
                    MstArtworkGradeUpModel.Empty);
        }

        IReadOnlyList<MstArtworkGradeUpModel> IMstArtworkGradeUpDataRepository.GetMstArtworkGradeUps(
            Rarity rarity,
            MasterDataId mstSeriesId,
            MasterDataId mstArtworkId)
        {
            ...
            var gradeUpModels = models
                .Where(model => model.MstArtworkId == mstArtworkId)
                .Where(model => model.MstSeriesId == mstSeriesId)    // 念の為シリーズIdでも絞る
                .Where(model => model.Rarity == rarity)
                .ToList();

            if(gradeUpModels.Any()) return gradeUpModels;
        ...
        IReadOnlyList<MstArtworkGradeUpModel> CreateArtworkGradeUpModels()
        {
            var datas = DataStore.Get<MstArtworkGradeUpData>();
            var costDatas = DataStore.Get<MstArtworkGradeUpCostData>();

            return datas
                .GroupJoin(
                    costDatas,
                    data => data.Id,
                    costData => costData.MstArtworkGradeUpId,
                    (data, costs) => MstArtworkGradeUpDataTranslator.Translate(data, costs.ToList()))
                .ToList();
        }

        MstArtworkEffectDescriptionModel IMstArtworkEffectDescriptionDataRepository.GetArtworkEffectDescriptionFirstOrDefault(MasterDataId mstArtworkId)
        {
            return GetOrCreateModelCache(CreateArtworkEffectDescriptionModels)
                .FirstOrDefault(model => model.MstArtworkId == mstArtworkId, MstArtworkEffectDescriptionModel.Empty);
        }

        IReadOnlyList<MstArtworkEffectDescriptionModel> CreateArtworkEffectDescriptionModels()
        {
            return DataStore.Get<MstArtworkEffectI18nData>()
                .Select(MstArtworkEffectDescriptionDataTranslator.Translate)
                .ToList();
        }
