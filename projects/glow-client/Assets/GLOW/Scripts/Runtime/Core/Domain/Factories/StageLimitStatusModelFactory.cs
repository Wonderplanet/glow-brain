...
namespace GLOW.Core.Domain.Factories
{
    public class StageLimitStatusModelFactory : IStageLimitStatusModelFactory
    {
            ...
            IReadOnlyList<MstCharacterModel> characterModels)
        {
            ...
            // PartyColor
            var colorStatus = limitStatusList.FirstOrDefault(model =>
                    model.Status == StageLimitPartyStatus.PartyColor, StageLimitStatus.Empty);
            if (!colorStatus.IsEmpty())
            {
                if (!colorStatus.CharacterColors.IsEmpty() &&
                    characterModels.Any(m => !colorStatus.CharacterColors.Contains(m.Color)))
                {
                    result.Add(colorStatus);
                }
            }
            ...
            PartyName partyName)
        {
            ...
            // PartyColor
            var colors = inGameSpecialRules
                .Where(model => model.RuleType == RuleType.PartyColor)
            ...
            if (!colors.IsEmpty())
            {
                result.Add(StageLimitStatus.Empty with
                {
                    Status = StageLimitPartyStatus.PartyColor,
