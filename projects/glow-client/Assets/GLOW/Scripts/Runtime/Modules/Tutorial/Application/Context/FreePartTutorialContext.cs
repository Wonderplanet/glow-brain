...
namespace GLOW.Modules.Tutorial.Application.Context
{
    public class FreePartTutorialContext : ITutorialFreePartContext, IDisposable, IFreePartTutorialPlayingStatus
    {
        ...
        [Inject] PlaceholderFactory<ReleaseHomeCreateTutorialSequence> TransitHomeCreateTutorialSequenceFactory { get; }
        [Inject] PlaceholderFactory<ReleaseArtworkEffectTutorialSequence> TransitArtworkEffectTutorialSequenceFactory { get; }
        ...
        async UniTask<bool> PlaySequenceIfNeeds()
        {
            ...
            if (!gameFetchOther.TutorialStatus.IsCompleted()) return false;
            ...
            if (CanBeginTutorial(TutorialFreePartIdDefinitions.ReleaseHomeCreate))
            {
                // 1.ホームコマ設定チュートリアル
                using var sequence = TransitHomeCreateTutorialSequenceFactory.Create();
                await sequence.Play(_cancellationTokenSource.Token);
                executed = true;
            }
            else if (CanBeginTutorial(TutorialFreePartIdDefinitions.ReleaseArtworkEffect))
            {
                // 2. 原画強化チュートリアル
                using var sequence = TransitArtworkEffectTutorialSequenceFactory.Create();
                await sequence.Play(_cancellationTokenSource.Token);
                executed = true;
            }
            ...
            {
                // 4.コイン獲得クエストチュートリアル(イベントコンテンツ誘導)
