...
namespace GLOW.Modules.Tutorial.Presentation.Sequence
{
    ...
    /// チュートリアルステージ1 導入パート
    ///
    /// 導入チュートリアルはスキップがあるので、停止はクラス内のメソッドで停止する
    /// </summary>
        ...
        ITutorialAssetDownloadPresentUserApproval
    {
        ...
        public override async UniTask Play(CancellationToken token)
        {
            ...
            try
            {
                ...
                // 召喚チュートリアル
                await PlaySummonTutorial(cancellationToken, button1);
                
                // 必殺技チュートリアル
                await PlaySpecialAttackTutorial(cancellationToken, button1);
                
                // 属性チュートリアル
                await PlayAttributeTutorial(cancellationToken, tipModels, button2);
                
                // キャラ詳細チュートリアル
                await PlayUnitDetailTutorial(cancellationToken, longPressButton1);

                // ジャンブルラッシュチュートリアル
                await PlayJumbleRushTutorial(cancellationToken, rushGaugeButton, tipModels);
                ...
                await ShowTutorialText(cancellationToken, "けれど、今回助けてくれたキャラたちは、\nそれぞれの作品の世界を救うために\n帰らないといけないんだ…！", 0);
            ...
            await SkipIntroductionTutorialUseCase.UpdateTutorialStatusBySkip(afterTutorialCancellationToken);
        ...
        async UniTask PlaySummonTutorial(CancellationToken cancellationToken, Button button1)
        {
        ...
        async UniTask PlaySpecialAttackTutorial(CancellationToken cancellationToken, Button button1)
        {
        ...
        async UniTask PlayAttributeTutorial(
        ...
        async UniTask PlayUnitDetailTutorial(
            CancellationToken cancellationToken,
            UIButtonLongPress longPressButton1)
        {
            ...
            await ShowTutorialText(cancellationToken, "画面をタップすると\n詳細情報を閉じるよ", 0);
        ...
        async UniTask PlayJumbleRushTutorial(
            CancellationToken cancellationToken,
            Button rushGaugeButton,
            IReadOnlyList<TutorialTipModel> tipModels)
        {
            // ジャンブルラッシュ用の敵出現の待機
                await CustomDelay(cancellationToken, 4000);
                
                await PauseInGame(cancellationToken);
                await FadeInGrayOut(cancellationToken);
                
                await ShowTutorialText(cancellationToken, "ファントムの援軍が\n出現したぞ！", 0);
                await ShowTutorialText(cancellationToken, "「JUMBLE RUSH」で\nファントムを一掃しよう！", 0);
                
                // ジャンブルラッシュゲージのハイライト
                HighlightTarget("rush_gauge");
                await ShowTutorialText(cancellationToken, "「RUSHゲージ」がたまると\n「JUMBLE RUSH」が使えるよ！", 0f);
                await ShowTutorialText(cancellationToken, "今回はゲージをMAXにするよ！", 0f);
                await HideTutorialText(cancellationToken);
                
                // ジャンブルラッシュゲージをマックスにして、最大になったエフェクトが出るまで少し待つ
                SetFullRushGauge();
                ...
                await UniTask.Delay(650, cancellationToken: cancellationToken);
                ...
                await UniTask.Delay(7200, cancellationToken:cancellationToken);
            
                await PauseInGame(cancellationToken);
                await FadeInGrayOut(cancellationToken);
                
                // ジャンブルラッシュについて1
                ShowTutorialDialogWithNextButton(tipModels[4].Title, tipModels[4].AssetPath);
                await WaitDismissModal<TutorialTipDialogViewController>(cancellationToken);

                // ジャンブルラッシュについて2
                ShowTutorialDialog(tipModels[5].Title, tipModels[5].AssetPath);
                await WaitDismissModal<TutorialTipDialogViewController>(cancellationToken);
        }
