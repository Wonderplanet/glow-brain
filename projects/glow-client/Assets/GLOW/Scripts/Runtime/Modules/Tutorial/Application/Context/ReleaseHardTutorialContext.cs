...
{
    public class ReleaseHardTutorialContext :
        IReleaseHardTutorialContext,
        IReleaseHardTutorialPlayingStatus,
        IDisposable
    {
        [Inject] IGameRepository GameRepository { get; }
        [Inject] IMstTutorialRepository MstTutorialRepository { get; }
        [Inject] PlaceholderFactory<ReleaseHardTutorialSequence> SequenceFactory { get; }
        ...
        PlayingTutorialSequenceFlag IReleaseHardTutorialPlayingStatus.IsPlayingTutorialSequence => _isPlayingTutorialSequence;
        CancellationTokenSource _cancellationTokenSource = new();

        async UniTask<bool> IReleaseHardTutorialContext.DoIfTutorial(Func<UniTask> action)
        {
        ...
        async UniTask<bool> PlaySequenceIfNeeds()
        {
            ...
            if (CanBeginTutorial(TutorialFreePartIdDefinitions.ReleaseHardStage))
            {
                using var tutorialSequence = SequenceFactory.Create();
