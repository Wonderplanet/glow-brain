...
namespace GLOW.Debugs.AdminDebug.Presentation
{
    public sealed class AdminDebugViewPresenter : IAdminDebugViewDelegate
    {
        ...
        void IAdminDebugViewDelegate.OnSelectCommand(AdminDebugMenuCommandModel command)
        {
            ...
            {
                var inputViewController = new AdminDebugInputViewController()
                {
                    TempleteView = Context.Container.Resolve<AdminDebugInputView>()
                };

                Context.Container.Inject(inputViewController);

                var cellViewModels = command.RequiredParameters
                    .Select(parameter => new AdminDebugInputCellViewModel(
                        Name: parameter.Key,
                ...
                var viewModel = new AdminDebugInputViewModel(
                    Command: command.Command,
                    Name: command.Name,
                    Description: command.Description,
                    CellViewModels: cellViewModels);

                inputViewController.SetViewModel(viewModel);
                ViewController.PresentModally(inputViewController);
