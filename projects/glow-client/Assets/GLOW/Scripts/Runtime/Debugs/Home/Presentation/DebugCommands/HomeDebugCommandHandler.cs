...
using Zenject;
using ArtworkGradeLevel = GLOW.Core.Domain.ValueObjects.ArtworkGradeLevel;
...
namespace GLOW.Debugs.Home.Presentation.DebugCommands
{
    public sealed class HomeDebugCommandHandler
    {
        ...
        [Inject] DebugArtworkEffectUseCase DebugArtworkEffectUseCase { get; }
        ...
        public void CreateDebugCommandRootMenu(IDebugCommandPresenter debugCommandPresenter)
        {
            ...
            debugCommandPresenter.AddNestedMenuButton(
                ...
                debugCommandPresenter =>
                    UnitTemporaryParameterDebugCommand.CreateTemporaryParameterMenu(debugCommandPresenter,
                        () =>
                        {
                            StartStageForDebug(debugCommandPresenter,
                                GetAllStagesDebugUseCase.GetAllStages()[0].MstStageModel.Id);
                        }));
            debugCommandPresenter.AddNestedMenuButton(
                ">> ステージ",
            ...
            debugCommandPresenter.AddNestedMenuButton(
                ">> 原画効果マスターデータチェック",
                CreateArtworkEffectCheckSelectMenu);
        ...
        void CreateViewCheckMenu(IDebugCommandPresenter debugCommandPresenter)
        {
            debugCommandPresenter.AddButton(
                "汎用ダイアログ（2ボタン）表示",
                () =>
            ...
            debugCommandPresenter.AddButton(
                ...
                () =>
                {
                    ...
                    var artworkIcon = PlayerResourceModelFactory.Create(
                    ...
                    var artworkIconViewModel = PlayerResourceIconViewModelTranslator.ToPlayerResourceIconViewModel(artworkIcon);
                    ...
                    var viewModel = new BoxGachaResultViewModel(
                        new List<GachaResultCellViewModel>()
                        {
                            ...
                            new GachaResultCellViewModel(artworkIconViewModel, IsNewUnitBadge.Empty),
        ...
        void CreateArtworkEffectCheckSelectMenu(IDebugCommandPresenter debugCommandPresenter)
        {
            var models = DebugArtworkEffectUseCase.GetModel();
            foreach (var model in models.GetSortElementFromMstSeries())
            {
                debugCommandPresenter.AddButton(
                    model.GetNameString(),
                    () => ShowArtworkEffectDetailView(model));
            }
        }
        ...
        void ShowArtworkEffectDetailView(DebugArtworkEffectElementUseCaseModel useCaseModel)
        {
            var canvas = Context.Container.ResolveId<UICanvas>(FrameworkInjectId.Canvas.System);

            var controller = ViewFactory.Create<DebugArtworkEffectDetailViewController>();
            controller.Setup(useCaseModel);
            canvas.RootViewController.Show(controller);
        }
        ...
        void ShowAdventBattleRankingResultView(AdventBattleRankingRank rank)
        {
            ...
            var argument = new AdventBattleRankingResultViewController.Argument(
                viewModel, 
                false, 
                () => { });
        ...
        void CompleteFreePartTutorial()
        {
            ...
            DoAsync.Invoke(cancellationToken, ScreenInteractionControl, async ct =>
            {
                ...
                await CompleteFreePartTutorialUseCase.CompleteFreePartTutorial(ct,
                    TutorialFreePartIdDefinitions.ReleaseHomeCreate);
                await CompleteFreePartTutorialUseCase.CompleteFreePartTutorial(ct,
                    TutorialFreePartIdDefinitions.ReleaseArtworkEffect);
                await CompleteFreePartTutorialUseCase.CompleteFreePartTutorial(ct,
                    TutorialFreePartIdDefinitions.TransitHomeCreate);
                await CompleteFreePartTutorialUseCase.CompleteFreePartTutorial(ct,
                    TutorialFreePartIdDefinitions.TransitArtworkEffect);
            });
        }
    }
}
