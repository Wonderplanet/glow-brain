...
namespace GLOW.Debugs.InGame.Presentation.DebugCommands
{
    public class InGameDebugCommandHandler
    {
        ...
        [Inject] DebugGetSkillOperationUnitInfosUseCase DebugGetSkillOperationUnitInfosUseCase { get; }
        [Inject] DebugSaveSpecialAttackTemplateUseCase DebugSaveSpecialAttackTemplateUseCase { get; }
        [Inject] DebugApplySpecialAttackTemplateUseCase DebugApplySpecialAttackTemplateUseCase { get; }
        ...
        public void CreateDebugCommandRootMenu(IDebugCommandPresenter debugCommandPresenter)
        {
            ...
            // スキル操作
            var unitInfos = DebugGetSkillOperationUnitInfosUseCase.GetUnitInfos();
            SkillOperationDebugCommand.AddSkillOperationButton(
                debugCommandPresenter,
                unitInfos,
                (characterId, side, attackElementDataList) =>
                {
                    // 保存
                    DebugSaveSpecialAttackTemplateUseCase.Execute(characterId, side, attackElementDataList);

                    // 盤面ユニットに適用
                    ApplyToFieldUnits(characterId, side);

                    Toast.MakeText("必殺技を変更しました").Show();
                });
        ...
        // 盤面ユニット適用メソッド
        void ApplyToFieldUnits(MasterDataId characterId, BattleSide side)
        {
            var targetUnit = InGameScene.CharacterUnits.FirstOrDefault(
                unit => unit.CharacterId == characterId && unit.BattleSide == side,
                CharacterUnitModel.Empty);

            if (targetUnit.IsEmpty()) return;

            // Execute()を使って上書き
            var newSpecialAttack = DebugApplySpecialAttackTemplateUseCase.Execute(
                characterId,
                side,
                targetUnit.SpecialAttack);

            var newUnit = targetUnit with
            {
                SpecialAttack = newSpecialAttack
            };

            InGameScene.CharacterUnits = InGameScene.CharacterUnits.Replace(targetUnit, newUnit);
        }
    }
}
#endif
